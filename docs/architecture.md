...
This document owns current module boundaries and runtime composition. User behavior lives in [product](product.md), configuration and migrations in [settings](settings.md), transport contracts in [providers](providers.md), and open observations in [recovery boundaries](recovery-plan.md).

## Overview

The repository builds one readable BetterDiscord plugin from modular source. Its facade owns lifecycle entry points and delegates host wiring to the corresponding modules. The [composition-root decision](../.agents/notes/implemented/architecture/2026-09-17-composition-root-boundary.md) owns extraction rationale and constraints; build and client identity are verified through the [debugging cookbook](cookbook/debugging.md).

Translation display uses channel-scoped Store updates, independent body/reply revisions and bounded retry. Host lifecycle refreshes remain separate. This map describes current ownership, without certifying a particular client installation.
...
The build uses esbuild in CommonJS mode with an ES2020 target, preserves the BetterDiscord metadata banner, strips release-disabled probes, and embeds a deterministic build ID. Metadata ownership and release alignment are defined in [Contributing](../CONTRIBUTING.md#release-metadata).
...
| Legacy plugin facade and composition | `src/legacy/runtime.js` | Plugin lifecycle, BDFDB patch shell, public compatibility facade, and compact lazy singleton boundaries; new host fan-out belongs in owning wiring modules |
...
Transport and error coalescing belong to [provider contracts](providers.md); output-language compatibility and removed settings belong to [settings migration](settings.md#migration-rule).
...
Runtime queues, display state, viewport state, counters, probes, and generations are in memory only. Persistent setting semantics belong to [Settings](settings.md).
...
Use the commands and iteration loop in [Contributing](../CONTRIBUTING.md). Discord display changes require client evidence in addition to automated checks. The [composition-root decision](../.agents/notes/implemented/architecture/2026-09-17-composition-root-boundary.md) owns extraction constraints; [recovery boundaries](recovery-plan.md) owns remaining lifecycle, rendering, compatibility and module-size observations. Update both language versions when architecture changes.
