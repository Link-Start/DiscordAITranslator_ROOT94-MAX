...
**Working parent-refresh trigger (2026-08-13, follow-up experiment).** A manual/auto experiment tried three refresh strategies against the mounted list and measured the render-pass delta. Forcing the channel-stream owner fiber directly — `BDFDB.ReactUtils.forceUpdate(ownerFiber)` where `ownerFiber` is the function component `oo` resolved by walking up from the messages scroller DOM node to the first fiber whose props carry `channelStream` — caused re-renders (`caused: true`). Forcing the far updateable class ancestor (`nearestUpdateableAncestor`, depth 24) caused nothing (`caused: false`), confirming the memo boundary blocks an ancestor force. **Conclusion:** the replacement refresh must resolve the channel-stream owner fiber from the scroller DOM at transaction time and force-update that fiber once; it must not rely on the before-render patch envelope (not a fiber) or on an updateable ancestor instance.
