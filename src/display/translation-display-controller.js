...
function createTranslationDisplayController({
	...
	let activeDeferredFlushCount = 0;
	let activeDeferredMessageCount = 0;
	let activeDeferredHostMessageCount = 0;
	let activeDeferredHostViewCount = 0;
	let deferredFlushErrorCount = 0;
	...
	function armDeferredFlush() {
		...
		deferredFlushTimer = scheduleTimer(async () => {
			...
			const messageCount = pending.reduce((total, [, wave]) => total + wave.messageIds.size, 0);
			const hostMessageCount = pending.reduce((total, [, wave]) => total + wave.hostMessageIds.size, 0);
			const hostViewCount = pending.reduce((total, [, wave]) => total + wave.hostViews.size, 0);
			activeDeferredFlushCount++;
			activeDeferredMessageCount += messageCount;
			activeDeferredHostMessageCount += hostMessageCount;
			activeDeferredHostViewCount += hostViewCount;
			try {
				for (const [channelId, wave] of pending) {
					...
					catch (error) {deferredFlushErrorCount++;}
				}
			}
			finally {
				activeDeferredFlushCount = Math.max(0, activeDeferredFlushCount - 1);
				activeDeferredMessageCount = Math.max(0, activeDeferredMessageCount - messageCount);
				activeDeferredHostMessageCount = Math.max(0, activeDeferredHostMessageCount - hostMessageCount);
				activeDeferredHostViewCount = Math.max(0, activeDeferredHostViewCount - hostViewCount);
			}
	...
	function getDiagnostics() {
		...
		let pendingHostMessageCount = 0;
		let pendingHostViewCount = 0;
		for (const wave of pendingRefreshByChannel.values()) {
			pendingMessageCount += wave.messageIds.size;
			pendingHostMessageCount += wave.hostMessageIds.size;
			pendingHostViewCount += wave.hostViews.size;
		}
		return Object.freeze({deferredFlushErrorCount, resources: Object.freeze({
			pendingChannelCount: pendingRefreshByChannel.size,
			pendingMessageCount,
			pendingHostMessageCount,
			pendingHostViewCount,
			activeDeferredFlushCount,
			activeDeferredMessageCount,
			activeDeferredHostMessageCount,
			activeDeferredHostViewCount,
			deferredFlushTimerArmed: deferredFlushTimer != null
		})});
	}
	...
	async function refreshRecords(records, {channelId = null, ownerMessageIds = [], ownerViews = [], sources = null} = {}) {
		...
		let outcome;
		try {outcome = await renderAdapter.refreshMessages({
			...
			// Scheduler retries and deferred waves both need a revision-stamped failure.
			try {onRenderOutcome({channelId: transactionChannelId, messageIds: [...requestedViews.keys()], revisionsByMessageId: Object.fromEntries([...requestedViews].map(([id, view]) => [id, view.revision])), outcome: {rejectedIds: [...requestedViews.keys()]}});} catch (ignored) {}
			throw error;
		}
		...
		try {onRenderOutcome({channelId: transactionChannelId, messageIds: [...requestedViews.keys()], revisionsByMessageId: Object.fromEntries([...requestedViews].map(([id, view]) => [id, view.revision])), outcome: filteredOutcome});} catch (error) {}
	...
	return Object.freeze({
		...
		async commitHistoricalBatch(results) {
			...
			const committedIds = outcome.committed.map(record => String(record.messageId));
			if (!outcome.committed.length) {
				if (!outcome.rejected.length) return createEmptyOutcome({committedIds});
				return createEmptyOutcome({committedIds, rejectedIds: outcome.rejected.map(result => String(result.messageId))});
			}
			...
			if (!canRepaintNow()) {
				...
				armDeferredFlush();
				const deferred = createEmptyOutcome({committedIds, deferredIds: outcome.committed.map(record => String(record.messageId)), paintPendingIds: committedIds});
			...
			let refreshOutcome;
			try {refreshOutcome = await refreshRecords(outcome.committed, {sources: {historical: outcome.committed.length}});}
			catch (error) {
				// Store acceptance survives a render exception. Use the existing bounded
				// display retry queue, without retranslating the accepted result.
				const currentIds = outcome.committed.filter(record => {const current = store.getDisplayState(record.messageId); return current && current.revision === record.revision;}).map(record => String(record.messageId));
				refreshOutcome = createEmptyOutcome({missingIds: currentIds, retryIds: currentIds, staleIds: committedIds.filter(id => !currentIds.includes(id))});
			}
			refreshOutcome.committedIds = committedIds;
		...
		async restoreChannel(channelId, {clearPreviews = false, clearSuppressions = false, queuedMessageIds = []} = {}) {
			const previewHostMessageIds = clearPreviews ? store.getPreviewHostMessageIds(channelId) : [];
			const restored = store.restoreChannel(channelId, "channel-disabled", {queuedMessageIds});
