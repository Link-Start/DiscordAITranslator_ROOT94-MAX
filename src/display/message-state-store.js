...
// Terminal records without any display, restore, request, or reply-preview ownership
// are only dedup receipts. Keep a generous recent window per channel rather than every
// source/embed clone observed during a long session. Records that still affect a visible
// or in-flight surface are excluded from this budget.
const MAX_INERT_TERMINAL_RECORDS_PER_CHANNEL = 500;
...
function createMessageStateStore({
	...
	maxInertTerminalRecordsPerChannel = MAX_INERT_TERMINAL_RECORDS_PER_CHANNEL
} = {}) {
	...
	const inertTerminalRecordLimit = Number.isSafeInteger(maxInertTerminalRecordsPerChannel) && maxInertTerminalRecordsPerChannel > 0
		? maxInertTerminalRecordsPerChannel
	...
	function applyResult(result, pruneAfterCommit = true) {
		...
		const committed = recordTransition(update(result.messageId, {
			status,
		...
		if (pruneAfterCommit) pruneCommittedTerminalRecords([committed]);
		return committed;
	}

	function restoreRecords(recordsToRestore, reason, origins, queuedMessageIds = new Set()) {
		const allowedOrigins = normalizeOrigins(origins);
		const restored = recordsToRestore
			// Historical collection has a queue-owned spinner before a store request.
			// Only explicitly cancelled queue IDs may restore such originless records.
			.filter(record => record && (record.origin ? allowedOrigins.has(record.origin) : queuedMessageIds.has(record.messageId)) && record.status !== MESSAGE_STATUSES.CANCELLED)
		...
		pruneCommittedTerminalRecords(restored);
		return restored;
	}
	...
	function collectPreviewMetadataOwners(channelId) {
		const owners = new Set(previewEligibility.get(channelId) || []);
		for (const messageId of (previewHostRenderRevisions.get(channelId) || new Map()).keys()) owners.add(messageId);
		for (const [referencedMessageId, hostMessageIds] of previewHostsByChannel.get(channelId) || []) {
			owners.add(referencedMessageId);
			for (const hostMessageId of hostMessageIds) owners.add(hostMessageId);
		}
		return owners;
	}

	function isInertTerminalRecord(record, previewMetadataOwners) {
		return !!(record
			&& TERMINAL_STATUSES.has(record.status)
			&& record.status !== MESSAGE_STATUSES.TRANSLATED
			&& !record.translation
			&& !record.restoredTranslation
			&& !record.restoredPreview
			&& !record.archive
			&& !record.suppressed
			&& !record.preview
			&& !record.previewPending
			&& record.requestIdentity === null
			&& !previewMetadataOwners.has(record.messageId));
	}

	function pruneInertTerminalRecords(channelId, protectedMessageIds = new Set()) {
		const previewMetadataOwners = collectPreviewMetadataOwners(channelId);
		const candidates = listChannel(channelId)
			.filter(record => isInertTerminalRecord(record, previewMetadataOwners))
			.sort((left, right) => left.revision - right.revision);
		const excess = Math.max(0, candidates.length - inertTerminalRecordLimit);
		return candidates.filter(record => !protectedMessageIds.has(record.messageId)).slice(0, excess).filter(deleteRecord);
	}

	function pruneCommittedTerminalRecords(committedRecords) {
		const protectedMessageIds = new Set();
		const channelIds = new Set();
		for (const record of committedRecords) {
			if (!record || !record.messageId || !record.channelId) continue;
			protectedMessageIds.add(record.messageId);
			channelIds.add(record.channelId);
		}
		for (const channelId of channelIds) pruneInertTerminalRecords(channelId, protectedMessageIds);
	}
	...
	return Object.freeze({
		...
		commitBatch(results) {
			...
			const committed = recorded.map(result => applyResult(result, false));
			pruneCommittedTerminalRecords(committed);
			return {committed, rejected: recordless};
		},
		...
		restoreChannel(channelId, reason = "channel-disabled", {origins = ALL_MESSAGE_ORIGINS, queuedMessageIds = []} = {}) {
			return restoreRecords(listChannel(channelId), reason, origins, new Set(queuedMessageIds.map(String)));
		},
		...
		markRenderOutcome({confirmedIds = [], missingIds = []} = {}) {
			...
			for (const messageId of confirmedIds) {
				const record = records.get(normalizeIdentity(messageId));
				if (record && record.channelId) affectedChannelIds.add(record.channelId);
			...
			for (const channelId of affectedChannelIds) pruneInertTerminalRecords(channelId);
		}
	});
}
