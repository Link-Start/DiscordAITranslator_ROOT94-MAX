...
function createDisplayRuntime(dependencies) {
	...
	const controller = createTranslationDisplayController({store, renderAdapter, journal, setTimeout: dependencies.setTimeout, canRepaintNow: dependencies.canRepaintNow, onRenderOutcome: dependencies.onRenderOutcome});
	...
	return Object.freeze({
		...
		getControllerDiagnostics: () => controller.getDiagnostics(),
		...
		getAutomaticPendingMessageIds: channelId => store.listChannel(channelId).filter(record => record.origin === "automatic" && (record.status === "pending" || record.status === "translating")).map(record => record.messageId),
