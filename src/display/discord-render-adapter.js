...
function createDiscordRenderAdapter({BDFDB, document, requestAnimationFrame, getUserScrollIntentSequence, captureScrollState, restoreScrollState, restoreScrollStateNow = () => {}, isRuntimeActive = () => true, liveRowRepaint = null}) {
	...
	function waitForPaint(beforePaint = null) {
		return new Promise(resolve => requestAnimationFrame(() => {
			if (typeof beforePaint == "function") beforePaint();
	...
	return {
		...
		async refreshMessages({channelId = null, messageIds = [], ownerMessageIds = [], ownerViews = [], views = [], sources = null}) {
			...
			// Store/Flux row projections can commit on the next animation frame. Restoring
			// only in the dispatch task uses the OLD row height, so the first translated
			// frame visibly jumps before the later settle ladder pulls it back. Re-apply
			// the same captured bottom/anchor inside each pre-paint callback, but only while
			// the runtime and user-intent lease are still current.
			const restoreBeforePaint = () => {
				try {
					if (!isRuntimeActive() || !scrollState || intentSequence !== getUserScrollIntentSequence()) return;
			...
			try {
				...
				if (liveRowRepaint && (unconfirmedIds.length || unconfirmedOwnerIds.length)) {
					...
					if (attemptedIds.length) {
						restoreBeforePaint();
						await waitForPaint(restoreBeforePaint);
