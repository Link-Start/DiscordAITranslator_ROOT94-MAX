...
// Shallow, mutable clone that keeps the prototype, so a patched message or
// forward snapshot still carries whatever record methods Discord's renderer
// calls on it. Data properties are made writable because every caller creates
// a clone specifically to paint one of those fields without touching the live
// Discord record.
function cloneWithPrototype(source) {
	if (!source || !["object", "function"].includes(typeof source)) return source;
	try {
		const clone = Object.create(Object.getPrototypeOf(source) || null);
		for (const key of Reflect.ownKeys(source)) {
			const descriptor = Object.getOwnPropertyDescriptor(source, key);
			if (!descriptor) continue;
			if (hasOwn(descriptor, "value")) Object.defineProperty(clone, key, Object.assign({}, descriptor, {writable: true, configurable: true}));
			else Object.defineProperty(clone, key, {value: source[key], enumerable: descriptor.enumerable, writable: true, configurable: true});
		}
		return clone;
	}
	catch (error) {
		try {return Object.assign({}, source);}
		catch (fallbackError) {return {};}
	}
}

function fillMissingDiscordMessageFields(target, source) {
	for (const key of Reflect.ownKeys(source)) {
		try {
			const sourceDescriptor = Object.getOwnPropertyDescriptor(source, key);
			if (!sourceDescriptor) continue;
			const sourceValue = hasOwn(sourceDescriptor, "value") ? sourceDescriptor.value : source[key];
			const targetDescriptor = Object.getOwnPropertyDescriptor(target, key);
			if (targetDescriptor) {
				const targetValue = hasOwn(targetDescriptor, "value") ? targetDescriptor.value : target[key];
				if (targetValue != null || sourceValue == null) continue;
				if (hasOwn(targetDescriptor, "value") && targetDescriptor.writable || !hasOwn(targetDescriptor, "value") && typeof targetDescriptor.set == "function") target[key] = sourceValue;
				continue;
			}
			Object.defineProperty(target, key, {value: sourceValue, enumerable: sourceDescriptor.enumerable, writable: true, configurable: true});
		}
		catch (error) {}
	}
	return target;
...
function isWritableDetachedMessage(candidate, source) {
	if (!candidate || candidate === source || !["object", "function"].includes(typeof candidate)) return false;
	try {
		if (!Object.isExtensible(candidate)) return false;
		for (const key of Reflect.ownKeys(source)) {
			const sourceDescriptor = Object.getOwnPropertyDescriptor(source, key);
			const candidateDescriptor = Object.getOwnPropertyDescriptor(candidate, key);
			if (!sourceDescriptor || !candidateDescriptor) return false;
			if (hasOwn(candidateDescriptor, "value")) {
				if (!candidateDescriptor.writable) return false;
				if (hasOwn(sourceDescriptor, "value") && sourceDescriptor.value != null && candidateDescriptor.value == null) return false;
			}
			else if (typeof candidateDescriptor.set != "function") return false;
		}
		return true;
	}
...
function prepareDiscordMessageClone(candidate, source) {
	if (!candidate || !["object", "function"].includes(typeof candidate)) return null;
	fillMissingDiscordMessageFields(candidate, source);
	if (isWritableDetachedMessage(candidate, source)) return candidate;
	const detachedCandidate = cloneWithPrototype(candidate);
	fillMissingDiscordMessageFields(detachedCandidate, source);
	return isWritableDetachedMessage(detachedCandidate, source) ? detachedCandidate : null;
}

// BDFDB's Message record is an integration boundary, not a stable application
// primitive. Prefer it while available so Discord-specific normalisation and
// methods survive, but keep render paths alive when a library update removes or
// changes the constructor. Constructors in the wild have also dropped newer
// message fields, so fill any missing own fields from the source record.
function cloneDiscordMessage(BDFDB, source) {
	if (!source || !["object", "function"].includes(typeof source)) return source;
	const Message = BDFDB && BDFDB.DiscordObjects && BDFDB.DiscordObjects.Message;
	if (typeof Message == "function") {
		try {
			const clone = prepareDiscordMessageClone(new Message(source), source);
			if (clone) return clone;
		}
		catch (error) {}
	}
	return prepareDiscordMessageClone(cloneWithPrototype(source), source) || cloneWithPrototype(source);
}
...
// Accepted results can contain only preserved source text. Compare the body,
// not the composed display text (which may append an original quote/spoiler).
// A kept segment alone does not imply that the other segments are unchanged.
function hasTranslatedBody(plugin, translation) {
	if (!translation) return false;
	const normalized = plugin.normalizeStoredTranslationData(translation) || translation;
	const body = String(normalized.translatedContent || normalized.content || "").replace(/\r\n?/g, "\n").trim();
	const original = String(normalized.originalContent || "").replace(/\r\n?/g, "\n").trim();
	if (!body) return false;
	return original ? body !== original : !(Number(normalized.keptSegmentCount) > 0);
}
	...
	const translationDisplayLogic = {
		...
		cloneStreamMessageWithBody(plugin, streamContent, bodyText) {
			...
			if (forwardSnapshots) {
				...
				const clonedMessage = cloneDiscordMessage(BDFDB, streamContent);
		...
		applyReceivedDisplayViewToStream(plugin, stream, view) {
			...
			if (!clonedMessage) {
				clonedMessage = cloneDiscordMessage(BDFDB, stream.content);
				for (const key of ["messageSnapshots", "message_snapshots", "messageReference", "message_reference"]) {
		...
		applyReceivedDisplayViewToContent(plugin, e, view) {
			...
			if (translationDisplayLogic.isForwardContainerMessage(plugin, e.instance && e.instance.props && e.instance.props.message)) {
				// The empty outer content is not the forwarded body. A revision here
				// would acknowledge a stale inner snapshot that has not rendered yet.
			...
			if (view.translated && view.translation) {
				...
				if (hasTranslatedBody(plugin, view.translation)) {
		...
		getStableReplyPreviewMessage(plugin, message) {
			...
			const stableMessage = cloneDiscordMessage(BDFDB, message);
		...
		applyStoredTranslationToMessage(plugin, message, translation, originalContentData = null) {
			...
			plugin.ensureReceivedDisplayRuntime().commitManualTranslation({
				...
				archive: {message: cloneDiscordMessage(BDFDB, message), originalContentData: originalContentData || plugin.extractOriginalContentData(message)}
			});
		...
		processMessageReply(plugin, e) {
			...
			const previewMessage = cloneDiscordMessage(BDFDB, stableReferencedMessage);
		...
		resolveLoadedMessageContentTranslation(plugin, message, channelId) {
			if (plugin.getReceivedAutoTranslateScope() != "loaded_messages" || !plugin.isTranslationEnabled(channelId) || plugin.ensureLiveTranslationQueue().isMessageQueued(message.id)) return null;
			if (plugin.isOwnMessage(message) || plugin.ensureReceivedDisplayRuntime().isSuppressed(message.id)) {if (typeof plugin.observeHistoricalAutoEligibility == "function") plugin.observeHistoricalAutoEligibility(message, {id: channelId}, plugin.extractOriginalContentData(message), {origin: "history-render", ignoreQueued: false}); return null;}
			...
			const observed = !cachedTranslation && typeof plugin.observeHistoricalAutoEligibility == "function" ? plugin.observeHistoricalAutoEligibility(message, {id: channelId}, originalContentData, {origin: "history-render", ignoreQueued: false}) : null;
			const eligible = cachedTranslation ? true : observed ? observed.eligible : plugin.shouldAutoTranslateReceivedMessage(message, {id: channelId}, originalContentData);
			...
			if (eligible) {
				plugin.queueAutoTranslateMessage(message, {id: channelId}, originalContentData, {
					...
					cachedTranslation,
					terminalRouteId: observed && observed.routeId || null
				});
			}
			return null;
		},
		prepareMessageContentDisplay(plugin, e) {
			...
			if (!translation && plugin.ensureReceivedDisplayRuntime().hasSourceArchive(message.id)) {
				message = e.instance.props.message = cloneDiscordMessage(BDFDB, plugin.ensureReceivedDisplayRuntime().consumeSourceArchive(message.id).message);
			}
		...
		createTranslationWatermarkNode(plugin, translation, key) {
			if (!translation || !translation.content || !hasTranslatedBody(plugin, translation)) return null;
		...
		applyMessageContentRenderDecorations(plugin, e, message, translation) {
			...
			if (hasTranslatedBody(plugin, translation)) {
				if (plugin.settings.general.highlightTranslatedMessages) e.returnvalue.props.className = BDFDB.DOMUtils.formatClassName(e.returnvalue.props.className, "translator-translated-message");
				e.returnvalue.props.style = Object.assign({}, e.returnvalue.props.style, {
					"--translator-accent-color": plugin.getTranslatedTextColor(),
					"--translator-text-color": plugin.getTranslatedTextColor()
			...
			if (embedTranslation) {
				...
				if (!e.returnvalue) e.instance.props.embed = Object.assign({}, embed, {
					...
					if (index > -1) {
						...
						// The description has its own source/translation pair: an unchanged
						// message body says nothing about whether this embed was translated.
						const description = translatedOrOriginal(embedTranslation.description, originalDescription);
						const descriptionTranslation = Object.assign({}, translation, {
							content: description,
							translatedContent: description,
							originalContent: originalDescription
						});
						const watermarkNode = translationDisplayLogic.createTranslationWatermarkNode(plugin, descriptionTranslation, "translator-embed-watermark");
...
module.exports = {MESSAGE_DIRECTIONS, cloneDiscordMessage, createTranslationDisplayLogic};
