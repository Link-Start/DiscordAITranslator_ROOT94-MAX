...
const {SENT_ECHO_GUARD_THRESHOLD} = require("./translation-similarity-policy");

// Han alone does not distinguish Chinese from Japanese, and several script families
// serve more than one language. These paired characters are only high-confidence
// orthography hints; ambiguous samples still use the configured network detector.
const SIMPLIFIED_HAN_HINTS = new Set("这为个来时说发后里会过们对从还进将种实学体给门间开关无与数网线车见听写读话语点边东国台万叶云广归当录历压变参复头号导层岁师应异总战戏户执扩担据换损断旧术权条极标样档欢测济现电画盘确礼离类组经结统维联节范观设证评识调质软适选释钟阳阴难项预顾风马验齐龙编辑轮转机讯".split(""));
const TRADITIONAL_HAN_HINTS = new Set("這為個來時說發後裡會過們對從還進將種實學體給門間開關無與數網線車見聽寫讀話語點邊東國臺萬葉雲廣歸當錄歷壓變參復頭號導層歲師應異總戰戲戶執擴擔據換損斷舊術權條極標樣檔歡測濟現電畫盤確禮離類組經結統維聯節範觀設證評識調質軟適選釋鐘陽陰難項預顧風馬驗齊龍編輯輪轉機訊".split(""));

function countCharacterHints(text, hints) {
	let count = 0;
	for (const character of text || "") if (hints.has(character)) count++;
	return count;
}
...
const sentTranslationPolicy = {
	...
	shouldSendOriginalInsteadOfSentTranslation(plugin, originalText, translation, input, output) {
		...
		return plugin.getTextSimilarityScore(originalText, translation) >= SENT_ECHO_GUARD_THRESHOLD;
	},
...
const languageHeuristicsRuntime = {
	...
	identifyNonLatinLanguage(plugin, text) {
		const counts = plugin.countScriptFamilies(text);
		const total = Object.values(counts).reduce((sum, count) => sum + count, 0);
		if (!total) return {languageId: null, confident: false};
		const share = family => (counts[family] || 0) / total;
		// Kana and Hangul are direct high-confidence signals. Han may accompany Kana.
		if ((counts.kana || 0) >= 1 && (counts.kana || 0) + (counts.han || 0) >= 2 && ((counts.kana || 0) + (counts.han || 0)) / total >= 0.6) return {languageId: "ja", confident: true};
		if ((counts.hangul || 0) >= 2 && share("hangul") >= 0.6) return {languageId: "ko", confident: true};
		if ((counts.thai || 0) >= 3 && share("thai") >= 0.7) return {languageId: "th", confident: true};
		if ((counts.greek || 0) >= 3 && share("greek") >= 0.7) return {languageId: "el", confident: true};
		if ((counts.han || 0) >= 4 && share("han") >= 0.6) {
			const simplified = countCharacterHints(text, SIMPLIFIED_HAN_HINTS);
			const traditional = countCharacterHints(text, TRADITIONAL_HAN_HINTS);
			if (simplified > traditional && simplified > 0) return {languageId: "zh-CN", confident: true};
			if (traditional > simplified && traditional > 0) return {languageId: "zh-TW", confident: true};
		}
		return {languageId: null, confident: false};
	},
	...
	isTranslationLikelyInTargetLanguage(plugin, text, targetLanguageId) {
		...
		// Non-Latin targets (Chinese, Japanese, ...): a single target-script letter proves the
		// answer was translated. The remaining Latin letters are names and identifiers the
		// translation legitimately carries over ("使用 Adobe Audition 或 Ultimate Vocal Remover"),
		// so a letter ratio would only reject correct answers. An answer without any target
		// letter still goes through the checks below.
		if (targetFamilies[0] != "latin" && analysis.targetLetterCount >= 1) return true;
...
function createLanguageHeuristics({BDFDB} = {}) {
	...
	const receivedSettingsPolicy = {
		...
		shouldTreatLanguageVariantsAsSame(_plugin) {
			return true;
		},
		shouldSkipMixedReceivedMessages(_plugin) {
			return false;
		},
		shouldSkipSameLanguageReceivedMessages(_plugin) {
			return true;
		},
	...
	const languageDetectionRuntime = {
		...
		detectLanguage(plugin, text, callback) {
			...
			if (strategy != "google_free") {
				const scriptDetection = languageHeuristicsRuntime.identifyNonLatinLanguage(plugin, detectableText);
				if (scriptDetection && scriptDetection.confident && scriptDetection.languageId) return callback(scriptDetection.languageId);
			...
			BDFDB.LibraryRequires.request("https://translate.googleapis.com/translate_a/single", {
				form: {
					...
					"sl": "auto",
					"tl": "en",
					// request/form performs URL encoding. Pre-encoding here turns CJK into
					// literal %E7... text and makes Google's detector see the wrong language.
					"q": detectableText
				}
			}, (error, response, body) => {
				const statusCode = response && (response.statusCode || response.status);
				if (!error && body && statusCode == 200) return callback(languageDetectionRuntime.parseDetectedLanguageResponse(plugin, body));
				callback(null);
			});
		}
	};
