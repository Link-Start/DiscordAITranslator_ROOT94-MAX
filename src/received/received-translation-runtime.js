...
const {TRANSLATION_SIMILARITY_THRESHOLD, RECEIVED_ECHO_GUARD_THRESHOLD} = require("../language/translation-similarity-policy");
...
// Both whole-message similarity checks must respect the same existing stored evidence.
// Semantic results were validated per segment; P3-b keeps explicitly preserve source text.
// This exemption applies only to similarity, never language, source or cache identity policy.
function hasStoredSimilarityExemption(translation) {
	return !!(translation && translation.semanticRevision && /^segment-validator-v\d+$/.test(String(translation.validatorVersion || "")))
		|| Number(translation && translation.keptSegmentCount) > 0;
}
...
const receivedMessageFilterRuntime = {
	isTranslationResultTooSimilar(plugin, translation) {
		...
		if (hasStoredSimilarityExemption(normalizedTranslation)) return false;
		...
		return plugin.getTextSimilarityScore(originalContent, translatedContent) >= RECEIVED_ECHO_GUARD_THRESHOLD;
	},
	getAutoTranslatedResultRejectReason(plugin, translation, channelId) {
		...
		if (!hasStoredSimilarityExemption(translation) && translation.originalContent && translation.translatedContent && plugin.getTextSimilarityScore(translation.originalContent, translation.translatedContent) >= TRANSLATION_SIMILARITY_THRESHOLD) return "too_similar";
		return null;
	},
	...
	getReceivedAutoTranslateEligibility(plugin, message, channel, originalContentData = null, ignoreQueued = false) {
		const result = (eligible, reason) => Object.freeze({eligible, reason});
		if (!channel || !channel.id || !message || !message.id) return result(false, "invalid_entry");
		if (!plugin.isTranslationEnabled(channel.id)) return result(false, "translation_disabled");
		if (plugin.isOwnMessage(message)) return result(false, "self_authored");
		if (plugin.ensureReceivedDisplayRuntime().isSuppressed(message.id)) return result(false, "suppressed");
		if (plugin.isMessageDisplayTranslated(message, channel.id)) return result(false, "already_translated");
		if (!ignoreQueued && plugin.ensureLiveTranslationQueue().isMessageQueued(message.id)) return result(false, "already_queued");
		const sourceData = originalContentData || plugin.extractOriginalContentData(message);
		if (plugin.getCachedReceivedSkipDecision(message, channel.id, sourceData)) return result(false, "skip_cache");
		if (receivedMessageFilterRuntime.isLinkOnlyReceivedContent(plugin, sourceData)) return result(false, "link_only");
		if (!plugin.hasTranslatableMessageContent(sourceData)) return result(false, "no_content");
		const planEligibility = typeof plugin.getReceivedTranslationPlanEligibility == "function" ? plugin.getReceivedTranslationPlanEligibility(sourceData, channel.id) : null;
		if (planEligibility && planEligibility.enabled) return result(!!planEligibility.eligible, planEligibility.reason || (planEligibility.eligible ? "plan_translate" : "plan_no_translate"));
		const receivedAnalysis = receivedMessageFilterRuntime.buildReceivedAutoTranslateAnalysis(plugin, sourceData, channel.id);
		if (!receivedAnalysis || !receivedAnalysis.analysisSource.hasUnprotectedContent) return result(false, "all_protected");
		const {analysisSource, targetLanguageId, analysis} = receivedAnalysis;
		if (!analysis.totalLetters) return result(false, "no_letters");
		if (analysis.totalLetters < plugin.getAutoTranslateMinimumLengthForAnalysis(analysis)) return result(false, "below_minimum");
		if (plugin.isClearlyTargetLanguageMessage(analysis, targetLanguageId)) return result(false, "target_language");
		if (plugin.shouldSkipSameLanguageReceivedMessages() && plugin.isMostlyTargetLanguageMessage(analysis, targetLanguageId)) return result(false, "same_language");
		if (receivedMessageFilterRuntime.shouldSkipByLocalLanguagePrecheck(plugin, analysisSource.text, analysis, targetLanguageId)) return result(false, "local_precheck");
	...
	shouldAutoTranslateReceivedMessage(plugin, message, channel, originalContentData = null, ignoreQueued = false) {
		return receivedMessageFilterRuntime.getReceivedAutoTranslateEligibility(plugin, message, channel, originalContentData, ignoreQueued).eligible;
	}
};
