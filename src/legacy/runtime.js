module.exports = (_ => {
	...
	return !window.BDFDB_Global || (!window.BDFDB_Global.loaded && !window.BDFDB_Global.started) ? class {
		...
		const {cloneDiscordMessage, createTranslationDisplayLogic} = require("../display/translation-display-logic");
		...
		const {renderSettingsPanel, flushDeferredSettingsWrites, cancelW2SettingsBenchmark} = require("../ui/settings-panel");
		...
		const {createPluginDefaults, MODULE_PATCHES} = require("../settings/plugin-defaults"), {ensureAiPromptLibrary, isAiPromptSendingEnabled} = require("../settings/prompt-library"), {getDefaultTranslationPreferences, LEGACY_DECISION_PROMPTS, translationPreferenceDigest} = require("../settings/translation-preferences");
		...
		const {projectHistoricalStatus} = require("../status/historical-status-projection");
		...
		const {createReceivedCacheSignature, createPluginTranslationCacheStore} = require("../cache/translation-cache-wiring");
		const {translationEngines, enginePortals, isCustomEngineKey, isOpenAiCompatibleEngineKey, isAiBatchCapableEngineKey, syncCustomEngines} = require("../providers/provider-client");
		...
		const {prepareHistoricalAiBatchQueueItem, refreshHistoricalWireObservationProbe} = require("../orchestrator/historical-queue-item-wiring");
		...
		const {createPluginHistoricalBatchPerformance} = require("../diagnostics/historical-batch-performance-wiring"), {createHistoricalCommitCacheFinalizer} = require("../orchestrator/historical-commit-cache-finalizer"), {validateHistoricalTranslationJobResult, classifyHistoricalStoredRejection} = require("../orchestrator/historical-validation-classifier"), {createTranslationTerminalLedger, summarizeProtectionRules} = require("../diagnostics/translation-terminal-ledger"), {createTranslationCurrentBehaviorMatrix, buildMatrixRowsFromTerminalLedger} = require("../diagnostics/translation-current-behavior-matrix");
		const {createTranslationPlanShadowStore} = require("../diagnostics/translation-plan-shadow-store"), {createTranslationDocumentShadowStore} = require("../diagnostics/translation-document-shadow-store");
		const {createHistoryAutoTerminalObserver} = require("../diagnostics/history-auto-terminal-observer");
		const translationSemanticRuntime = require("../planner/translation-semantic-runtime"), {SEMANTIC_REVISION} = require("../planner/translation-semantic-revision");
		...
		const {createPluginMessageDeletionLifecycle} = require("../lifecycle/message-deletion-lifecycle-wiring"), {runIsolatedCleanupSteps} = require("../lifecycle/stop-cleanup");
		...
		const {getGeneralSettingLabels, getLabelsForUiLanguage, resolveDetectLanguageLabel} = require("../i18n/labels");
		...
		const channelTitleStore = createChannelTitleStore(), loadedTranslationStatusStore = createLoadedTranslationStatusStore({isChineseUiLanguage: () => _this && _this.isChineseUiLanguage()});
		const historicalDisplayTracker = createHistoricalDisplayTracker({isStatusForChannel: channelId => loadedTranslationStatusStore.isForChannel(channelId), getBatchNumber: () => loadedTranslationStatusStore.getCurrentBatchNumber(), getRevision: (_channelId, messageId) => {const view = _this && _this.getReceivedDisplayRuntimeView(messageId); return view ? view.revision : null;}, updateStatus: updates => _this && _this.updateLoadedAutoTranslationStatus(updates), onResolved: report => historicalBatchPerformance.recordDomConfirmByBatchKey(report && report.batchKey, {confirmedCount: report && report.confirmedCount || 0})}), historicalBatchPerformance = createPluginHistoricalBatchPerformance({isSafetyLimiterEnabled: () => !_this || !_this.settings || !_this.settings.performance || _this.settings.performance.historicalSafetyDownshift !== false, loadPrimarySamples: () => _this ? BDFDB.DataUtils.load(_this, "historicalPrimarySamples") : null, savePrimarySamples: value => {_this && BDFDB.DataUtils.save(value, _this, "historicalPrimarySamples");}, recordSemanticObservation: event => {if (!_this) return null; const client = _this.ensureProviderClient(), token = event && event.token || client.beginLatencyRequest({kind: "historical", lane: event && event.lane || "history-primary"}); const payload = Object.assign({}, event || {}, {token}), recorded = client.recordSemanticObservation(payload), reason = payload.reason || "unknown", stage = reason === "wrong-language" ? "target-language" : reason === "too-similar" ? "similarity" : reason === "placeholder-mismatch" ? "placeholder" : "parse"; client.recordAttemptOutcome({token, outcome: "failed", stage, reason}); return recorded;}}), historicalCommitCacheFinalizer = createHistoricalCommitCacheFinalizer({BDFDB, performance: historicalBatchPerformance, displayTracker: historicalDisplayTracker}), translationTerminalLedger = createTranslationTerminalLedger({load: () => _this ? BDFDB.DataUtils.load(_this, "translationTerminalLedger") : null, save: value => {_this && BDFDB.DataUtils.save(value, _this, "translationTerminalLedger");}}), historyAutoTerminalObserver = createHistoryAutoTerminalObserver({getEligibility: (message, channel, source, ignoreQueued) => {if (!_this) return {eligible: false, reason: "runtime_inactive"}; const detailed = _this.getReceivedAutoTranslateEligibility(message, channel, source, ignoreQueued), effective = _this.shouldAutoTranslateReceivedMessage(message, channel, source, ignoreQueued); return effective === detailed.eligible ? detailed : {eligible: !!effective, reason: effective ? "eligible_override" : detailed.reason};}, begin: meta => translationTerminalLedger.begin(meta), update: (routeId, fields) => translationTerminalLedger.update(routeId, fields), terminal: (routeId, result) => translationTerminalLedger.terminal(routeId, result)}), translationPlanShadowStore = createTranslationPlanShadowStore({load: () => _this ? BDFDB.DataUtils.load(_this, "translationPlanShadow") : null, save: value => {_this && BDFDB.DataUtils.save(value, _this, "translationPlanShadow");}}), translationDocumentShadowStore = createTranslationDocumentShadowStore({load: () => _this ? BDFDB.DataUtils.load(_this, "translationDocumentShadow") : null, save: value => {_this && BDFDB.DataUtils.save(value, _this, "translationDocumentShadow");}});
		...
		return class Translator extends Plugin {
			...
			handleEditedMessageSubmit (methodArguments, originalMethod) {
				...
				return new Promise((resolve, reject) => {
					...
					this.observeTranslationDocumentPlan({body: originalText}, {lane: "sent", direction: "sent", channelId}); this.shouldAutoTranslateSentMessage(originalText, channelId, shouldTranslate => {
			...
			onStart () {
				pluginRuntimeActive = true; translationTerminalLedger.start(); translationPlanShadowStore.start(); translationDocumentShadowStore.start(); historicalBatchPerformance.start(); historicalBatchPerformance.setConcurrency(this.settings.performance && this.settings.performance.historicalConcurrency || "auto"); this.ensureLiveTranslationQueue().setLiveSlotCapacity(this.settings.performance && this.settings.performance.liveConcurrency || 1); this.ensureProviderClient().resetLatency();
				...
				this.ensureLiveTranslationQueue().restartRequestGeneration(); if (this.replyPreviewQueueInstance) this.replyPreviewQueueInstance.start();
				...
				require("../display/message-content-revision").installMessageContentRevisionGuard({plugin: this, BDFDB, Webpack: BdApi.Webpack});
			...
			onStop () {
				...
				return runIsolatedCleanupSteps([
					() => this.translationPipelineInstance && this.translationPipelineInstance.disableWholeMarkerSingleCanary(), () => (cancelW2SettingsBenchmark(this, "plugin-stopped"), this.ensureProviderClient().cancelSyntheticBenchmark(), flushDeferredSettingsWrites(this)),
					() => this.providerClientInstance && this.providerClientInstance.disableWholeMarkerBatchCanary(),
					() => channelToggleOperations.reset(),
					() => messageUpdateProbe && messageUpdateProbe.stop(),
					() => messageUpdateExperiment && messageUpdateExperiment.stop(),
					() => this.ensureMessageDeletionLifecycle().stop(), () => this.replyPreviewQueueInstance && this.replyPreviewQueueInstance.stop(),
					...
					() => {const drain = historicalBatchPerformance.stop(); this.cancelHistoricalTranslationJobs(null, "plugin-stopped"); this.ensureProviderClient().abortProviderAttempts("plugin-stopped"); return drain;},
					...
					() => this.ensureLiveTranslationQueue().clearQueue(),
					...
					() => {historyAutoTerminalObserver.stop(); translationTerminalLedger.stop();},
					() => {translationPlanShadowStore.stop(); translationDocumentShadowStore.stop(); this.ensureProviderClient().resetCompactWireShadow();},
			...
			onSettingsClosed () {if (this.providerClientInstance) this.providerClientInstance.disableWholeMarkerBatchCanary(); if (this.translationPipelineInstance) this.translationPipelineInstance.disableWholeMarkerSingleCanary(); cancelW2SettingsBenchmark(this, "settings-closed"); flushDeferredSettingsWrites(this); this.ensureProviderClient().cancelSyntheticBenchmark(); this.ensureProviderClient().resetCompactWireShadow();
			...
			getEngineLabel (engineKey) {
				...
				// Custom providers (and a renamed oaicompat slot) show the user's name.
				if (engine && engine.custom) return engine.name || (isChinese ? "自定义平台" : this.isRussianUiLanguage() ? "Пользовательская платформа" : "Custom provider");
				if (engineKey == "googleapi") return "Google";
				if (engineKey == "googlecloud") return "Google Cloud";
				if (engineKey == "microsoft") return "Azure";
				if (engineKey == "gemini") return "Gemini";
				if (engineKey == "oaicompat") return isChinese ? "自定义平台" : this.isRussianUiLanguage() ? "Пользовательская платформа" : "Custom provider";
				if (engineKey == "baidu") return "Baidu";
			...
			getReceivedTranslationRequestConfigurationData (channelId) {
				return {
					...
					providerSemanticRevision: this.shouldUseAtomicSemanticRevision(messageTypes.RECEIVED, channelId) ? SEMANTIC_REVISION : "legacy"
				};
			}
			...
			createReceivedTranslationSignature (message, channelId, originalContentData = null) {
				return createReceivedCacheSignature(this, message, channelId, originalContentData);
			}
			...
			getTranslationCacheEvictedCount () {return this.ensureTranslationCacheStore().getEvictedCount();}
			...
			ensureReplyPreviewQueue () {
				if (!this.replyPreviewQueueInstance) this.replyPreviewQueueInstance = createReplyPreviewQueue({getPlugin: () => this, messageTypes, isRuntimeActive: () => pluginRuntimeActive, setTimeout: (callback, delay) => BDFDB.TimeUtils.timeout(callback, delay), clearTimeout: timer => BDFDB.TimeUtils.clear(timer)});
			...
			getReceivedAutoTranslateSkipReason (originalContentData, channelId) {
				const legacyReason = receivedMessageFilterRuntime.getReceivedAutoTranslateSkipReason(this, originalContentData, channelId); if (["link_only", "symbol_only"].includes(legacyReason)) return legacyReason; const plan = this.getReceivedTranslationPlanEligibility(originalContentData, channelId); return plan && plan.enabled ? plan.reason : legacyReason;
			}
			...
			isMessageTranslationPending (messageId, channelId = null) {
				const marker = this.ensureLiveTranslationQueue().getQueuedMarker(messageId); return this.isHistoricalMessagePending(messageId, channelId) || !!(marker && marker.type !== "historical");
			}
			...
			getDefaultAiAutoTranslatePrompt () {
				return getDefaultTranslationPreferences({isChinese: this.isChineseUiLanguage(), isRussian: this.isRussianUiLanguage()});
			}

			getLegacyAiAutoTranslatePrompts () {
				return LEGACY_DECISION_PROMPTS;
			}
			...
			getAiAutoTranslatePrompt (translationData = null) {
				const promptState = ensureAiPromptLibrary(this.settings.filters, {legacyPrompts: this.getLegacyAiAutoTranslatePrompts(), migratedName: this.isChineseUiLanguage() ? "迁移的提示词" : this.isRussianUiLanguage() ? "Перенесённый промпт" : "Migrated prompt"});
				if (promptState.changed) BDFDB.DataUtils.save(this.settings.filters, this, "filters");
				const selectedPrompt = promptState.items.find(item => item.id == promptState.selectedId);
				let prompt = !isAiPromptSendingEnabled(this.settings.filters) ? "" : selectedPrompt && selectedPrompt.body.trim() ? selectedPrompt.body : this.getDefaultAiAutoTranslatePrompt();
			...
			shouldSkipReceivedTranslationBeforeRequest (originalContentData, channelId) {
				const plan = this.getReceivedTranslationPlanEligibility(originalContentData, channelId); return plan && plan.enabled ? !plan.eligible : receivedMessageFilterRuntime.shouldSkipReceivedTranslationBeforeRequest(this, originalContentData, channelId);
			}
			...
			shouldAutoTranslateReceivedMessage (message, channel, originalContentData = null, ignoreQueued = false) {return receivedMessageFilterRuntime.shouldAutoTranslateReceivedMessage(this, message, channel, originalContentData, ignoreQueued);}
			getReceivedAutoTranslateEligibility (message, channel, originalContentData = null, ignoreQueued = false) {return receivedMessageFilterRuntime.getReceivedAutoTranslateEligibility(this, message, channel, originalContentData, ignoreQueued);}
			getReceivedTranslationPlanEligibility (originalContentData, channelId, options = {}) {try {const inputChoice = this.getLanguageChoice(languageTypes.INPUT, messageTypes.RECEIVED, channelId), outputChoice = this.getLanguageChoice(languageTypes.OUTPUT, messageTypes.RECEIVED, channelId), inputId = String(inputChoice || "").toLowerCase() == "$discord" ? this.normalizeLanguageId(inputChoice) : inputChoice, outputId = String(outputChoice || "").toLowerCase() == "$discord" ? this.normalizeLanguageId(outputChoice) : outputChoice, input = this.ensureSettingsStore().getLanguage(inputId) || {id: inputId || "auto"}, output = this.ensureSettingsStore().getLanguage(outputId) || {id: outputId || "zh-CN"}; if (!output || output.special) return Object.freeze({enabled: false, eligible: false, reason: "legacy"}); const source = this.buildTranslationRequestText(originalContentData || {}), request = this.createAtomicSemanticRevisionContract(source, {place: messageTypes.RECEIVED, channelId, engineKey: options.engineKey || this.getEffectivePrimaryEngine(channelId), inputLanguageId: input.id || "auto", targetLanguageId: output.id || "zh-CN", fieldPath: "body", attempt: 1, maxAttempts: 3}), targetFamilies = this.getLanguageScriptFamilies(output.id || "zh-CN"), allowPlanOnly = targetFamilies.length > 0 && !targetFamilies.includes("latin"), decision = request && request.enabled ? request : request && request.fallbackReason === "capability-unverified" && allowPlanOnly ? translationSemanticRuntime.createSemanticPlanEligibility({source, direction: "received", fieldPath: "body", targetLanguageId: output.id || "zh-CN"}) : request, eligible = !!(decision && decision.enabled && decision.segmentOrder.length && translationSemanticRuntime.isPlanTranslationMeaningful(decision.plan)); return decision && decision.enabled ? Object.freeze({enabled: true, eligible, reason: eligible ? "plan_translate" : "plan_no_translate", request: decision, planOnly: decision.planOnly === true}) : Object.freeze({enabled: false, eligible: false, reason: decision && decision.fallbackReason || "legacy"});} catch {return Object.freeze({enabled: false, eligible: false, reason: "legacy"});}}

			queueAutoTranslateMessage (message, channel, originalContentData = null, queueOptions = {}) {return this.ensureLiveTranslationQueue().queueMessage(message, channel, originalContentData, queueOptions);}
			...
			retryFailedHistoricalTranslations (channelId = null) {
				...
				const retryItems = failedEntry.items.slice(0, this.getReceivedAutoTranslateLoadedLimit()); // captured-key recovery happens only at an accepted retry attempt
			...
			createCollectedHistoricalTranslationJob (channelId) {
				...
				job = this.createHistoricalTranslationJob({
					...
					dependencies: {
						...
						translateBatch: (preparedItems, _job, onChunkOutcome) => this.translateHistoricalTranslationJobBatch(preparedItems, job, onChunkOutcome),
						...
						commitProgress: summary => this.commitHistoricalTranslationJob(summary, job, {partial: true}),
						onStateChange: () => this.updateHistoricalTranslationJobStatus(job), onCancel: (_job, reason) => historicalBatchPerformance.abortJobAttempts(job, reason), onInvalidate: (_job, _messageId, reason) => historicalBatchPerformance.abortJobAttempts(job, reason)
					}
				});
				entry.jobs.push(job);
				return job;
			...
			collectHistoricalTranslationMessage (queueItem) {
				...
				if (!queueItem.historicalSourceMetrics) {const observedAt = Date.now(); Object.defineProperty(queueItem, "historicalSourceMetrics", {value: Object.freeze({sourceLoadAt: observedAt, scanPrefetchAt: observedAt, sourceLoadCount: 1, prefetchedCount: 0, total: 1}), enumerable: false});}
				const channelId = queueItem.channel.id, routeId = queueItem.terminalRouteId, reject = reason => {if (routeId) {this.recordHistoricalAutoCheckpoint(routeId, "collector", reason); this.finishTranslationTerminalRoute(routeId, {outcome: "skipped", stage: "precheck", reason, displayCommit: "not-entered"}); queueItem.terminalRouteId = null;} return false;};
				if (!this.isTranslationEnabled(channelId)) return reject("translation_disabled"); if (!queueItem.retryFailed && this.ensureHistoricalJobRegistry().hasFailedMessage(channelId, queueItem.message.id, queueItem.signature || this.createReceivedTranslationSignature(queueItem.message, channelId, queueItem.originalContentData))) return reject("retained_failure");
				const liveQueue = this.ensureLiveTranslationQueue(), liveOwner = liveQueue.getQueuedMarker(queueItem.message.id);
				if (liveOwner && liveQueue.isRequestCurrent(liveOwner, queueItem.message)) return reject("already_queued");
				const entry = this.getHistoricalTranslationJobQueue(channelId); if (entry.intakeBlocked) return reject("intake_blocked");
				let job = entry.jobs[entry.jobs.length - 1];
				if (job && job.state == "collecting" && !job.sealed && job.items.size >= this.getReceivedAutoTranslateLoadedLimit()) return reject("loaded_limit");
				if (!job || job.state != "collecting" || job.sealed) job = this.createCollectedHistoricalTranslationJob(channelId);
				if (!job.add(queueItem)) return reject("duplicate_collect");
				if (routeId) this.recordHistoricalAutoCheckpoint(routeId, "collector", "accepted");
			...
			finishHistoricalTranslationSnapshot (channelId) {
				...
				if (!job) return false;
				job.seal(); for (const record of job.items.values()) if (record && record.source && record.source.terminalRouteId) this.recordHistoricalAutoCheckpoint(record.source.terminalRouteId, "seal", "sealed");
			...
			startCollectedHistoricalTranslationJobs (channelId, options = {}) {
				...
				if (!job && config.sealCurrent) { job = entry.jobs.find(candidate => candidate && candidate.state == "collecting"); if (job) {job.seal(); for (const record of job.items.values()) if (record && record.source && record.source.terminalRouteId) this.recordHistoricalAutoCheckpoint(record.source.terminalRouteId, "seal", "sealed");} }
				...
				job.lastConsumedLiveRequestTicketAtStart = this.ensureLiveTranslationQueue().getLastConsumedLiveRequestTicket(channelId); job.historicalPerformanceToken = historicalBatchPerformance.begin(job);
				const runningPromise = Promise.resolve(job.start()).finally(_ => {
					historicalBatchPerformance.finish(this, job); job.historicalPerformanceToken = null; for (const record of job.items.values()) {
						const routeId = record && record.source && record.source.terminalRouteId;
						if (routeId) {const status = record.status === "translated" ? "translated" : record.status === "skipped" ? "skipped" : record.status === "cancelled" ? "cancelled" : "failed"; this.finishTranslationTerminalRoute(routeId, {outcome: status, stage: status === "translated" || status === "cancelled" ? "display-currentness" : null, reason: status === "translated" ? "committed" : record.reason || status, displayCommit: status === "translated" || status === "skipped" ? "atomic" : status === "failed" ? "status" : "not-applied"}); record.source.terminalRouteId = null;}
						const messageId = record && record.source && record.source.message && record.source.message.id;
						if (messageId) this.ensureLiveTranslationQueue().clearHistoricalQueuedMessage(messageId, job.id);
					}
			...
			invalidateHistoricalTranslationMessage (messageId, channelId, currentSignature) {
				...
				for (const job of entry && entry.jobs || []) {
					...
					if (job.invalidateMessage(messageId, "source-edited")) {invalidated = true; invalidatedJobIds.push(job.id);}
				}
				...
				if (invalidated) {
					for (const jobId of invalidatedJobIds) this.ensureLiveTranslationQueue().clearHistoricalQueuedMessage(messageId, jobId);
					this.clearCachedTranslation(messageId);
			...
			cancelHistoricalTranslationJobs (channelId = null, reason = "cancelled") {
				historicalDisplayTracker.clear(channelId);
				const entries = channelId ? [this.getHistoricalTranslationJobQueue(channelId, false)].filter(Boolean) : this.ensureHistoricalJobRegistry().listQueues();
				for (const entry of entries) {
					...
					for (const job of entry.jobs) {
						job.cancel(reason);
						for (const record of job.items.values()) if (record.source && record.source.message) this.ensureLiveTranslationQueue().clearHistoricalQueuedMessage(record.source.message.id, job.id);
					}
					entry.jobs = [];
			...
			prepareHistoricalTranslationJobItem (queueItem, job) {
				...
				if (!queueItem.terminalRouteId) {const observed = this.observeHistoricalAutoEligibility(queueItem.message, queueItem.channel || {id: job.channelId}, queueItem.originalContentData || this.extractOriginalContentData(queueItem.message), {origin: "historical-load", ignoreQueued: true}); Object.defineProperty(queueItem, "terminalRouteId", {value: observed && observed.routeId || this.beginTranslationTerminalRoute({lane: "history-primary", entry: "historical-load", eligibility: "unknown", shape: queueItem.originalContentData && queueItem.originalContentData.embeds && queueItem.originalContentData.embeds.length ? "embed-forward" : "text", promptFamily: "batch-json", validatorFamily: "history-batch", requestFamily: "batch-json"}), writable: true, configurable: true});}
				if (queueItem.originalContentData && queueItem.originalContentData.embeds && queueItem.originalContentData.embeds.length) this.updateTranslationTerminalRoute(queueItem.terminalRouteId, {laneTag: "embed-forward"});
				this.recordTranslationTerminalStage(queueItem.terminalRouteId, "precheck", "started");
				...
				const prepared = this.prepareHistoricalAiBatchQueueItem(queueItem, channelId, input, output), cacheLookup = queueItem.historicalCacheLookup || null, cacheHit = !!(prepared && prepared.cachedTranslation || cacheLookup && cacheLookup.hadEntry && prepared && prepared.skipped && !cacheLookup.invalidated);
				this.observeReceivedBodyTranslationPlan(queueItem.originalContentData && queueItem.originalContentData.content || queueItem.message.content || "", {lane: "history-primary", channelId, legacyHardSkip: null, legacyEligibility: prepared && !prepared.skipped ? "eligible" : "filtered"}); this.observeTranslationDocumentPlan(queueItem.originalContentData || {body: queueItem.message.content || ""}, {lane: "history-primary", direction: "received", channelId});
				historicalBatchPerformance.recordCache(job, {hit: cacheHit ? 1 : 0, miss: cacheHit ? 0 : 1, invalidated: cacheLookup && cacheLookup.invalidated ? 1 : 0});
				if (!prepared) {this.recordTranslationTerminalStage(queueItem.terminalRouteId, "precheck", "prepare_failed"); return {status: "failed", reason: "prepare_failed"};}
				if (prepared.cachedTranslation) {this.recordTranslationTerminalStage(queueItem.terminalRouteId, "cache", "translation_hit", {eligibility: "eligible", cacheRead: "translation-hit", laneTag: "cache-hit"}); return {status: "translated", translation: Object.assign({channelId, auto: true}, prepared.cachedTranslation)};}
				if (prepared.skipped) {this.recordTranslationTerminalStage(queueItem.terminalRouteId, prepared.skipReason ? "cache" : "precheck", prepared.skipReason || "local_guard", {eligibility: "filtered", sourceFilterReason: prepared.skipReason || "local_guard", cacheRead: prepared.skipReason ? "skip-hit" : "none"}); return {status: "skipped", reason: prepared.skipReason || "local_guard"};}
				this.recordTranslationTerminalStage(queueItem.terminalRouteId, "protection", "rules_applied", Object.assign({eligibility: "eligible"}, summarizeProtectionRules(prepared.exceptions)));
				return {status: "pending", prepared};
			}
			translateHistoricalTranslationJobBatch (preparedItems, job, onChunkOutcome = null) {
				if (!preparedItems.length || !this.isHistoricalTranslationJobCurrent(job)) return Promise.resolve(null);
				const engineKey = this.getHistoricalAiBatchEngineKey(job.channelId), primaryEngineKey = engineKey || this.getHistoricalPrimaryEngineKey(job.channelId);
				if (!primaryEngineKey) return Promise.resolve({translations: null, failureKind: "configuration", statusCode: null});
				const recorded = new Set();
				const recordSettled = items => {
					for (const prepared of items) {
						if (recorded.has(prepared)) continue;
						recorded.add(prepared);
						const routeId = prepared && prepared.queueItem && prepared.queueItem.terminalRouteId;
						try {if (routeId) this.recordHistoricalAutoCheckpoint(routeId, "primary", "settled");} catch (error) {}
						try {if (routeId && prepared.semanticCompatibilityFallback) this.recordTranslationTerminalStage(routeId, "repair", "legacy_batch_compatibility_fallback", {providerDispatch: true, providerRole: "fallback", requestFamily: "legacy-batch-fallback", semanticRevision: prepared.semanticFallbackRevision});} catch (error) {}
					}
				};
				const notifyOutcome = event => {recordSettled(event.preparedItems); if (typeof onChunkOutcome === "function") return onChunkOutcome(event);};
				const progress = update => this.updateLoadedAutoTranslationStatus({channelId: job.channelId, jobId: job.id, processed: update.answered});
				if (!engineKey) return historicalBatchPerformance.runClassicPrimaryItems(this, primaryEngineKey, preparedItems, job, (prepared, timingContext) => this.requestHistoricalClassicPrimaryItem(prepared, job, timingContext), progress, notifyOutcome).then(outcome => {recordSettled(preparedItems); return outcome;});
				for (const prepared of preparedItems) {
					const routeId = prepared && prepared.queueItem && prepared.queueItem.terminalRouteId;
					if (routeId) this.recordTranslationTerminalStage(routeId, "provider", "dispatch", {providerRole: "primary", requestFamily: "batch-json", engineFamily: isCustomEngineKey(engineKey) ? "custom" : this.supportsAiAutoTranslateDecisionEngine(engineKey) ? "ai" : "classic", decisionApplied: this.shouldUseAiAutoTranslateDecision(job.channelId), promptFamily: "batch-json", historyCheckpoint: {stage: "primary", reason: "dispatch"}});
				}
				return historicalBatchPerformance.runPrimaryBatch(this, engineKey, preparedItems, job, progress, notifyOutcome).then(outcome => {recordSettled(preparedItems); return outcome;});
			}
			requestHistoricalClassicPrimaryItem (prepared, job, timingContext) {return new Promise(resolve => {if (!prepared || !prepared.message || !this.isHistoricalTranslationJobCurrent(job)) return resolve({cancelledBeforeDispatch: true}); const engineKey = this.getHistoricalPrimaryEngineKey(job.channelId), routeId = prepared.queueItem && prepared.queueItem.terminalRouteId, messageId = String(prepared.message.id), semanticRequest = prepared.semanticRequest && prepared.semanticRequest.adapter === "classic-marked" ? prepared.semanticRequest : null; if (!engineKey || !translationEngines[engineKey]) return resolve({translations: null, failureKind: "configuration", statusCode: null}); prepared.classicPrimaryDispatched = true; prepared.wireObservationToken = timingContext && timingContext.token || prepared.wireObservationToken; if (routeId) this.recordTranslationTerminalStage(routeId, "provider", "dispatch", {providerRole: "primary", requestFamily: semanticRequest ? "classic-marked" : "single-text", engineFamily: isCustomEngineKey(engineKey) ? "custom" : this.supportsAiAutoTranslateDecisionEngine(engineKey) ? "ai" : "classic", decisionApplied: false, promptFamily: semanticRequest ? "segment-data-only" : "single-manual", validatorFamily: semanticRequest ? "segment-validator-v3" : "history-batch", semanticRevision: semanticRequest && semanticRequest.semanticRevision || null, historyCheckpoint: {stage: "primary", reason: "dispatch"}}); this[translationEngines[engineKey].funcName]({input: prepared.input, output: prepared.output, text: semanticRequest ? semanticRequest.wire : prepared.protectedText, specialCase: null, engine: translationEngines[engineKey], autoDecision: false, decisionPrompt: "", timingContext: timingContext ? Object.assign({}, timingContext, {wireObservationProbe: prepared.wireObservationProbe, lane: "history-primary", engineFamily: "classic"}) : timingContext, requestContext: timingContext && timingContext.requestContext || null, semanticRequest: semanticRequest ? {adapter: semanticRequest.adapter, semanticRevision: semanticRequest.semanticRevision, targetLanguageId: semanticRequest.targetLanguageId, wire: semanticRequest.wire, segmentOrder: semanticRequest.segmentOrder} : null, silent: true}, translation => {if (!this.isHistoricalTranslationJobCurrent(job)) return resolve({cancelledBeforeDispatch: true}); resolve({translations: {[messageId]: semanticRequest && translation ? {semanticWire: translation} : translation || null}, failureKind: null, statusCode: null});});});}
			repairHistoricalTranslationJobBatch (preparedItems, job) {
				if (!preparedItems.length || !this.isHistoricalTranslationJobCurrent(job)) return Promise.resolve(null);
				preparedItems = preparedItems.filter(prepared => !prepared.semanticRequest || this.advanceHistoricalSemanticRepair(prepared)); if (!preparedItems.length) return Promise.resolve(null);
				for (const prepared of preparedItems) {this.observeReceivedBodyTranslationPlan(prepared.originalContentData && prepared.originalContentData.content || "", {lane: "batch-repair", channelId: job.channelId, legacyHardSkip: false, legacyEligibility: "eligible"}); this.observeTranslationDocumentPlan(prepared.originalContentData, {lane: "batch-repair", direction: "received", channelId: job.channelId});}
				for (const prepared of preparedItems) {const routeId = prepared && prepared.queueItem && prepared.queueItem.terminalRouteId; if (routeId) this.recordTranslationTerminalStage(routeId, "repair", "batch_repair", {laneTag: "batch-repair", promptFamily: "batch-json", historyCheckpoint: {stage: "batch-repair", reason: "planned"}});}
				const engineKey = this.getHistoricalAiBatchEngineKey(job.channelId);
				if (!engineKey) return Promise.resolve(null);
				for (const prepared of preparedItems) {const routeId = prepared && prepared.queueItem && prepared.queueItem.terminalRouteId; if (routeId) this.recordTranslationTerminalStage(routeId, "repair", "batch_repair_dispatch", {providerDispatch: true, providerRole: "repair", requestFamily: "batch-json", engineFamily: isCustomEngineKey(engineKey) ? "custom" : this.supportsAiAutoTranslateDecisionEngine(engineKey) ? "ai" : "classic", decisionApplied: this.shouldUseAiAutoTranslateDecision(job.channelId), historyCheckpoint: {stage: "batch-repair", reason: "dispatch"}});}
				...
				return historicalBatchPerformance.runRepairBatch(this, job, preparedItems.length, timingContext => this.requestAiBatchTranslationDetailed(engineKey, preparedItems, timingContext), engineKey, preparedItems).then(outcome => {for (const prepared of preparedItems) {const routeId = prepared && prepared.queueItem && prepared.queueItem.terminalRouteId; if (routeId && prepared.semanticCompatibilityFallback) this.recordTranslationTerminalStage(routeId, "repair", "legacy_batch_compatibility_fallback", {providerDispatch: true, providerRole: "fallback", requestFamily: "legacy-batch-fallback", semanticRevision: prepared.semanticFallbackRevision});} return outcome;});
			}
			advanceHistoricalSemanticRepair (prepared) {if (!prepared || !prepared.semanticRequest || !prepared.semanticOutcome) return prepared; const repair = this.planAtomicSemanticRepair(prepared.semanticRequest, prepared.semanticOutcome, {parentSettled: true, maxItems: 4096, maxChars: 60000}); if (!repair.dispatchable || repair.requests.length !== 1) return null; prepared.semanticRequest = repair.requests[0]; prepared.semanticPriorValid = prepared.semanticOutcome.valid; prepared.semanticRepair = true; prepared.protectedText = prepared.semanticRequest.wire; return refreshHistoricalWireObservationProbe(this, prepared);}
			validateHistoricalTranslationJobResult (prepared, rawTranslation, job) {return validateHistoricalTranslationJobResult(this, prepared, rawTranslation, job, historicalBatchPerformance);}
			repairHistoricalTranslationJobItem (prepared, job) {
				if (!prepared || !prepared.message || !this.isHistoricalTranslationJobCurrent(job)) return Promise.resolve({status: "failed", reason: "stale_job"});
				this.observeReceivedBodyTranslationPlan(prepared.originalContentData && prepared.originalContentData.content || "", {lane: "item-repair", channelId: job.channelId, legacyHardSkip: false, legacyEligibility: "eligible"}); this.observeTranslationDocumentPlan(prepared.originalContentData, {lane: "item-repair", direction: "received", channelId: job.channelId});
				const requestText = this.buildTranslationRequestText(prepared.originalContentData), routeId = prepared && prepared.queueItem && prepared.queueItem.terminalRouteId;
				if (routeId) this.recordTranslationTerminalStage(routeId, "repair", "item_repair", {laneTag: "item-repair", promptFamily: prepared.semanticRequest ? "segment-data-only" : "single-manual", validatorFamily: prepared.semanticRequest ? "segment-validator-v3" : "single-text", historyCheckpoint: {stage: "item-repair", reason: "dispatch"}});
				if (prepared.semanticRequest) {if (!this.advanceHistoricalSemanticRepair(prepared)) return Promise.resolve({status: "failed", reason: "attempt_limit"}); const engineKey = prepared.semanticRequest.engineKey || this.getHistoricalAiBatchEngineKey(job.channelId) || this.getHistoricalPrimaryEngineKey(job.channelId); if (!engineKey || !translationEngines[engineKey]) return Promise.resolve({status: "failed", reason: "configuration"}); if (routeId) this.recordTranslationTerminalStage(routeId, "repair", "segment_repair_dispatch", {providerDispatch: true, providerRole: "repair", requestFamily: prepared.semanticRequest.adapter, engineFamily: isCustomEngineKey(engineKey) ? "custom" : this.supportsAiAutoTranslateDecisionEngine(engineKey) ? "ai" : "classic", validatorFamily: "segment-validator-v3", semanticRevision: prepared.semanticRequest.semanticRevision, historyCheckpoint: {stage: "item-repair", reason: "dispatch"}}); return historicalBatchPerformance.runRepairItem(this, job, historicalTraceContext => new Promise(resolve => {if (!this.isHistoricalTranslationJobCurrent(job)) return resolve({status: "failed", reason: "stale_job"}); const request = prepared.semanticRequest, timingContext = historicalTraceContext.createTimingContext({role: "retry", observationRole: "repair", engineKey, engineFamily: isCustomEngineKey(engineKey) ? "custom" : this.supportsAiAutoTranslateDecisionEngine(engineKey) ? "ai" : "classic", lane: "item-repair", messageCount: 1, historicalBatch: false, wireObservationProbe: prepared.wireObservationProbe}), requestContext = timingContext && timingContext.requestContext || null; prepared.wireObservationToken = timingContext && timingContext.token || prepared.wireObservationToken; this[translationEngines[engineKey].funcName]({input: prepared.input, output: prepared.output, text: request.wire, specialCase: null, engine: translationEngines[engineKey], autoDecision: false, decisionPrompt: "", timingContext, requestContext, semanticRequest: {adapter: request.adapter, semanticRevision: request.semanticRevision, targetLanguageId: request.targetLanguageId, systemPrompt: request.systemPrompt, wire: request.wire, segmentOrder: request.segmentOrder}, silent: true}, raw => {if (!this.isHistoricalTranslationJobCurrent(job)) return resolve({status: "failed", reason: "stale_job"}); const semantic = this.validateAtomicSemanticResponse(request, raw, {priorValid: prepared.semanticPriorValid || {}, likelyTarget: value => this.isTranslationLikelyInTargetLanguage(value, prepared.output && prepared.output.id), similarity: (source, value) => this.getTextSimilarityScore(source, value), maxSimilarity: 0.94}); prepared.semanticOutcome = semantic; historicalBatchPerformance.recordParseValidate(job, {parsed: 1, valid: semantic.ok ? 1 : 0, invalid: semantic.ok ? 0 : 1}); if (!semantic.ok) return resolve({status: "failed", reason: semantic.reason || "semantic_invalid"}); if (semantic.unchanged) return resolve({status: "skipped", reason: "ai_skip_signal"}); const stored = this.createStoredReceivedTranslationData(prepared.message, job.channelId, prepared.originalContentData, prepared.signature, semantic.translation, prepared.input, prepared.output, true); if (stored) {const fields = request.workload && request.workload.fields || {}; stored.semanticRevision = request.semanticRevision; stored.semanticWorkloadKey = request.workload && request.workload.key || null; stored.plannerVersion = request.plan && request.plan.plannerVersion || null; stored.planHash = this.getAtomicSemanticPlanHash(request); stored.validatorVersion = fields.validatorVersion || null; stored.outputSchemaVersion = fields.outputSchemaVersion || null; if (semantic.keptCount > 0) {stored.keptSegmentCount = semantic.keptCount; stored.keptReasons = Object.assign({}, semantic.keptReasons || {});}} resolve(stored ? {status: "translated", translation: stored} : {status: "failed", reason: "unknown"});});}), prepared).then(outcome => outcome && outcome.cancelledBeforeDispatch ? {status: "failed", reason: "stale_job"} : outcome);}
				return historicalBatchPerformance.runRepairItem(this, job, historicalTraceContext => new Promise(resolve => {
						...
						if (!this.isHistoricalTranslationJobCurrent(job)) return resolve({status: "failed", reason: "stale_job"});
						historicalBatchPerformance.recordParseValidate(job, {parsed: 1, valid: translation ? 1 : 0, invalid: translation ? 0 : 1});
						if (!translation) {historicalBatchPerformance.recordValidationReason(job, prepared, meta.skipped ? "policy_rejected" : "empty", {repairEligible: false, phase: "item"}); return resolve({status: meta.skipped ? "skipped" : "failed", reason: meta.skipped ? meta.reason || "same_language" : "provider_failed"});}
						...
						const tooSimilar = storedTranslation && !rejectReason ? this.isTranslationResultTooSimilar(storedTranslation) : false;
						const validationReason = classifyHistoricalStoredRejection(storedTranslation, rejectReason, tooSimilar);
						if (validationReason) {historicalBatchPerformance.recordValidationReason(job, prepared, validationReason, {repairEligible: false, phase: "item"}); return resolve({status: "skipped", reason: rejectReason || "too_similar"});}
						resolve({status: "translated", translation: storedTranslation});
					}, null, {showToast: false, showFailureToast: false, trackBusy: false, auto: true, forcePlainTranslation: true, channelId: job.channelId, latencyKind: "historical", messageCount: 1, historicalTraceContext, terminalLane: "item-repair", terminalEntry: "historical-repair", terminalProviderRole: "repair"});
				}), prepared).then(outcome => outcome && outcome.cancelledBeforeDispatch ? {status: "failed", reason: "stale_job"} : outcome);
			}
			...
			isHistoricalTranslationJobItemCurrent (item, job, observation = null) {
				...
				if (racingDisplayView && (racingDisplayView.translated || racingDisplayView.showLoading)) {if (observation) observation.displayOwnedCount++; return false;}
				...
				const current = this.createReceivedTranslationSignature(item.message, job.channelId, expectedContentData) == this.createReceivedTranslationSignature(currentMessage, job.channelId, currentContentData); if (!current && observation) observation.sourceChangedCount++; return current;
			}
			commitHistoricalTranslationJob (summary, job, options = {}) {return historicalCommitCacheFinalizer.commit(this, summary, job, options);}
			updateHistoricalTranslationJobStatus (job) {if (job && job.channelId) this.updateLoadedAutoTranslationStatus({channelId: job.channelId, jobId: job.id});}
			retryFailedHistoricalDisplays (channelId) {if (!pluginRuntimeActive || !this.isTranslationEnabled(channelId)) return 0; const count = historicalDisplayTracker.retryFailed({channelId, schedule: (id, key) => this.scheduleReceivedDisplayFlush(channelId, id, null, key, "historical")}); this.updateLoadedAutoTranslationStatus({channelId}); return count;}
			getLoadedHistoricalWorkStatus (channelId) {const queue = this.getHistoricalTranslationJobQueue(channelId, false), failed = this.ensureHistoricalJobRegistry().getFailedSnapshot(channelId); return projectHistoricalStatus({channelId, jobs: queue ? queue.jobs : [], display: historicalDisplayTracker.getSnapshot(channelId), automaticPendingIds: this.ensureReceivedDisplayRuntime().getAutomaticPendingMessageIds(channelId), retryableIds: (failed && failed.items || []).map(item => String(item.message.id)), includeEmpty: loadedTranslationStatusStore.isForChannel(channelId) && loadedTranslationStatusStore.getStatus().aggregate === true});}
			handleHistoricalDisplayOutcome (report) {return historicalDisplayTracker.handle(report);}
			...
			getHistoricalPrimaryEngineKey (channelId = null) {const engineKey = this.getEffectivePrimaryEngine(channelId), input = Object.assign({}, this.ensureSettingsStore().getLanguage(this.getLanguageChoice(languageTypes.INPUT, messageTypes.RECEIVED, channelId)) || {}), output = Object.assign({}, this.ensureSettingsStore().getLanguage(this.getLanguageChoice(languageTypes.OUTPUT, messageTypes.RECEIVED, channelId)) || {}); if (!input.id || !output.id || output.special) return null; return this.validTranslator(engineKey, input, output, null) ? engineKey : null;}
			getHistoricalAiBatchEngineKey (channelId = null) {const engineKey = this.getHistoricalPrimaryEngineKey(channelId); return engineKey && isAiBatchCapableEngineKey(engineKey) ? engineKey : null;}
			clearHistoricalTranslationFailures (channelId = null) {const registry = this.ensureHistoricalJobRegistry(); if (channelId) {const snapshot = registry.getFailedSnapshot(channelId), count = [].concat(snapshot && snapshot.items || []).length; registry.deleteFailedSnapshot(channelId); return count;} const count = registry.listFailedSnapshots ? registry.listFailedSnapshots().reduce((total, entry) => total + [].concat(entry && entry.items || []).length, 0) : 0; registry.clearFailedSnapshots(); return count;}

			prepareHistoricalAiBatchQueueItem (queueItem, channelId, input, output) {return prepareHistoricalAiBatchQueueItem(this, queueItem, channelId, input, output, messageTypes.RECEIVED);}
			...
			requestAiBatchTranslationDetailed (engineKey, preparedItems, timingContext = null) {
				...
				return this.ensureProviderClient().requestAiBatchTranslationDetailed(engineKey, preparedItems, timingContext);
			}
			...
			ensureHistoricalJobRegistry () {return this.historicalJobRegistryInstance || (this.historicalJobRegistryInstance = createHistoricalJobRegistry());}
			getHistoricalBatchPerformanceSnapshot () {return historicalBatchPerformance.getSnapshot();}
			setHistoricalBatchExperimentConcurrency (value, options = {}) {return historicalBatchPerformance.setConcurrency(value, options);}
			beginTranslationTerminalRoute (meta) {return translationTerminalLedger.begin(meta);}
			updateTranslationTerminalRoute (routeId, fields) {return translationTerminalLedger.update(routeId, fields);}
			recordTranslationTerminalStage (routeId, stage, reason, fields) {return translationTerminalLedger.stage(routeId, stage, reason, fields);}
			finishTranslationTerminalRoute (routeId, terminal) {const result = translationTerminalLedger.terminal(routeId, terminal); historyAutoTerminalObserver.release(routeId); return result;}
			observeHistoricalAutoEligibility (message, channel, source, options = {}) {const detailed = this.getReceivedAutoTranslateEligibility(message, channel, source, options.ignoreQueued === true), effective = this.shouldAutoTranslateReceivedMessage(message, channel, source, options.ignoreQueued === true), eligibility = effective === detailed.eligible ? detailed : {eligible: !!effective, reason: effective ? "eligible_override" : detailed.reason}; return historyAutoTerminalObserver.observe(message, channel, source, Object.assign({}, options, {eligibility}));}
			recordHistoricalAutoCheckpoint (routeId, stage, reason, fields = {}) {return historyAutoTerminalObserver.checkpoint(routeId, stage, reason, fields);}
			getHistoryAutoTerminalResources () {return historyAutoTerminalObserver.getResourceSnapshot();}
			getTranslationTerminalLedgerSnapshot () {const snapshot = translationTerminalLedger.getSnapshot(), contracts = [{matrixId: "self-authored-received", entry: "received-auto", eligibility: "filtered", sourceFilterReason: "self_authored", lane: "none", shape: "text", engineFamily: "none", decisionApplied: null, promptFamily: "none", validatorFamily: "received-filter", placeholderOccurrences: 0, ruleCounts: {}, cacheRead: "none", cacheWrite: "none", outcome: "skipped", stage: "precheck", reason: "self_authored", providerDispatchCount: 0, providerRoles: {}, request: {providerFamily: "none", role: null, promptFamily: "none", bodyBytes: null, bodyIdentity: null}, repairTrajectory: [], displayCommit: "not-entered"}, {matrixId: "received-skip-cache", entry: "received-auto", eligibility: "filtered", sourceFilterReason: "skip_cache", lane: "none", shape: "text", engineFamily: "none", decisionApplied: null, promptFamily: "none", validatorFamily: "received-filter", placeholderOccurrences: 0, ruleCounts: {}, cacheRead: "skip-hit", cacheWrite: "none", outcome: "skipped", stage: "precheck", reason: "skip_cache", providerDispatchCount: 0, providerRoles: {}, request: {providerFamily: "none", role: null, promptFamily: "none", bodyBytes: null, bodyIdentity: null}, repairTrajectory: [], displayCommit: "not-entered"}], currentBehaviorMatrix = createTranslationCurrentBehaviorMatrix(contracts.concat(buildMatrixRowsFromTerminalLedger(snapshot))); return Object.freeze(Object.assign({}, snapshot, {currentBehaviorMatrix}));}
			observeReceivedBodyTranslationPlan (source, metadata = {}) {try {const channelId = metadata.channelId, input = this.ensureSettingsStore().getLanguage(this.getLanguageChoice(languageTypes.INPUT, messageTypes.RECEIVED, channelId)) || {id: "auto"}, output = this.ensureSettingsStore().getLanguage(this.getLanguageChoice(languageTypes.OUTPUT, messageTypes.RECEIVED, channelId)) || {id: "zh-CN"}, [, segments] = this.removeExceptions(String(source || ""), messageTypes.RECEIVED), protection = summarizeProtectionRules(segments); return translationPlanShadowStore.observe(Object.assign({}, metadata, {source: String(source || ""), direction: "received", fieldPath: "body", inputLanguageId: input.id || "auto", targetLanguageId: output.id || "zh-CN", legacyProtectedSegments: segments, legacyRuleCounts: protection.ruleCounts}));} catch (error) {return null;}}
			observeTranslationDocumentPlan (source, metadata = {}) {try {const channelId = metadata.channelId, output = this.ensureSettingsStore().getLanguage(this.getLanguageChoice(languageTypes.OUTPUT, metadata.direction == "sent" ? messageTypes.SENT : messageTypes.RECEIVED, channelId)) || {id: metadata.direction == "sent" ? "en" : "zh-CN"}; return translationDocumentShadowStore.observe(source || {}, Object.assign({}, metadata, {targetLanguageId: output.id}));} catch {return null;}}
			getTranslationDocumentShadowSnapshot () {return translationDocumentShadowStore.getSnapshot();}
			resetTranslationDocumentShadow () {return translationDocumentShadowStore.reset();}
			getTranslationPlanShadowSnapshot () {return translationPlanShadowStore.getSnapshot();}
			resetTranslationPlanShadow () {return translationPlanShadowStore.reset();}
			flushTranslationPlanShadow () {return translationPlanShadowStore.flush();}
			resetTranslationTerminalLedger () {return translationTerminalLedger.reset();}
			flushTranslationTerminalLedger () {return translationTerminalLedger.flush();}
			getProtectionRuleSummary (segments) {return summarizeProtectionRules(segments);}
			resetHistoricalPrimarySamples () {return historicalBatchPerformance.resetPrimarySamples();}
			flushHistoricalPrimarySamples () {return historicalBatchPerformance.flushPrimarySamples();}
			...
			ensureLiveTranslationQueue () {
				if (!this.liveTranslationQueueInstance) this.liveTranslationQueueInstance = createPluginLiveTranslationQueue({plugin: this, BDFDB, loadedTranslationStatusStore, historicalBatchPerformance, getRuntimeActive: () => pluginRuntimeActive, languageTypes, messageTypes});
			...
			ensureReceivedDisplayRepaintScheduler () {
				if (!this.receivedDisplayRepaintSchedulerInstance) this.receivedDisplayRepaintSchedulerInstance = createPluginDisplayRepaintScheduler({plugin: this, BDFDB, onRenderOutcome: report => {historicalDisplayTracker.handle(report); this.ensureLiveTranslationQueue().performanceTrace.onRenderOutcome(report);}});
			...
			processMessageContent (e) {
				...
				if (displayView && !translationDisplayLogic.isForwardContainerMessage(this, message)) e.returnvalue.props["data-translator-revision"] = String(displayView.revision);
			...
			getEffectivePrimaryEngine (channelId = null) {return this.ensureSettingsStore().getChannelPrimaryEngineOverride(channelId) || this.getGlobalPrimaryEngine();}

			getEffectiveBackupEngine (channelId = null) {const backupEngineKey = this.settings && this.settings.engines && this.settings.engines.backup; return !translationEngines[backupEngineKey] || backupEngineKey == this.getEffectivePrimaryEngine(channelId) ? "----" : backupEngineKey;}
			shouldUseAtomicSemanticRevision (place, channelId = null) {return place == messageTypes.RECEIVED && isOpenAiCompatibleEngineKey(this.getEffectivePrimaryEngine(channelId));}
			createProtectedSemanticRequest (source, options = {}) {try {const place = options.place || messageTypes.RECEIVED, rawSource = String(source || ""), protection = this.prepareSemanticSource(rawSource, place), preference = this.getAiAutoTranslatePrompt({input: this.getLanguageData(options.inputLanguageId || "auto"), output: this.getLanguageData(options.targetLanguageId || "zh-CN")}), request = translationSemanticRuntime.createSemanticRequest(Object.assign({}, options, {source: protection.source, direction: place == messageTypes.SENT ? "sent" : "received", customPrompt: preference, customPromptDigest: translationPreferenceDigest(preference)})); if (request && request.enabled) translationSemanticRuntime.attachSemanticLocalState(request, {protectedSegments: protection.protectedSegments, cachePlanHash: translationSemanticRuntime.hashSemanticSource(rawSource)}); return request;} catch {return Object.freeze({enabled: false, fallbackReason: "planner-error", semanticRevision: "legacy"});}}
			createAtomicSemanticRevisionContract (source, options = {}) {try {const place = options.place || messageTypes.RECEIVED, channelId = options.channelId || null; if (!this.shouldUseAtomicSemanticRevision(place, channelId)) return Object.freeze({enabled: false, fallbackReason: "capability-unverified", semanticRevision: "legacy"}); return this.createProtectedSemanticRequest(source, Object.assign({}, options, {place, engineKey: options.engineKey || this.getEffectivePrimaryEngine(channelId)}));} catch {return Object.freeze({enabled: false, fallbackReason: "planner-error", semanticRevision: "legacy"});}}
			getAtomicSemanticLocalState (request) {return translationSemanticRuntime.getSemanticLocalState(request);}
			getAtomicSemanticPlanHash (request) {const state = this.getAtomicSemanticLocalState(request); return state.cachePlanHash || request && request.rootPlan && request.rootPlan.sourceHash || request && request.plan && request.plan.sourceHash || null;}
			validateAtomicSemanticResponse (request, value, options = {}) {const outcome = translationSemanticRuntime.validateSemanticResponse(request, value, options); if (!outcome || !outcome.ok || !outcome.translation) return outcome; const state = this.getAtomicSemanticLocalState(request), translation = this.addSemanticExceptions(outcome.translation, state.protectedSegments); return translation === outcome.translation ? outcome : Object.freeze(Object.assign({}, outcome, {translation}));}
			planAtomicSemanticRepair (request, outcome, options = {}) {return translationSemanticRuntime.planSemanticRepair(request, outcome, options);}
			...
			engineSupportsLanguagePair (engineKey, input, output) {
				const engine = translationEngines[engineKey], supported = this.engineSupportsLanguage(engineKey, input) && this.engineSupportsLanguage(engineKey, output);
				return !!(output && output.special || supported && (!engine.languagePairs || input.auto || engine.languagePairs.some(pair => pair[0] == input.id && pair[1] == output.id || pair[0] == output.id && pair[1] == input.id)));
			}
			...
			async toggleTranslation (channelId) {
				...
				if (wasEnabled) {
					...
					const displayGeneration = this.getReceivedDisplayGeneration(channelId), queue = this.getHistoricalTranslationJobQueue(channelId, false), queuedMessageIds = (queue && queue.jobs || []).flatMap(job => [...job.items.keys()]);
					...
					this.resetAutoTranslationTracking(channelId);
					try {
						const outcome = await this.restoreReceivedDisplayChannel(channelId, {clearPreviews: true, clearSuppressions: true, queuedMessageIds});
						if (channelToggleOperations.isCurrent(channelId, operationVersion) && !this.isTranslationEnabled(channelId)) {
							for (const messageId of new Set([].concat(outcome && outcome.missingIds || [], outcome && outcome.retryIds || []))) this.scheduleReceivedDisplayFlush(channelId, messageId, null, null, "restore");
						}
					}
			...
			setLanguages () {
				// Register user-defined OpenAI-compatible engines before validity checks.
				syncCustomEngines(this.settings.engines);
				let engineSelectionChanged = false;
				if (!translationEngines[this.settings.engines.translator]) {this.settings.engines.translator = "googleapi"; engineSelectionChanged = true;}
				if (this.settings.engines.backup != "----" && !translationEngines[this.settings.engines.backup]) {this.settings.engines.backup = "----"; engineSelectionChanged = true;}
				if (this.settings.engines.translator == this.settings.engines.backup) {
					this.settings.engines.backup = "----";
					engineSelectionChanged = true;
				}
				if (engineSelectionChanged) BDFDB.DataUtils.save(this.settings.engines, this, "engines");
				...
				const builtLanguages = BDFDB.ObjectUtils.deepAssign(
					!Object.values(translationEngines).some(translationEngine => translationEngine.auto) ? {} : {
						auto: {
							auto: true,
							name: resolveDetectLanguageLabel(this.getUiLanguageId(), this.labels),
							id: "auto"
						}
					},
			...
			getChineseLanguageName (languageId) {
				...
				try {
					if (typeof Intl != "undefined" && typeof Intl.DisplayNames == "function") {
						const displayNames = new Intl.DisplayNames(["zh-Hans"], {type: "language", fallback: "none"});
			...
			getLanguageDisplayName (language) {
				...
				if (!languageData) return "";
				if (languageData.auto) return resolveDetectLanguageLabel(this.getUiLanguageId(), this.labels);
				// One name per interface language; the old tri-script composite crowded every dropdown.
				if (this.isChineseUiLanguage()) return this.getChineseLanguageName(languageData.id) || languageData.ownlang || languageData.id;
				return languageData.name || BDFDB.LanguageUtils.getName(languageData) || languageData.id;
			}
			...
			protectRegexMatches (string, regex, protectedSegments = {}, count = 0, options = {}) {return protectionLogic.protectRegexMatches(this, string, regex, protectedSegments, count, options);}
			protectCodeBlockSegments (string, protectedSegments = {}, count = 0) {return protectionLogic.protectCodeBlockSegments(this, string, protectedSegments, count);}
			protectAutoDetectedSegments (string, protectedSegments = {}, count = 0) {return protectionLogic.protectAutoDetectedSegments(this, string, protectedSegments, count);}
			protectDiscordMarkupSegments (string, protectedSegments = {}, count = 0) {return protectionLogic.protectDiscordMarkupSegments(this, string, protectedSegments, count);}
			protectWrappedTextSegments (string, protectedSegments = {}, count = 0, place = null) {return protectionLogic.protectWrappedTextSegments(this, string, protectedSegments, count, place);}
			protectConfiguredTerms (string, protectedSegments = {}, count = 0) {return protectionLogic.protectConfiguredTerms(this, string, protectedSegments, count);}
			protectAutoTechnicalTerms (string, protectedSegments = {}, count = 0, place = null) {return protectionLogic.protectAutoTechnicalTerms(this, string, protectedSegments, count, place);}
			protectMixedLanguageLatinTokens (string, protectedSegments = {}, count = 0) {return protectionLogic.protectMixedLanguageLatinTokens(this, string, protectedSegments, count);}
			getUnicodeEmojiDetector () {return protectionLogic.getUnicodeEmojiDetector();}
			isUnicodeEmojiGrapheme (segment) {return protectionLogic.isUnicodeEmojiGrapheme(this, segment);}
			getUnicodeEmojiRegex () {return protectionLogic.getUnicodeEmojiRegex();}
			protectUnicodeEmojiSegments (string, protectedSegments = {}, count = 0) {return protectionLogic.protectUnicodeEmojiSegments(this, string, protectedSegments, count);}
			createProtectionPlaceholder (count) {return protectionLogic.createProtectionPlaceholder(this, count);}
			getProtectionPlaceholderRegex (count) {return protectionLogic.getProtectionPlaceholderRegex(this, count);}
			formatProtectedExceptionForDisplay (exception) {return protectionLogic.formatProtectedExceptionForDisplay(this, exception);}
			hasAllProtectionPlaceholders (string, protectedSegments) {return protectionLogic.hasAllProtectionPlaceholders(this, string, protectedSegments);}
			addExceptions (string, protectedSegments) {return protectionLogic.addExceptions(this, string, protectedSegments);}
			prepareSemanticSource (string, place = messageTypes.RECEIVED) {return protectionLogic.prepareSemanticSource(this, string, place);}
			addSemanticExceptions (string, protectedSegments) {return protectionLogic.addSemanticExceptions(this, string, protectedSegments);}
			removeExceptions (string, place) {return protectionLogic.removeExceptions(this, string, place);}

			getGoogleTranslatePageURL (input, output, text) {return `https://translate.google.com/#${BDFDB.LanguageUtils.languages[input] ? input : "auto"}/${output}/${encodeURIComponent(text)}`;}
			setLabelsByLanguage () {return getLabelsForUiLanguage(this.getUiLanguageId());}
		};
