...
const LOADED_STATUS_PHASES = Object.freeze(["collecting", "queued", "requesting", "repairing", "committing", "displaying", "done", "failed"]);
...
const LOADED_STATUS_PROGRESS_FIELDS = Object.freeze(["total", "processed", "displayed", "displayPending", "displayFailed", "readyCount", "workTotal", "skipped", "failed"]);
...
function getWorkElapsed(status, currentTime) {
	if (status && status.workElapsedMs != null) return formatSeconds(status.workElapsedMs + (status.active && status.workStartedAt != null ? Math.max(0, currentTime - status.workStartedAt) : 0));
	if (!status || status.workStartedAt == null) return null;
	return formatSeconds((status.active ? currentTime : status.workFinishedAt ?? currentTime) - status.workStartedAt);
}
...
function getPhaseLabel(phase, chinese) {
	switch (phase) {
		...
		case "displaying": return chinese ? "待显示" : "awaiting display";
...
function renderCompactStatusText(status, currentTime) {
	...
	const elapsed = getWorkElapsed(status, currentTime) || formatSeconds(status && status.active && status.phaseStartedAt ? currentTime - status.phaseStartedAt : 0);
	// Successful completion shows only the cumulative ratio. Cache-only jobs may
	// finish within a second; their 0s must not remain on the completed capsule.
	const complete = status && status.done && !status.active && !displayPending && !status.displayFailed && !failed && !retryable;
	const duration = complete ? "" : ` ${elapsed}`;
	if (status && status.aggregate) {
		return `${status.readyCount}/${status.workTotal}${duration}`;
	}
	if (status && status.displayFailed) return `${sessionDisplayed}/${sessionDisplayed}${duration}`;
	...
	if (status && status.phase === "repairing") return `${repairReady}/${repairReady + (retryable || failed)}${duration}`;
	if (status && (status.phase === "failed" || status.done && (failed || retryable))) {
		const shown = Math.max(sessionDisplayed, displayed);
		return `${shown}/${shown + retryable}${duration}`;
	}
	if (status && status.done && displayPending) return `${Math.max(sessionDisplayed, displayed)}/${Math.max(sessionDisplayed, displayed) + Math.max(0, total - displayed - skipped - failed)}${duration}`;
	...
	// the classic displayed/total reading. Only unfinished work shows elapsed time;
	// failure and pending details belong in the hover text and recovery action.
	if (status && status.done) return `${sessionDisplayed}/${sessionDisplayed}${duration}`;
	...
	return `${cumulativeRatio}${duration}`;
}
...
function renderStatusDetailText(status, chinese, phaseSegment) {
	...
	if (status && status.aggregate) {
		const label = getPhaseLabel(status.phase, chinese);
		const phaseText = phaseSegment || (chinese ? `，${label}` : `, ${label}`);
		const pending = status.displayPending || 0, displayFailed = status.displayFailed || 0;
		return chinese
			? `已加载翻译：本次累计 ${status.readyCount}/${status.workTotal}${phaseText}${pending ? `，待显示 ${pending}` : ""}${displayFailed ? `，显示失败 ${displayFailed}` : ""}${retryable ? `，翻译待重试 ${retryable}` : ""}${skipped ? `，跳过 ${skipped}` : ""}`
			: `Loaded translation: session ${status.readyCount}/${status.workTotal}${phaseText}${pending ? `, ${pending} awaiting display` : ""}${displayFailed ? `, ${displayFailed} display failed` : ""}${retryable ? `, ${retryable} translation retry pending` : ""}${skipped ? `, skipped ${skipped}` : ""}`;
	}
...
function createLoadedTranslationStatusStore({
	...
	const sessionReadyIds = new Map();
	// Counts are cumulative; duration belongs only to the current/last work round.
	// Retain each channel's last duration when its visible capsule is cleared.
	const workClocks = new Map();
	...
	function updateAggregateCounts(next) {
		if (!next.aggregate) return;
		const committed = sessionDisplayedIds.get(normalizeChannelId(next.channelId)) || new Set();
		const pending = new Set(next.pendingMessageIds || []);
		let overlap = 0;
		for (const id of pending) if (committed.has(String(id))) overlap++;
		const key = normalizeChannelId(next.channelId);
		if (!sessionReadyIds.has(key)) sessionReadyIds.set(key, new Set());
		const ready = sessionReadyIds.get(key);
		// Recollecting cached rows does not undo an earlier completed translation.
		// Newly committed rows still wait for the display owner to release pending.
		for (const id of committed) if (!pending.has(String(id))) ready.add(String(id));
		next.readyCount = ready.size;
		next.workTotal = committed.size + pending.size - overlap + (next.retryableCounted ? 0 : Math.max(0, next.retryable || 0));
	}
	function finishClock(channelId) {
		const clock = workClocks.get(normalizeChannelId(channelId));
		if (clock && clock.startedAt != null) {clock.elapsedMs = Math.max(0, now() - clock.startedAt); clock.startedAt = null;}
	}
	function updateClock(next) {
		const key = normalizeChannelId(next.channelId);
		if (!workClocks.has(key)) workClocks.set(key, {elapsedMs: 0, startedAt: null});
		const clock = workClocks.get(key);
		// Empty collection/refresh is not a new translation round. Once work starts,
		// keep its clock through all jobs and phases until the channel is idle.
		const hasWork = next.total > 0 || next.displayPending > 0 || (next.pendingMessageIds || []).length > 0;
		if (next.active && hasWork && clock.startedAt == null) {clock.elapsedMs = 0; clock.startedAt = now();}
		if (!next.active) finishClock(key);
		next.workElapsedMs = clock.elapsedMs;
		next.workStartedAt = clock.startedAt;
	}
	...
	return Object.freeze({
		getStatus() {
			...
			return Object.assign({}, status, status.aggregate ? {pendingMessageIds: [...(status.pendingMessageIds || [])]} : null);
		},
		getChannelId() {
		...
		update(updates = {}) {
			...
			if (!updates.aggregate && updates && updates.batch != null && normalizeChannelId(updates.channelId != null ? updates.channelId : status.channelId) === normalizeChannelId(status.channelId) && Math.max(0, updates.batch) < Math.max(0, status.batch || 0)) return this.getStatus();
			const previous = status;
			const sameChannel = normalizeChannelId(updates.channelId == null ? previous.channelId : updates.channelId) === normalizeChannelId(previous.channelId);
			if (!sameChannel) finishClock(previous.channelId);
			const next = Object.assign({}, sameChannel ? previous : createEmptyStatus(), updates);
			const nextJobKey = `${normalizeChannelId(next.channelId)}:${next.jobId || Math.max(0, next.batch || 0)}`;
			...
			updateClock(next);
			if (!next.aggregate && sealedTotal === null && !next.collecting && (next.active || next.done) && next.total > 0) sealedTotal = Math.max(0, next.total || 0);
			if (!next.aggregate && sealedTotal !== null && !next.collecting) next.total = sealedTotal;
			updateAggregateCounts(next);
		...
		recordSessionDisplayed(channelId, messageIds = []) {
			...
			// Commit is not a render acknowledgement. The next owner status update
			// supplies pending IDs before any newly committed row may become ready.
			return this.getStatus();
		},
		setConfigurationSignature(channelId, signature) {
			...
			sessionReadyIds.delete(key);
			workClocks.delete(key);
