// Settings-panel and menu labels for every UI language BDFDB reports. Pure data:
// one branch per locale, no plugin state, no behavior.
...
function getGeneralSettingLabels({isChinese = false, isRussian = false} = {}) {
	...
	if (isRussian) Object.assign(labels, {
		...
		showOriginalInReplyPreview: "В ответах показывать только перевод",
...
// The visible name of the "auto" language entry, resolved from the plugin interface
// language at display time. setLanguages() bakes a name into the language table and
// BDFDB refreshes plugin.labels only when Discord's own language changes, so after the
// plugin language is switched in General both of those still carry the previous word.
function resolveDetectLanguageLabel(uiLanguageId, fallbackLabels) {
	const labels = uiLanguageId ? getLabelsForUiLanguage(uiLanguageId) : null;
	return labels && labels.detect_language || fallbackLabels && fallbackLabels.detect_language || "Detect Language";
}

module.exports = {getGeneralSettingLabels, getLabelsForUiLanguage, resolveDetectLanguageLabel};
