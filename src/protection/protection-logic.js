...
const TRANSLATION_PROTECTION_SIGNATURE_VERSION = "2026-08-29-semantic-protect-v12";
...
function createProtectionLogic({
	...
	const protectionLogic = {
		...
		trimTrailingUrlPunctuation(plugin, text) {
			let protectedText = String(text || ""), trailingText = "";
			const pairs = {")": "(", "]": "[", "}": "{", "）": "（", "】": "【", "》": "《"};
			while (protectedText) {
				const tail = protectedText.at(-1);
				if (/[,.;:!?\'"`，。！？；：、」]/.test(tail)) {
					trailingText = tail + trailingText;
					protectedText = protectedText.slice(0, -1);
					continue;
				}
				const open = pairs[tail];
				if (!open) break;
				const openCount = protectedText.split(open).length - 1;
				const closeCount = protectedText.split(tail).length - 1;
				if (closeCount <= openCount) break;
				trailingText = tail + trailingText;
				protectedText = protectedText.slice(0, -1);
			}
			return protectedText ? {protectedText, trailingText} : protectionLogic.trimTrailingProtectedPunctuation(plugin, text);
		},
		...
		protectAutoDetectedSegments(plugin, string, protectedSegments = {}, count = 0) {
			...
			const trimTrailing = fullMatch => protectionLogic.trimTrailingUrlPunctuation(plugin, fullMatch);
			result = protectionLogic.protectRegexMatches(plugin, string, /\bhttps?:\/\/[^\s<>\u3000]+/gi, protectedSegments, count, {normalize: trimTrailing});
			string = result.string;
			protectedSegments = result.protectedSegments;
			count = result.count;

			result = protectionLogic.protectRegexMatches(plugin, string, /\b(?:www\.)?(?:[a-z0-9](?:[a-z0-9-]{0,61}[a-z0-9])?\.)+[a-z]{2,24}(?::\d{1,5})?(?:[/?#][^\s<>\u3000]*)?/gi, protectedSegments, count, {
				normalize: fullMatch => {
			...
			result = protectionLogic.protectRegexMatches(plugin, string, /\b(?:\d{1,3}\.){3}\d{1,3}(?::\d{1,5})?\b/g, protectedSegments, count);
			string = result.string;
			protectedSegments = result.protectedSegments;
			count = result.count;

			string = string.replace(/(^|\s)(\/[A-Za-z][A-Za-z0-9_-]{0,31})(?=\s|$)/g, (fullMatch, leading, command) => {
		...
		protectConfiguredTerms(plugin, string, protectedSegments = {}, count = 0) {
			...
			for (let term of protectedTerms) {
				...
				// Earlier passes have already emitted exact local placeholders. Never let
				// a configured value such as "0" or "⟦" dismantle those markers.
				string = string.split(/(⟦\d+⟧)/g).map(part => /^⟦\d+⟧$/.test(part) ? part : part.replace(regex, (match, leading, protectedTerm) => {
		...
		protectAutoTechnicalTerms(plugin, string, protectedSegments = {}, count = 0, place = null) {
			if (!string) return {string, protectedSegments, count};
			...
			// The all-caps acronym guess ("API", but also "NO", "YES", emphasis like "do NOT") is
			// retired: it decided that whole messages had nothing to translate and left emphasised
			// words untranslated inside machine-translated sentences. The AI planner classifies
			// acronyms itself; users pin the ones they care about with protected terms.
		...
		getSemanticProtectionStartIndex(_plugin, string) {
			const source = String(string || "");
			const reserved = new Set();
			for (const match of source.matchAll(/(?:⟦\s*(?:DTA\s*)?\d+\s*⟧|【\s*\d+\s*】|\[\s*\d+\s*\]|<\s*<\s*<\s*\d+\s*>\s*>\s*>|[｛\{]\s*[｛\{]\s*\d+\s*[｝\}]\s*[｝\}])/g)) {
				const digits = String(match[0]).match(/\d+/), value = Number(digits && digits[0]);
				if (Number.isSafeInteger(value) && value >= 0) reserved.add(value);
			}
			// Every protected span consumes at least one source code unit. Reserve a
			// contiguous block large enough for every possible allocation, rather than
			// using max+1: MAX_SAFE_INTEGER+1 loses precision and can reuse one key.
			const required = source.length + 1;
			let start = 0;
			for (const value of [...reserved].sort((left, right) => left - right)) {
		...
		protectExistingPlaceholderLookalikes(plugin, string, protectedSegments = {}, count = 0) {
			return protectionLogic.protectRegexMatches(plugin, string, /(?:⟦\s*(?:DTA\s*)?\d+\s*⟧|【\s*\d+\s*】|\[\s*\d+\s*\]|<\s*<\s*<\s*\d+\s*>\s*>\s*>|[｛\{]\s*[｛\{]\s*\d+\s*[｝\}]\s*[｝\}])/g, protectedSegments, count);
		},
		prepareSemanticSource(plugin, string, place = MESSAGE_PLACES.RECEIVED) {
			string = String(string || "");
			let protectedSegments = {}, count = protectionLogic.getSemanticProtectionStartIndex(plugin, string), result;
			result = protectionLogic.protectExistingPlaceholderLookalikes(plugin, string, protectedSegments, count);
			string = result.string; protectedSegments = result.protectedSegments; count = result.count;
			// Shield planner-owned hard structure before applying user rules. This keeps a
			// configured term such as ``` from dismantling a fenced block before planning.
			result = protectionLogic.protectDiscordMarkupSegments(plugin, string, protectedSegments, count);
			string = result.string; protectedSegments = result.protectedSegments; count = result.count;
			result = protectionLogic.protectCodeBlockSegments(plugin, string, protectedSegments, count);
			string = result.string; protectedSegments = result.protectedSegments; count = result.count;
			result = protectionLogic.protectWrappedTextSegments(plugin, string, protectedSegments, count, place);
			string = result.string; protectedSegments = result.protectedSegments; count = result.count;
			result = protectionLogic.protectAutoDetectedSegments(plugin, string, protectedSegments, count);
			string = result.string; protectedSegments = result.protectedSegments; count = result.count;
			if (protectionLogic.shouldProtectConfiguredTermsForPlace(plugin, place)) {
				result = protectionLogic.protectConfiguredTerms(plugin, string, protectedSegments, count);
				string = result.string; protectedSegments = result.protectedSegments; count = result.count;
			}
			return Object.freeze({
				source: string,
				protectedSegments: Object.freeze(Object.assign({}, protectedSegments)),
				placeholderCount: Object.keys(protectedSegments).length
			});
		},
		addSemanticExceptions(plugin, string, protectedSegments) {
			const keys = Object.keys(protectedSegments || {});
			if (!keys.length) return string;
			// The semantic Provider never sees these placeholders: the planner replays
			// them locally. Restore only the exact marker we generated, so source text
			// such as [0], 【0】 or ⟦DTA0⟧ can never be mistaken for a local marker.
			for (let pass = 0; pass <= keys.length; pass++) {
				let changed = false;
				for (const count of keys) {
					const marker = protectionLogic.createProtectionPlaceholder(plugin, count);
					if (!String(string || "").includes(marker)) continue;
					string = String(string).split(marker).join(protectionLogic.formatProtectedExceptionForDisplay(plugin, String(protectedSegments[count])));
					changed = true;
				}
				if (!changed) break;
			}
		...
		removeExceptions(plugin, string, place) {
			...
			let autoTechnicalTermsResult = protectionLogic.protectAutoTechnicalTerms(plugin, string, protectedSegments, count, place);
