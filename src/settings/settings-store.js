...
//   rewrite a key. Here the only ways in are setCredential, setCredentialField,
//   setCredentialFlag, the interface-detection writers and the per-model reasoning
//   writers, and each persists exactly
//   what it wrote.
...
const {normalizeReasoningRawValue, createReasoningRawKey} = require("./reasoning-raw-value");
const {readBaiduCredentials, isBaiduCredentialComplete} = require("./baidu-credentials");

const LANGUAGE_DIRECTIONS = Object.freeze({INPUT: "input", OUTPUT: "output"});
const REASONING_MODEL_PREFERENCE_LIMIT = 50;
const REASONING_TIER_STATE_LIMIT = 16;
const REASONING_TIER_STATES = Object.freeze(["pending", "sent", "confirmed", "reduced", "ignored", "rejected"]);
const REASONING_TIER_EVIDENCE = Object.freeze(["none", "confirmed", "contradicted"]);
const REASONING_CUSTOM_KINDS = Object.freeze(["enum", "number"]);
// Only these four spellings existed before typed raws, so they are the only ones an
// older build can still read out of the legacy `effort` field.
const LEGACY_REASONING_EFFORTS = Object.freeze(["minimal", "low", "medium", "high"]);
...
function normalizeReasoningModelId(modelId) {
	return typeof modelId == "string" ? modelId.trim().slice(0, 512) : "";
}

function normalizeReasoningCapability(value) {
	if (!isRecord(value)) return null;
	const cleanString = (field, maxLength = 512) => typeof value[field] == "string" ? value[field].trim().slice(0, maxLength) : "";
	const normalized = {
		support: cleanString("support", 32),
		candidateId: cleanString("candidateId"),
		resolvedValue: cleanString("resolvedValue"),
		evidence: cleanString("evidence", 32),
		endpointKey: cleanString("endpointKey", 2048),
		checkedAt: Number.isFinite(Number(value.checkedAt)) ? Number(value.checkedAt) : 0
	};
	const format = cleanString("format", 64);
	if (format) normalized.format = format;
	return normalized;
}

function normalizeInterfaceDetection(value) {
	if (!isRecord(value)) return null;
	const resolved = typeof value.resolved == "string" ? value.resolved.trim().slice(0, 64) : "";
	const endpointKey = typeof value.endpointKey == "string" ? value.endpointKey.trim().slice(0, 2048) : "";
	const evidence = value.evidence == "catalog" || value.evidence == "validation" ? value.evidence : "";
	if (!resolved || !endpointKey || !evidence) return null;
	return {
		resolved,
		endpointKey,
		evidence,
		adapterVersion: Math.max(1, Number(value.adapterVersion) || 1),
		checkedAt: Number.isFinite(Number(value.checkedAt)) ? Number(value.checkedAt) : 0
	};
}

function normalizeReasoningTierState(value) {
	if (!isRecord(value)) return null;
	const state = REASONING_TIER_STATES.includes(value.state) ? value.state : "";
	if (!state) return null;
	const normalized = {
		state,
		evidence: REASONING_TIER_EVIDENCE.includes(value.evidence) ? value.evidence : "none",
		checkedAt: Number.isFinite(Number(value.checkedAt)) ? Number(value.checkedAt) : 0
	};
	// The verdict carries the exact value it belongs to, so nothing downstream has to
	// parse the tagged key back into a typed raw.
	const raw = normalizeReasoningRawValue(value.raw);
	if (raw !== null) normalized.raw = raw;
	return normalized;
}

function normalizeReasoningCustomDescriptor(value) {
	if (!isRecord(value) || !REASONING_CUSTOM_KINDS.includes(value.kind)) return null;
	const descriptor = {kind: value.kind};
	if (value.kind == "number") {
		for (const field of ["min", "max", "step"]) if (Number.isFinite(Number(value[field]))) descriptor[field] = Number(value[field]);
		const specials = Array.isArray(value.specialValues) ? value.specialValues.filter(entry => Number.isFinite(Number(entry))).map(Number) : [];
		if (specials.length) descriptor.specialValues = specials;
	}
	return descriptor;
}

// The tier table is addressed by tagged raw key; the record keeps the typed value.
function normalizeReasoningControlProfile(value) {
	if (!isRecord(value)) return null;
	const cleanString = (field, maxLength = 128) => typeof value[field] == "string" ? value[field].trim().slice(0, maxLength) : "";
	const tiers = [];
	const seenTierKeys = new Set();
	for (const entry of Array.isArray(value.tiers) ? value.tiers : []) {
		const raw = normalizeReasoningRawValue(isRecord(entry) ? entry.raw : entry);
		const rawKey = createReasoningRawKey(raw);
		if (!rawKey || seenTierKeys.has(rawKey)) continue;
		seenTierKeys.add(rawKey);
		const tier = {rawKey, raw};
		const availability = isRecord(entry) && entry.availability === "unsupported" ? "unsupported" : "supported";
		tier.availability = availability;
		tiers.push(tier);
	}
	const tierStates = {};
	if (isRecord(value.tierStates)) for (const rawKey of Object.keys(value.tierStates)) {
		const state = normalizeReasoningTierState(value.tierStates[rawKey]);
		if (state && /^[snb]:/.test(rawKey)) tierStates[rawKey.slice(0, 130)] = state;
	}
	const offRaw = normalizeReasoningRawValue(value.offRaw);
	const profile = {
		schemaId: cleanString("schemaId"),
		controlKind: cleanString("controlKind", 32),
		tiers,
		tierStates,
		custom: normalizeReasoningCustomDescriptor(value.custom),
		speed: ["idle", "running", "done"].includes(value.speed) ? value.speed : "idle",
		endpointKey: cleanString("endpointKey", 2048),
		format: cleanString("format", 64),
		adapterVersion: Math.max(1, Number(value.adapterVersion) || 1),
		checkedAt: Number.isFinite(Number(value.checkedAt)) ? Number(value.checkedAt) : 0
	};
	if (offRaw !== null) profile.offRaw = offRaw;
	return profile;
}

// A record written before typed raws carries one capability verdict and an enum
// effort. Read it as the first tier verdict so an upgrade never looks untested.
function deriveControlProfileFromLegacyCapability(capability, mode) {
	if (!isRecord(capability) || !capability.resolvedValue) return null;
	const rawKey = createReasoningRawKey(capability.resolvedValue);
	if (!rawKey) return null;
	// Legacy support+evidence collapses into the tier vocabulary: an accepted field
	// with no evidence only proves it was sent, never that thinking changed.
	const state = capability.support == "unsupported" ? "rejected"
		: capability.support == "reduced" ? "reduced"
		: capability.support == "accepted" ? (capability.evidence == "confirmed" ? "confirmed" : capability.evidence == "contradicted" ? "ignored" : "sent")
		: "pending";
	const profile = normalizeReasoningControlProfile({
		schemaId: "",
		tiers: [{raw: capability.resolvedValue}],
		tierStates: {[rawKey]: {state, evidence: capability.evidence, checkedAt: capability.checkedAt}},
		endpointKey: capability.endpointKey,
		format: capability.format,
		checkedAt: capability.checkedAt
	});
	if (profile && mode === "off") profile.offRaw = normalizeReasoningRawValue(capability.resolvedValue);
	return profile;
}

function normalizeReasoningModelPref(value, fallback = {}) {
	value = isRecord(value) ? value : {};
	fallback = isRecord(fallback) ? fallback : {};
	const pickString = (field, defaultValue) => {
		const candidate = value[field] !== undefined ? value[field] : fallback[field];
		return typeof candidate == "string" && candidate.trim() ? candidate.trim().slice(0, 128) : defaultValue;
	};
	const capabilitySource = Object.prototype.hasOwnProperty.call(value, "capability") ? value.capability : fallback.capability;
	const capability = normalizeReasoningCapability(capabilitySource);
	const mode = pickString("mode", "follow");
	const effort = pickString("effort", "low");
	const rawSource = value.onRaw !== undefined ? value.onRaw : fallback.onRaw;
	const onRaw = normalizeReasoningRawValue(rawSource);
	const profileSource = Object.prototype.hasOwnProperty.call(value, "controlProfile") ? value.controlProfile : fallback.controlProfile;
	const normalized = {
		mode,
		profile: pickString("profile", "auto"),
		effort,
		capability,
		checkedAt: Number.isFinite(Number(value.checkedAt)) ? Number(value.checkedAt) : Number.isFinite(Number(fallback.checkedAt)) ? Number(fallback.checkedAt) : 0
	};
	// Additive: the legacy four keys above keep their exact meaning and type.
	normalized.onRaw = onRaw !== null ? onRaw : normalizeReasoningRawValue(effort);
	// Marked only when a build that understands exact raws wrote this value on purpose.
	// Without it the record is a legacy one and has to be read the way the old build
	// dispatched it, which is what keeps an upgrade from refusing a working setup.
	const explicit = value.rawExplicit !== undefined ? value.rawExplicit : fallback.rawExplicit;
	if (explicit === true && normalized.onRaw !== null) normalized.rawExplicit = true;
	const controlProfile = normalizeReasoningControlProfile(profileSource) || deriveControlProfileFromLegacyCapability(capability, mode);
	if (controlProfile) normalized.controlProfile = controlProfile;
	return normalized;
}

function normalizeReasoningModels(value) {
	if (!isRecord(value)) return {};
	const result = {};
	for (const modelId of Object.keys(value)) {
		const normalizedModelId = normalizeReasoningModelId(modelId);
		if (!normalizedModelId || !isRecord(value[modelId])) continue;
		result[normalizedModelId] = normalizeReasoningModelPref(value[modelId]);
	}
	return result;
}
...
function createSettingsStore({
	...
	function saveReasoningModels(engineKey, reasoningModels) {
		if (!engineKey) return null;
		if (!authKeys[engineKey]) authKeys[engineKey] = {};
		if (isEmptyRecord(reasoningModels)) delete authKeys[engineKey].reasoningModels;
		else authKeys[engineKey].reasoningModels = reasoningModels;
		persistAuthKeys(authKeys);
		return authKeys[engineKey].reasoningModels || null;
	}

	function pruneReasoningModels(reasoningModels) {
		const modelIds = Object.keys(reasoningModels);
		if (modelIds.length <= REASONING_MODEL_PREFERENCE_LIMIT) return reasoningModels;
		modelIds.sort((left, right) => (Number(reasoningModels[left].checkedAt) || 0) - (Number(reasoningModels[right].checkedAt) || 0));
		for (let index = 0; index < modelIds.length - REASONING_MODEL_PREFERENCE_LIMIT; index++) delete reasoningModels[modelIds[index]];
	...
	return Object.freeze({
		...
		getCredentialField(engineKey, field) {
			...
			if (engineKey == "baidu" && (field == "appId" || field == "secretKey")) return readBaiduCredentials(credential || {})[field];
		...
		setCredentialField(engineKey, field, value) {
			...
			if (!authKeys[engineKey]) authKeys[engineKey] = {};
			const nextValue = (value || "").trim ? (value || "").trim() : value;
			if (engineKey == "baidu" && (field == "appId" || field == "secretKey")) {
				const parts = Object.assign(readBaiduCredentials(authKeys[engineKey]), {[field]: nextValue});
				Object.assign(authKeys[engineKey], parts);
				// Keep older builds usable, while retaining partial fields for the next edit.
				authKeys[engineKey].key = isBaiduCredentialComplete(parts) ? `${parts.appId} ${parts.secretKey}` : "";
			}
			if ((field == "endpoint" || field == "interfaceFormat") && authKeys[engineKey][field] !== nextValue) delete authKeys[engineKey].interfaceDetection;
			authKeys[engineKey][field] = nextValue;
			persistAuthKeys(authKeys);
			return authKeys[engineKey];
		},
		...
		getInterfaceDetection(engineKey) {
			return normalizeInterfaceDetection(engineKey && authKeys[engineKey] && authKeys[engineKey].interfaceDetection);
		},
		setInterfaceDetection(engineKey, detection) {
			if (!engineKey) return null;
			const normalized = normalizeInterfaceDetection(Object.assign({}, isRecord(detection) ? detection : {}, {checkedAt: now()}));
			if (!normalized) return null;
			if (!authKeys[engineKey]) authKeys[engineKey] = {};
			authKeys[engineKey].interfaceDetection = normalized;
			persistAuthKeys(authKeys);
			return normalized;
		},
		clearInterfaceDetection(engineKey) {
			const credential = engineKey && authKeys[engineKey];
			if (!credential || !Object.prototype.hasOwnProperty.call(credential, "interfaceDetection")) return false;
			delete credential.interfaceDetection;
			persistAuthKeys(authKeys);
			return true;
		},
		getReasoningModelPref(engineKey, modelId) {
			modelId = normalizeReasoningModelId(modelId);
			const credential = engineKey && authKeys[engineKey];
			if (!credential || !modelId) return null;
			const reasoningModels = normalizeReasoningModels(credential.reasoningModels);
			return reasoningModels[modelId] || null;
		},
		setReasoningModelPref(engineKey, modelId, preference) {
			modelId = normalizeReasoningModelId(modelId);
			if (!engineKey || !modelId) return null;
			if (!authKeys[engineKey]) authKeys[engineKey] = {};
			const reasoningModels = normalizeReasoningModels(authKeys[engineKey].reasoningModels);
			const current = reasoningModels[modelId] || {};
			const next = normalizeReasoningModelPref(Object.assign({}, isRecord(preference) ? preference : {}, {checkedAt: now()}), current);
			reasoningModels[modelId] = next;
			pruneReasoningModels(reasoningModels);
			saveReasoningModels(engineKey, reasoningModels);
			return reasoningModels[modelId] || null;
		},
		// The exact value the user picked. Mirrored into the legacy `effort` field only
		// when the old enum could express it, so a rollback keeps the intent.
		setReasoningModelOnRaw(engineKey, modelId, raw) {
			const onRaw = normalizeReasoningRawValue(raw);
			if (!engineKey || onRaw === null) return null;
			const current = this.getReasoningModelPref(engineKey, modelId) || {};
			const patch = Object.assign({}, current, {onRaw});
			patch.rawExplicit = true;
			if (typeof onRaw == "string" && LEGACY_REASONING_EFFORTS.includes(onRaw)) patch.effort = onRaw;
			return this.setReasoningModelPref(engineKey, modelId, patch);
		},
		setReasoningModelControlProfile(engineKey, modelId, controlProfile) {
			if (!engineKey) return null;
			const current = this.getReasoningModelPref(engineKey, modelId) || {};
			const next = normalizeReasoningControlProfile(controlProfile);
			if (!next) return null;
			// Verdicts already collected survive a profile refresh.
			if (current.controlProfile && isEmptyRecord(next.tierStates)) next.tierStates = current.controlProfile.tierStates;
			return this.setReasoningModelPref(engineKey, modelId, Object.assign({}, current, {controlProfile: next}));
		},
		setReasoningModelTierState(engineKey, modelId, raw, tierState) {
			const rawKey = createReasoningRawKey(raw);
			const state = normalizeReasoningTierState(Object.assign({}, isRecord(tierState) ? tierState : {}, {raw, checkedAt: now()}));
			if (!engineKey || !rawKey || !state) return null;
			const current = this.getReasoningModelPref(engineKey, modelId) || {};
			const base = current.controlProfile || {};
			const details = isRecord(tierState) ? tierState : {};
			const tierStates = Object.assign({}, base.tierStates);
			delete tierStates[rawKey];
			tierStates[rawKey] = state;
			const keys = Object.keys(tierStates);
			for (let index = 0; index < keys.length - REASONING_TIER_STATE_LIMIT; index++) delete tierStates[keys[index]];
			const profile = Object.assign({}, base, {tierStates});
			for (const field of ["schemaId", "endpointKey", "format"]) if (typeof details[field] == "string" && details[field]) profile[field] = details[field];
			if (Number.isFinite(Number(details.adapterVersion))) profile.adapterVersion = Number(details.adapterVersion);
			const patch = Object.assign({}, current, {controlProfile: profile});
			// Keep the single-value legacy capability in step so an older build still
			// reads a usable verdict for the value it knows about.
			const legacyRaw = normalizeReasoningRawValue(raw);
			if (typeof legacyRaw == "string") patch.capability = {
				support: state.state === "rejected" ? "unsupported" : state.state === "reduced" ? "reduced" : "accepted",
				candidateId: typeof details.candidateId == "string" ? details.candidateId : current.capability && current.capability.candidateId || "",
				resolvedValue: legacyRaw,
				evidence: state.evidence,
				endpointKey: profile.endpointKey || current.capability && current.capability.endpointKey || "",
				format: profile.format || current.capability && current.capability.format || "",
		...
		setReasoningModelCapability(engineKey, modelId, capability) {
			...
			if (!engineKey || !modelId) return null;
			const current = this.getReasoningModelPref(engineKey, modelId) || {};
			return this.setReasoningModelPref(engineKey, modelId, Object.assign({}, current, {capability: normalizeReasoningCapability(capability)}));
		},
		clearReasoningModelCapability(engineKey, modelId) {
			...
			return this.setReasoningModelPref(engineKey, modelId, Object.assign({}, current, {capability: null}));
		},
		clearReasoningModelPrefs(engineKey) {
			const credential = engineKey && authKeys[engineKey];
			if (!credential || !Object.prototype.hasOwnProperty.call(credential, "reasoningModels")) return 0;
			const count = Object.keys(normalizeReasoningModels(credential.reasoningModels)).length;
			delete credential.reasoningModels;
			persistAuthKeys(authKeys);
			return count;
		},
		deleteCredential(engineKey) {
			if (!engineKey || !Object.prototype.hasOwnProperty.call(authKeys, engineKey)) return false;
			delete authKeys[engineKey];
			persistAuthKeys(authKeys);
			return true;
		},
		...
		replaceAuthKeys(nextAuthKeys) {
			if (!isRecord(nextAuthKeys)) return authKeys;
			if (isEmptyRecord(nextAuthKeys)) {
				if (!isEmptyRecord(authKeys)) return authKeys;
				const storedAuthKeys = loadAuthKeys();
				if (isRecord(storedAuthKeys) && !isEmptyRecord(storedAuthKeys)) authKeys = storedAuthKeys;
				return authKeys;
			}
			authKeys = nextAuthKeys;
			persistAuthKeys(authKeys);
			return authKeys;
		},
