...
const {MESSAGE_DIRECTIONS, cloneDiscordMessage, createTranslationDisplayLogic} = require("../src/display/translation-display-logic");
...
function createLogic(BDFDB = createFakeBDFDB()) {
	return createTranslationDisplayLogic({BDFDB});
}

test("cloneDiscordMessage prefers the BDFDB record and restores fields its constructor drops", () => {
	class Message {
		constructor(data) {
			this.id = data.id;
	...
	const snapshots = [{message: {content: "forwarded"}}];
	const source = {
		id: "m1",
		channel_id: "c1",
		content: "original",
	...
	const clone = cloneDiscordMessage({DiscordObjects: {Message}}, source);

	assert.ok(clone instanceof Message, "the supported constructor remains the preferred path");
	assert.equal(clone.recordMethod(), "record");
	for (const key of ["id", "channel_id", "content", "embeds", "attachments", "author", "messageSnapshots", "messageReference"]) assert.equal(clone[key], source[key], `${key} survives a lossy constructor`);
	clone.content = "translated";
	assert.equal(source.content, "original", "painting the clone does not rewrite the source record");
});

test("cloneDiscordMessage preserves the source prototype when the constructor is missing or throws", () => {
	class SourceMessage {
	...
	const source = Object.assign(new SourceMessage(), {
		...
		message_reference: {message_id: "source-1"}
	});
	const throwingBDFDB = {DiscordObjects: {Message: class Message {constructor() {throw new Error("changed constructor");}}}};

	for (const BDFDB of [{DiscordObjects: {}}, throwingBDFDB]) {
		const clone = cloneDiscordMessage(BDFDB, source);
		assert.notEqual(clone, source);
		assert.ok(clone instanceof SourceMessage, "the fallback keeps renderer-visible record methods");
		assert.equal(clone.recordMethod(), "source-record");
		assert.equal(clone.message_snapshots, source.message_snapshots);
		assert.equal(clone.message_reference, source.message_reference);
		clone.content = "translated";
		assert.equal(source.content, "original");
	}
});

test("cloneDiscordMessage rejects a constructor that returns the source record itself", () => {
	function Message(source) {return source;}
	const source = {id: "m1", content: "original", embeds: []};

	const clone = cloneDiscordMessage({DiscordObjects: {Message}}, source);

	assert.notEqual(clone, source, "a constructor must not smuggle the live record through the clone boundary");
	clone.content = "translated";
	assert.equal(source.content, "original");
});

test("cloneDiscordMessage detaches frozen and non-writable constructor results", () => {
	class FrozenMessage {
		constructor(data) {
			Object.assign(this, data);
			Object.freeze(this);
		}
		recordMethod() {return "frozen-record";}
	}
	class ReadOnlyMessage {
		constructor(data) {
			this.id = data.id;
			Object.defineProperty(this, "content", {value: data.content, enumerable: true, writable: false, configurable: false});
		}
		recordMethod() {return "readonly-record";}
	}
	const source = {id: "m1", channel_id: "c1", content: "original", embeds: []};

	for (const Message of [FrozenMessage, ReadOnlyMessage]) {
		const clone = cloneDiscordMessage({DiscordObjects: {Message}}, source);
		assert.notEqual(clone, source);
		assert.ok(clone instanceof Message, "the detached record keeps constructor-provided methods");
		assert.equal(Object.isExtensible(clone), true);
		assert.equal(Object.getOwnPropertyDescriptor(clone, "content").writable, true);
		clone.content = "translated";
		clone.extraDisplayField = true;
		assert.equal(clone.content, "translated");
		assert.equal(clone.extraDisplayField, true);
		assert.equal(source.content, "original");
	}
});

test("all display clone paths stay usable when the BDFDB Message constructor throws", () => {
	const BDFDB = createFakeBDFDB();
	BDFDB.DiscordObjects.Message = class Message {constructor() {throw new Error("changed constructor");}};
	const logic = createLogic(BDFDB);

	const ordinarySource = {id: "normal", channel_id: "c1", content: "original", attachments: [{id: "a1"}], author: {id: "u1"}};
	const ordinaryClone = logic.cloneStreamMessageWithBody(createFakePlugin(), ordinarySource, "translated");
	assert.equal(ordinaryClone.content, "translated");
	assert.equal(ordinaryClone.attachments, ordinarySource.attachments);
	assert.equal(ordinaryClone.author, ordinarySource.author);
	assert.equal(ordinarySource.content, "original");

	const snapshotEntry = {message: {content: "forwarded original"}};
	const forwardSource = {id: "forward", content: "", messageSnapshots: [snapshotEntry], messageReference: {message_id: "source"}};
	const forwardClone = logic.cloneStreamMessageWithBody(createFakePlugin(), forwardSource, "forwarded translation");
	assert.equal(forwardClone.messageSnapshots[0].message.content, "forwarded translation");
	assert.equal(forwardClone.messageReference.message_id, "source");
	assert.equal(snapshotEntry.message.content, "forwarded original");

	const paintedEmbed = {id: "e1", rawTitle: "Translated title", fields: []};
	const embedStream = {content: {id: "embed-message", content: "original", embeds: [paintedEmbed]}};
	logic.applyReceivedDisplayViewToStream(createFakePlugin(), embedStream, {
		translated: false,
		content: "original",
		source: {content: "original", embeds: [{title: "Original title", description: "", footerText: "", fields: []}]}
	});
	assert.equal(embedStream.content.embeds[0].rawTitle, "Original title");
	assert.equal(paintedEmbed.rawTitle, "Translated title");

	const replyPlugin = createFakePlugin();
	const referencedMessage = {id: "reply", channel_id: "c1", content: "reply original", author: {id: "u2"}};
	const replyEvent = {instance: {props: {referencedMessage: {message: referencedMessage}, baseMessage: null}}};
	assert.doesNotThrow(() => logic.processMessageReply(replyPlugin, replyEvent));
	assert.notEqual(replyEvent.instance.props.referencedMessage.message, referencedMessage);
	assert.equal(replyEvent.instance.props.referencedMessage.message.content, "reply original");
	assert.equal(referencedMessage.content, "reply original");

	const manualPlugin = createFakePlugin();
	const manualSource = {id: "manual", channel_id: "c1", content: "manual original", embeds: [], attachments: [{id: "a2"}]};
	assert.doesNotThrow(() => logic.applyStoredTranslationToMessage(manualPlugin, manualSource, {translatedContent: "manual translation", originalContent: "manual original"}));
	const archive = manualPlugin.calls.find(call => call[0] == "commitManualTranslation")[1].archive;
	assert.notEqual(archive.message, manualSource);
	assert.equal(archive.message.content, "manual original");
	assert.equal(manualSource.content, "manual original");

	const restorePlugin = createFakePlugin();
	const archivedSource = {id: "restore", channel_id: "c1", content: "restored original", author: {id: "u3"}};
	restorePlugin.display.archives.set("restore", {message: archivedSource});
	const restoreEvent = {instance: {props: {message: {id: "restore", channel_id: "c1", content: "translated paint"}}}};
	assert.doesNotThrow(() => logic.prepareMessageContentDisplay(restorePlugin, restoreEvent));
	assert.equal(restoreEvent.instance.props.message.content, "restored original");
	assert.notEqual(restoreEvent.instance.props.message, archivedSource);
	assert.equal(archivedSource.content, "restored original");
});
...
for (const path of ["received-view", "message-content"]) {
	for (const scenario of [
		{name: "fully kept source", originalContent: "保持原文", translatedContent: "保持原文", keptSegmentCount: 1, highlighted: false},
		{name: "unchanged semantic result", originalContent: "保持原文", translatedContent: "保持原文", highlighted: false},
		{name: "source with surrounding whitespace", originalContent: "保持原文\r\n", translatedContent: " 保持原文\n", keptSegmentCount: 1, highlighted: false},
		{name: "source with normalized line endings", originalContent: "第一行\r\n第二行", translatedContent: "第一行\n第二行", keptSegmentCount: 1, highlighted: false},
		{name: "kept body with appended original", originalContent: "保持原文", translatedContent: "保持原文", content: "保持原文\n> 保持原文", keptSegmentCount: 1, highlighted: false},
		{name: "partially kept translation", originalContent: "Hello\nProjectName", translatedContent: "你好\nProjectName", keptSegmentCount: 1, highlighted: true},
		{name: "translated body", originalContent: "Hello", translatedContent: "你好", highlighted: true},
		{name: "legacy kept result without source", translatedContent: "ProjectName", keptSegmentCount: 1, highlighted: false},
		{name: "legacy translated result without source", translatedContent: "你好", highlighted: true}
	]) {
		test(`${path} styles ${scenario.name} according to whether the body changed`, () => {
			const logic = createLogic();
			const plugin = createFakePlugin();
			plugin.settings.general.highlightTranslatedMessages = true;
			const translation = Object.freeze({
				content: scenario.content || scenario.translatedContent,
				translatedContent: scenario.translatedContent,
				originalContent: scenario.originalContent,
				keptSegmentCount: scenario.keptSegmentCount,
				input: {id: "en"}, output: {id: "zh-CN"}
			});
			// A recycled message node must lose stale translated colours while keeping
			// Discord's own styles, its acknowledgement revision and the result watermark.
			const e = {returnvalue: {props: {
				className: "base translator-translated-message",
				style: {"--translator-accent-color": "#old", "--translator-text-color": "#old", fontSize: 16},
				children: [scenario.translatedContent]
			}}};
			if (path === "received-view") logic.applyReceivedDisplayViewToContent(plugin, e, {messageId: "m1", revision: 7, translated: true, translation});
			else logic.applyMessageContentRenderDecorations(plugin, e, {id: "m1"}, translation);
			assert.equal(e.returnvalue.props.className.includes("translator-translated-message"), scenario.highlighted);
			assert.equal(e.returnvalue.props.style["--translator-text-color"], scenario.highlighted ? "#abcdef" : undefined);
			assert.equal(e.returnvalue.props.style["--translator-accent-color"], scenario.highlighted ? "#abcdef" : undefined);
			assert.equal(e.returnvalue.props.style.fontSize, 16);
			assert.equal(e.returnvalue.props.children[0], scenario.translatedContent);
			assert.equal(e.returnvalue.props.children.length, scenario.highlighted ? 2 : 1);
			if (scenario.highlighted) assert.equal(e.returnvalue.props.children.at(-1).props.children.props.children.props.children, "(translated)");
			if (path === "received-view") assert.equal(e.returnvalue.props["data-translator-revision"], "7");
		});
	}
}
...
for (const bodyChanged of [false, true]) {
	for (const descriptionChanged of [false, true]) {
		test(`embed watermark follows its description (body changed: ${bodyChanged}, description changed: ${descriptionChanged})`, () => {
			const logic = createLogic();
			const plugin = createFakePlugin({getDisplayedTranslationChannelId: () => "c1"});
			const description = descriptionChanged ? "卡片描述" : "Card description";
			plugin.display.displayStates.set("m1", {status: "translated", translation: {
				channelId: "c1", content: bodyChanged ? "消息" : "Message",
				translatedContent: bodyChanged ? "消息" : "Message", originalContent: "Message",
				embeds: {e1: {description}}
			}});
			const originalEmbed = Object.freeze({id: "e1", message_id: "m1", rawDescription: "Card description"});
			const e = {instance: {props: {embed: originalEmbed}}};
			logic.processEmbed(plugin, e);
			const node = {props: {className: "embed-description", children: e.instance.props.embed.rawDescription}};
			e.returnvalue = {props: {children: [node]}};
			logic.processEmbed(plugin, e);
			assert.equal(node.props.children[0], description);
			assert.equal(node.props.children.length, descriptionChanged ? 2 : 1);
			if (descriptionChanged) assert.equal(node.props.children[1].props.children.props.children.props.children, "(translated)");
			assert.equal(originalEmbed.rawDescription, "Card description");
		});
	}
}
...
test("only the inner forwarded body acknowledges its display revision", () => {
	const logic = createLogic();
	const plugin = createFakePlugin();
	plugin.settings.general.showOriginalMessage = true;
	plugin.settings.general.showOriginalDirectly = true;
	...
	assert.equal(e.returnvalue.props["data-translator-revision"], undefined, "the empty parent cannot acknowledge a stale inner body");
	...
	const inner = {instance: {props: {message: {id: "fwd", content: "original", messageSnapshots: e.instance.props.message.messageSnapshots}}}, returnvalue: {props: {className: "message", children: ["original"]}}};
	logic.applyReceivedDisplayViewToContent(plugin, inner, {revision: 8, messageId: "fwd", status: "cancelled", translated: false, showLoading: false});
	assert.equal(inner.returnvalue.props["data-translator-revision"], "8", "the actual body acknowledges spinner removal");
});
...
test("createTranslationWatermarkNode stays silent for preserved source and marks only actual translations", () => {
	const logic = createLogic();
	const plugin = createFakePlugin();
	const kept = logic.createTranslationWatermarkNode(plugin, {content: "Hello", keptSegmentCount: 1, keptReasons: {"wrong-language": 1}, input: {id: "en"}, output: {id: "zh-CN"}}, "k");
	assert.equal(kept, null);
	const translated = logic.createTranslationWatermarkNode(plugin, {content: "你好", keptSegmentCount: 0, input: {id: "en"}, output: {id: "zh-CN"}}, "k");
	assert.equal(translated.props.children.props.children.props.children, "(translated)");
	const fallback = logic.createTranslationWatermarkNode(plugin, {content: "Hello", keptSegmentCount: 2, input: {id: "en"}, output: {id: "zh-CN"}}, "k");
	assert.equal(fallback, null, "preserved originals stay silent in every locale");
});
