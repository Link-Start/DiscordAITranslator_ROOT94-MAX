...
function createPlugin(overrides = {}) {
	...
	const plugin = {
		...
		// --- protection / analysis -------------------------------------------------
		settings: {general: {}, exceptions: {protectedTerms: [], wordStart: []}},
...
test("S8b M0b exposes the exact received entry/filter reason without changing the boolean gate", () => {
	const message = {id: "200", content: "hello there my friend"}, channel = {id: "channel-1"};
	for (const [expected, plugin] of [
		["eligible", createPlugin()],
		["translation_disabled", createPlugin({isTranslationEnabled: () => false})],
		["self_authored", createPlugin({isOwnMessage: () => true})],
		["suppressed", createPlugin({displayRuntime: {isSuppressed: () => true}})],
		["skip_cache", createPlugin({getCachedReceivedSkipDecision: () => "same_language"})]
	]) {
		const result = receivedMessageFilterRuntime.getReceivedAutoTranslateEligibility(plugin, message, channel);
		assert.equal(result.reason, expected); assert.equal(result.eligible, expected === "eligible");
		assert.equal(receivedMessageFilterRuntime.shouldAutoTranslateReceivedMessage(plugin, message, channel), result.eligible);
	}
});
