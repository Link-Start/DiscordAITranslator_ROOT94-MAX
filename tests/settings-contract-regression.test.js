...
test("plugin metadata uses a one-sentence bilingual description and linked repository author", () => {
	...
	// The BD plugin card shows this line: one sentence, Chinese and English.
	assert.match(source, /^ \* @description 自动翻译收发消息，支持多家翻译服务与 AI 判定 \/ Auto-translates Discord messages with multiple providers and AI\.$/m);
...
test("chinese language names never fall back to an English row label", () => {
	// Electron's ICU data can return English for a locale it cannot localize.
	// Known gaps are explicit, and an unknown language prefers its own-language
	// name before the code instead of mixing English into the Chinese list.
	const source = fs.readFileSync(path.resolve(__dirname, "..", "src", "legacy", "runtime.js"), "utf8");
	assert.match(source, /new Intl\.DisplayNames\(\["zh-Hans"\], \{type: "language", fallback: "none"\}\)/);
	assert.match(source, /ba: "巴什基尔语"/);
	assert.match(source, /getChineseLanguageName\(languageData\.id\) \|\| languageData\.ownlang \|\| languageData\.id/);
});
