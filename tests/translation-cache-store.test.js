...
test("context-aware short-word eligibility invalidates previous skip-cache decisions with policy v4", () => {
	assert.equal(RECEIVED_SKIP_CACHE_POLICY_VERSION, 4);
	const message = createMessage("exact-history", "mixed exact fixture"), signature = signatureFor("c1", message.content);
	const {store} = createHarness({loadCache: () => ({[message.id]: {signature: "", cachedAt: 1, skipped: {policyVersion: 3, reason: "too_similar", preview: ""}}})});
	store.loadPersisted();
	store.getEntry(message.id).signature = store.hashSignature(signature);
	assert.equal(store.getCachedSkipDecision(message, "c1"), null);
	assert.equal(store.getEntry(message.id), null);
});
...
test("loading an oversized cache applies the chosen capacity and persists the newest records", () => {
	const persisted = Object.fromEntries(Array.from({length: 150}, (_, i) => [`paid-${i}`, {cachedAt: i, translation: {translatedContent: `result-${i}`}}]));
	persisted.malformed = null;
	const {store, saves} = createHarness({getCapacity: () => "100", loadCache: () => persisted});
	store.loadPersisted();
	assert.equal(store.getEntryCount(), 100);
	assert.equal(store.hasEntry("paid-49"), false);
	assert.equal(store.getEntry("paid-50").translation.translatedContent, "result-50");
	store.flushPendingSave();
	assert.equal(saves.at(-1).keys.length, 100);
});

test("invalid persisted capacity values retain the 500 default", () => {
	for (const value of [undefined, null, true, {}, "", "abc", 0, 99, 10001, Infinity, 500.5]) {
		assert.equal(createHarness({getCapacity: () => value}).store.getCapacity(), 500, String(value));
	}
});

test("a configured capacity retains more than 500 paid results and shrinking only evicts the oldest", () => {
	let capacity = 1000;
	const {store, advance, saves} = createHarness({getCapacity: () => capacity});
	for (let index = 0; index < 750; index++) {
		store.persistTranslation(`paid-${index}`, `sig-${index}`, {translatedContent: `result-${index}`});
		advance(1);
	}
	assert.equal(store.getEntryCount(), 750);
	capacity = 100;
	assert.equal(store.applyCapacityLimit(), 650);
	assert.equal(store.getEntryCount(), 100);
	assert.equal(store.hasEntry("paid-649"), false);
	assert.equal(store.getEntry("paid-650").translation.translatedContent, "result-650");
	assert.equal(store.getEntry("paid-749").translation.translatedContent, "result-749");
	store.flushPendingSave();
	assert.equal(saves.at(-1).keys.length, 100);
	capacity = 2000;
	assert.equal(store.applyCapacityLimit(), 0);
	assert.equal(store.getEntryCount(), 100, "increasing capacity never clears paid entries");
});
...
test("manual clearAll removes translations and skip decisions and persists one empty cache", () => {
	const {store, saves, flushSave, pendingTimerCount} = createHarness();
	store.persistTranslation("m1", "sig-1", {translatedContent: "一"});
	store.persistSkipDecision("m2", "sig-2", "same_language", "preview");
	flushSave();
	assert.equal(store.getEntryCount(), 2);
	assert.equal(store.clearAll(), 2);
	assert.equal(store.getEntryCount(), 0);
	assert.equal(pendingTimerCount(), 1);
	flushSave();
	assert.deepEqual(saves.at(-1).keys, []);
	assert.equal(store.clearAll(), 0);
	assert.equal(pendingTimerCount(), 0, "clearing an already-empty cache writes nothing");
});
