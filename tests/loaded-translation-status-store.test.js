...
test("aggregate errors retain elapsed time while working and freeze it after failure", () => {
	const h = createHarness();
	h.store.recordSessionDisplayed("c1", ["ready"]);
	h.store.update({channelId: "c1", aggregate: true, active: true, phase: "requesting", pendingMessageIds: ["pending"], displayFailed: 1});
	h.advance(12000);
	assert.match(h.store.getStatusText(), /^\d+\/\d+ 12s$/, "an error must not hide the working clock");
	h.store.update({active: false, phase: "failed"});
	h.advance(8000);
	assert.match(h.store.getStatusText(), /^\d+\/\d+ 12s$/, "terminal failure retains elapsed work without ticking forever");
	h.store.update({active: true, phase: "displaying", displayFailed: 0, displayPending: 1});
	h.advance(4000);
	assert.match(h.store.getStatusText(), /^\d+\/\d+ 4s$/, "explicit retry starts a fresh round and excludes previous work and idle time");
	h.store.update({phase: "requesting", retryable: 1, displayPending: 0});
	h.advance(2000);
	assert.match(h.store.getStatusText(), /^\d+\/\d+ 6s$/, "phase changes and translation failures do not erase working time");
});
...
test("the primary capsule stays compact across requesting, repair, completion and failure", () => {
	...
	assert.equal(harness.store.getStatusText(legacyStatus({active: true, total: 50, processed: 50, displayed: 48, failed: 2, retryable: 2, phase: "repairing"})), "48/50 0s");
	assert.equal(harness.store.getStatusText(legacyStatus({active: true, total: 50, processed: 50, displayed: 0, failed: 2, retryable: 2, phase: "repairing"})), "48/50 0s", "atomic paint does not hide how many results are already ready");
	...
	assert.equal(harness.store.getStatusText(legacyStatus({done: true, total: 50, processed: 50, displayed: 48, failed: 2, retryable: 2, phase: "failed"})), "48/50 0s");
});
...
test("a completed provider batch keeps mounted-but-unpainted rows visible as display pending", () => {
	...
	assert.equal(harness.store.getStatusText(), "43/50 0s");
...
test("a completed empty scan keeps only the numeric zero ratio", () => {
	const harness = createHarness();
	assert.equal(harness.store.getStatusText(legacyStatus({done: true, total: 0})), "0/0");
	...
	assert.notEqual(harness.store.getStatusText(failed), "0/0", "a retained failure stays in the denominator");
});
...
test("failure and repair readings stay on the cumulative basis instead of snapping back to the batch", () => {
	...
	assert.equal(harness.store.getStatusText(), "39/40 0s", "the cumulative numerator holds and the retryable failure joins the denominator");
	...
	assert.equal(harness.store.getStatusText(), "39/40 0s", "repair keeps the cumulative reading too");
});
