...
test("shouldSendOriginalInsteadOfSentTranslation uses the fixed 0.94 sent-message floor", () => {
	const plugin = createFakePlugin();
...
test("detectLanguage google_free ignores a confident local result and lets form encode raw text", () => {
	const plugin = createFakePlugin({
	...
	assert.equal(plugin.requestCalls[0].options.form.q, "the quick brown fox and the small dog are here");
});
...
test("the received safety guards stay fixed on, and the length floors stay at zero", () => {
	const plugin = createFakePlugin();
	...
	assert.equal(off.heuristics.receivedSettingsPolicy.shouldTreatLanguageVariantsAsSame(off), true);
	assert.equal(off.heuristics.receivedSettingsPolicy.shouldSkipSameLanguageReceivedMessages(off), true);
...
test("obsolete stored flags cannot split language variants", () => {
	const plugin = createFakePlugin({filters: {treatLanguageVariantsAsSame: false}});
	assert.equal(plugin.isSameLanguageOrVariant("zh-CN", "zh-TW"), true);
...
test("isTranslationLikelyInTargetLanguage accepts a Chinese answer that carries over names and identifiers", () => {
	const plugin = createFakePlugin();
	// Field case 2026-09-13: the model translated correctly, the letter ratio (3 Chinese letters
	// against 36 Latin ones) rejected it and the source was kept as the "translation".
	for (const text of ["使用 Adobe Audition 或 Ultimate Vocal Remover (UVR)", "用 Adobe Audition 或 UVR", "建议对此使用 DaVinci Resolve 或 Adobe Premiere Pro。", "也可以使用 Blender。", "否"]) {
		assert.equal(languageHeuristicsRuntime.isTranslationLikelyInTargetLanguage(plugin, text, "zh-CN"), true, text);
	}
	// Without a single Chinese letter the answer is still not a translation.
	for (const text of ["Use Adobe Audition or Ultimate Vocal Remover (UVR)", "hello there my friend", "oui"]) {
		assert.equal(languageHeuristicsRuntime.isTranslationLikelyInTargetLanguage(plugin, text, "zh-CN"), false, text);
	}
	// Latin targets keep the language identification path unchanged.
	assert.equal(languageHeuristicsRuntime.isTranslationLikelyInTargetLanguage(plugin, "je ne sais pas ce que tu veux dire avec ce mot", "en"), false);
	assert.equal(languageHeuristicsRuntime.isTranslationLikelyInTargetLanguage(plugin, "ok hello", "fr"), true);
});
