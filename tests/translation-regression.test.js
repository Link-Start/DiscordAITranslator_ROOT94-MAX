...
const {LEGACY_DECISION_RULES} = require("../src/settings/translation-preferences");
const {CORE_SYSTEM_PROMPT} = require("../src/planner/translation-plan-serializer");
...
test("reasoning transport controls do not invalidate semantically identical translation caches", () => {
	const plugin = createPluginInstance();
	const message = {id: "reasoning-signature", content: "hello", embeds: []};
	const sourceData = {content: "hello", embeds: []};
	const before = plugin.createReceivedTranslationSignature(message, "channel-reasoning", sourceData);
	plugin.ensureSettingsStore().setCredentialField("oaicompat", "reasoningMode", "off");
	plugin.ensureSettingsStore().setCredentialField("oaicompat", "reasoningProfile", "deepseek");
	assert.equal(plugin.createReceivedTranslationSignature(message, "channel-reasoning", sourceData), before);
});

test("short foreign chat terms are covered by the fixed rules; the editable preferences only say how to render them", () => {
	const plugin = createPluginInstance();
	const preferences = plugin.getDefaultAiAutoTranslatePrompt();
	assert.match(preferences, /短词|short words/i, "the default preferences still tell the model how to render short words");
	assert.doesNotMatch(preferences, /__SKIP_TRANSLATION__|跳过/, "whether to skip is no longer decided by the user's prompt");
	assert.match(LEGACY_DECISION_RULES, /Short words, interjections, laughter.*translate them when they are not in the target language/i, "the legacy received lane keeps the no-skip rule as fixed code");
	assert.match(CORE_SYSTEM_PROMPT, /translate every segment into exactly that target language/, "the typed wire asks for every segment, however short");
});
...
test("legacy received preset cannot override fixed received safety guards", () => {
	const plugin = createPluginInstance();
	...
	// Mixed-language skipping is fixed off; same-language and echo guards are fixed on.
	assert.equal(plugin.shouldSkipMixedReceivedMessages(), false);
	assert.equal(plugin.shouldSkipSameLanguageReceivedMessages(), true);
});

test("obsolete received safety flags cannot disable fixed guards", () => {
	const plugin = createPluginInstance();
...
test("live translateMessage forwards the automatic flag to translateText", async () => {
	...
	assert.equal(translateOptions.latencyKind, "live");
	assert.equal(translateOptions.messageCount, 1);
});

test("AI primary and backup attempts share one anonymous latency token", async () => {
	const plugin = createPluginInstance();
	const contexts = [];
	plugin.settings.engines.translator = "deepseek";
	plugin.settings.engines.backup = "openai";
	plugin.getLanguageChoice = direction => direction == "input" ? "en" : "zh-CN";
	plugin.validTranslator = () => true;
	plugin.deepSeekTranslate = (data, callback) => {
		contexts.push(data.timingContext);
		callback("");
	};
	plugin.openAiTranslate = (data, callback) => {
		contexts.push(data.timingContext);
		callback("你好，朋友");
	};

	const result = await new Promise(resolve => plugin.translateText("hello friend", "received", translation => resolve(translation), null, {
		showToast: false,
		showFailureToast: false,
		trackBusy: false,
		channelId: "channel-latency-token"
	}));
	assert.equal(result, "你好，朋友");
	assert.equal(contexts.length, 2);
	assert.ok(contexts[0].token);
	assert.equal(contexts[0].token, contexts[1].token);
	assert.equal(contexts[0].role, "primary");
	assert.equal(contexts[1].role, "backup");
	assert.equal(contexts[0].token.kind, "manual");
	assert.equal(contexts[0].token.inputChars, 12);
	assert.ok(contexts[0].requestContext);
	assert.equal(contexts[0].requestContext.compatibilityBudget, contexts[1].requestContext.compatibilityBudget);
});

test("logical live cancellation settles the pipeline and fences backup dispatch", async () => {
	const plugin = createPluginInstance();
	const channel = {id: "channel-live-cancel-fence"};
	const message = {id: "live-cancel-fence-1", channel_id: channel.id, content: "hello", embeds: [], attachments: [], author: {id: "other-user"}};
	...
	assert.equal(typeof primaryCallback, "function");
	assert.equal(primaryContext.logicalRequestId, String(liveRequest.id));
	assert.equal(primaryContext.signal, liveRequest.signal);

	plugin.invalidateLiveTranslationRequests(channel.id);
	assert.equal(await pending, false, "logical abort settles without waiting for the callback transport");
	primaryCallback("");
	await new Promise(resolve => setImmediate(resolve));

	assert.equal(backupCalls, 0, "a cancelled primary never dispatches the backup provider");
	assert.equal(primaryContext.isCurrent(), false);
});
