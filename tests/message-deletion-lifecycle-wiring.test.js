...
function createFixture() {
	...
	const plugin = {
		...
		ensureSentTranslationStore: () => ({createManualRequestKey: (channelId, messageId) => `${channelId}:${messageId}`, cancelManualRequest: key => (calls.push(["removeManualMessage", key]), true)}),
...
test("message deletion wiring delegates every channel cleanup owner without changing arguments", async () => {
	...
	assert.equal(fixture.dependencies.removeManualMessage("m1", "c1"), true);
