...
test("channel disable schedules local cleanup for unconfirmed restored rows while translation is off", async () => {
	const harness = createHarness();
	try {
		const {plugin} = harness;
		delete plugin.isTranslationEnabled;
		plugin.setChannelEnablementStateValue("channel-a", true);
		plugin.restoreReceivedDisplayChannel = async () => ({confirmedIds: ["ready"], missingIds: ["stale"], retryIds: ["stale"]});
		const scheduled = [];
		plugin.scheduleReceivedDisplayFlush = (...args) => scheduled.push(args);
		await plugin.toggleTranslation("channel-a");
		assert.equal(plugin.isTranslationEnabled("channel-a"), false);
		assert.deepEqual(scheduled.map(args => args.slice(0, 2)), [["channel-a", "stale"]]);
	}
...
test("disabling clears a preview-only translation by refreshing its replying host row", async () => {
	...
	try {
		...
		assert.equal(calls.messageUpdates, updatesBeforeDisable + 1, "preview cleanup targets its replying host once");
		assert.equal(calls.rerenderAll, rebuildsBeforeDisable, "preview cleanup preserves the Composer");
	}
	finally {harness.restore();}
});
...
test("reply preview commit and restore refresh every host in one scoped transaction", async () => {
	...
	try {
		...
		const updatesBeforeCommit = calls.messageUpdates;
		...
		assert.equal(calls.messageUpdates, updatesBeforeCommit + 2, "one preview wave targets both host rows");
		assert.equal(calls.rerenderAll, beforeCommit, "the preview wave never remounts the chat");
		const beforeRestore = calls.rerenderAll;
		const updatesBeforeRestore = calls.messageUpdates;

		await plugin.restoreReceivedDisplayChannel("channel-a", {clearPreviews: true});

		assert.equal(calls.messageUpdates, updatesBeforeRestore + 2, "preview restore targets both hosts once");
		assert.equal(calls.rerenderAll, beforeRestore);
...
test("a deleted message is removed from display cache live history and reply preview ownership", async () => {
	...
	try {
		...
		// Keep an older live source pending while history owns the edited source.
		// Identical current sources are now rejected by the collector before overlap.
		const liveSource = {...message, content: "earlier source"};
		const liveRequest = liveQueue.createRequest(liveSource, channelId, {content: liveSource.content});
		liveQueue.markMessageQueued(message.id, liveRequest);
		assert.equal(plugin.collectHistoricalTranslationMessage({message, channel: {id: channelId}, originalContentData: {content: message.content}, historicalLoad: true, deferHistoricalSnapshotStart: true}), true);
		const historicalJob = plugin.getHistoricalTranslationJobQueue(channelId, false).jobs[0];
		assert.equal(liveQueue.isRequestCurrent(liveRequest), true, "both source versions still need deletion cleanup");
		const rebuildsBeforeDelete = calls.rerenderAll;
		const updatesBeforeDelete = calls.messageUpdates;
		...
		assert.equal(calls.messageUpdates, updatesBeforeDelete + 1, "deletion targets the surviving reply host");
		assert.equal(calls.rerenderAll, rebuildsBeforeDelete);
