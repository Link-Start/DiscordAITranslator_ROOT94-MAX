...
test("the historical snapshot seals while live commits keep restoring scroll", async () => {
	...
	try {
		...
		const jobs = [];
		const createJob = plugin.createCollectedHistoricalTranslationJob.bind(plugin);
		plugin.createCollectedHistoricalTranslationJob = id => {const job = createJob(id); jobs.push(job); return job;};
		...
		while (Date.now() < deadline) {
			finishedJob = jobs.find(job => job.state === "committed");
		...
		assert.ok(finishedJob, "the snapshot must seal and the job must finish despite continuous live scroll restores");
		assert.equal(finishedJob.sealed, true);
		assert.equal(finishedJob.items.size, 8);
		assert.equal([...finishedJob.items.values()].every(record => record.status === "translated"), true);
		assert.equal(messages.every(message => plugin.getReceivedDisplayRuntimeView(message.id).translated), true);
	}
...
test("S8b M0b exact fixture live burst keeps all eight protection occurrences in its matrix row", async () => {
	const {harness, plugin, calls} = createLiveBurstPlugin(25);
	try {
		plugin.settings.exceptions.wrapperPairs = ['"|"', '“|”', '`|`'];
		plugin.requestAiBatchTranslation = (_engineKey, preparedItems) => {calls.batch++; calls.batchedItems += preparedItems.length; return Promise.resolve(Object.fromEntries(preparedItems.map(item => [String(item.message.id), `译文 ${[...String(item.protectedText).matchAll(/⟦\d+⟧/g)].map(match => match[0]).join(" ")}`])));};
		const channel = {id: "channel-m0b-exact-burst"}, messages = Array.from({length: 4}, (_, index) => ({id: String(9100 + index), channel_id: channel.id, content: require("../fixtures/s8b-m0a-mixed-language-fixtures").original14Markdown, embeds: [], attachments: [], author: {id: "other-user"}}));
		for (const message of messages) {const data = {content: message.content, embeds: []}; plugin.captureReceivedMessageSource({messageId: message.id, channelId: channel.id, generation: plugin.getReceivedDisplayCommitGeneration(channel.id), sourceSignature: plugin.createReceivedTranslationSignature(message, channel.id, data), source: data}); plugin.queueAutoTranslateMessage(message, channel, data);}
		assert.equal(await drainBurst(plugin, messages), true);
		const routes = plugin.getTranslationTerminalLedgerSnapshot().recent.filter(route => route.lane === "live-burst");
		assert.ok(routes.length >= 1); assert.equal(routes.every(route => route.placeholderOccurrences === 8 && route.ruleCounts["configured-or-wrapper"] === 1 && route.ruleCounts["auto-hyphen-token"] === 5), true, JSON.stringify(routes));
		assert.equal(routes.every(route => route.promptFamily === "batch-json" && route.validatorFamily === "history-batch" && route.providerDispatchCount === 1), true, JSON.stringify(routes));
	}
	finally {harness.restore();}
});
