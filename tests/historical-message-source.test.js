...
test("H1 source observer sees load, scan and prefetch stages while throwing hooks preserve output", async () => {
	const events = [];
	const options = {
		listCachedMessages: async () => [createMessage(200, "cached")],
		prefetchMessages: async () => [createMessage(100, "prefetched")],
		isEligible: () => true,
		toQueueItem: message => ({id: message.id, content: message.content}),
		isGenerationCurrent: () => true
	};
	const observed = createHistoricalMessageSource(Object.assign({}, options, {observer: {onStage: event => events.push(event)}}));
	const throwing = createHistoricalMessageSource(Object.assign({}, options, {observer: {onStage: () => {throw new Error("observer failure");}}}));
	const request = {channelId: "channel-1", generation: 1, renderedMessages: [], limit: 2};
	const observedResult = await observed.build(request);
	assert.deepEqual(await throwing.build(request), observedResult);
	assert.deepEqual(events.map(event => `${event.stage}:${event.phase}`), [
		"source_load:start", "source_load:settle", "scan_prefetch:scan", "scan_prefetch:prefetch_start", "scan_prefetch:prefetch_settle", "scan_prefetch:settle"
	]);
	assert.equal(events.at(-1).count, 2);
	assert.equal(events.at(-1).prefetchedCount, 1);
	assert.equal(JSON.stringify(events).includes("message-200"), false);
	assert.equal(JSON.stringify(events).includes("message-100"), false);
});
