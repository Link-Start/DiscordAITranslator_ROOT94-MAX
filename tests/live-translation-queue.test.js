...
test("the UI live concurrency 2 overlaps two requests and downscales by draining", async () => {
	const harness = createHarness();
	assert.equal(harness.queue.setLiveSlotCapacity("2"), 2);
	harness.addLiveItem("m1", "c1");
	harness.addLiveItem("m2", "c2");
	harness.addLiveItem("m3", "c3");
	assert.deepEqual(harness.log.single, ["m1", "m2"]);
	assert.equal(harness.queue.getLiveSlotActiveCount(), 2);
	assert.deepEqual(harness.queueIds(), ["m3"]);
	assert.equal(harness.queue.setLiveSlotCapacity("1"), 1);
	await harness.resolveSingle(0);
	assert.deepEqual(harness.log.single, ["m1", "m2"], "one remaining request still fills cap1");
	await harness.resolveSingle(1);
	assert.deepEqual(harness.log.single, ["m1", "m2", "m3"]);
	await harness.resolveSingle(2);
});
...
test("a live item carries queue wait from enqueue to provider dispatch", async () => {
	let currentTime = 100;
	const waits = [];
	const harness = createHarness({
		now: () => currentTime,
		translateSingleItem: queueItem => {
			waits.push(queueItem.queueWaitMs);
