...
function createPlugin(overrides = {}) {
	const settings = {
		general: Object.assign({}, overrides.general || {}),
...
test("the factory returns a frozen object so no caller can swap a pass out", () => {
	...
	assert.equal(protection.protectQuotedTextSegments, undefined, "quote handling belongs to wrapper-pair rules");
});
...
test("emails, urls, bare domains, addresses and slash commands are auto-detected", () => {
	...
	assert.deepEqual(Object.values(protection.protectAutoDetectedSegments(plugin, "go to example.com:8/a?x=(y)#part now").protectedSegments), ["example.com:8/a?x=(y)#part"]);
	...
	assert.deepEqual(Object.values(protection.protectAutoDetectedSegments(plugin, "ping 192.168.0.1:8 ok").protectedSegments), ["192.168.0.1:8"]);
	assert.deepEqual(Object.values(protection.protectAutoDetectedSegments(plugin, "run /help now").protectedSegments), ["/help"]);
	assert.deepEqual(Object.values(protection.protectAutoDetectedSegments(plugin, "run /x now").protectedSegments), ["/x"]);
...
test("balanced url parentheses stay protected while unmatched sentence closers stay outside", () => {
	const protection = createProtection();
	const plugin = createPlugin();
	const source = "See https://en.wikipedia.org/wiki/Function_(mathematics), https://example.com/a?x=(y), and https://example.com/end).";
	const result = protection.protectAutoDetectedSegments(plugin, source);
	assert.deepEqual(Object.values(result.protectedSegments), [
		"https://en.wikipedia.org/wiki/Function_(mathematics)",
		"https://example.com/a?x=(y)",
		"https://example.com/end"
	]);
	assert.match(result.string, /⟦0⟧, ⟦1⟧, and ⟦2⟧\)\./);
});
...
test("all-caps words are no longer guessed as acronyms; a configured protected term still protects one", () => {
	const protection = createProtection();
	const plugin = createPlugin();
	assert.equal(protection.protectAutoTechnicalTerms(plugin, "HELLO CRYZYYY").count, 0);
	assert.equal(protection.protectAutoTechnicalTerms(plugin, "我需要CDK用于GPT").count, 0, "the all-caps guess is retired");
	assert.equal(protection.protectAutoTechnicalTerms(plugin, "NO").count, 0);
	assert.equal(protection.protectAutoTechnicalTerms(plugin, "do NOT restart it").count, 0);
	// Deterministic shapes keep their protection.
	assert.deepEqual(Object.values(protection.protectAutoTechnicalTerms(plugin, "open src/app.js at v1.2.3 with snake_case").protectedSegments), ["src/app.js", "v1.2.3", "snake_case"]);
	const pinned = protection.removeExceptions(createPlugin({exceptions: {protectedTerms: ["GPT"]}}), "我需要CDK用于GPT", "received");
	assert.deepEqual(Object.values(pinned[1]), ["GPT"], "only the term the user pinned is protected");
});
