...
test("detectLanguage: empty text short-circuits without calling Google", () => {
	const plugin = createPluginInstance();
	let requestCalls = 0;
	plugin._testBdfdb.LibraryRequires.request = () => { requestCalls++; throw new Error("clear CJK script detection must not use the network"); };
...
test("detectLanguage: successful Google response returns src and sends raw form text exactly once", () => {
	const plugin = createPluginInstance();
	...
	plugin._testBdfdb.LibraryRequires.request = (url, opts, cb) => {
		...
		cb(null, {statusCode: 200}, JSON.stringify({src: "zh-CN"}));
	};
	return new Promise(resolve => {
		const sample = "目前编辑非本轮历史消息时由于底层转换机";
		plugin.detectLanguage(`  ${sample}  `, languageId => {
			assert.equal(seenUrl, "https://translate.googleapis.com/translate_a/single");
			assert.equal(seenQuery, sample, "the form encoder owns percent-encoding; pre-encoding corrupts CJK text");
			assert.equal(languageId, "zh-CN");
			resolve();
		});
	});
});

test("language detection locally identifies clear Chinese scripts when Google is unavailable", async () => {
	const plugin = createPluginInstance();
	plugin.settings.filters.languageDetectionStrategy = "local_first";
	let requestCalls = 0;
	plugin._testBdfdb.LibraryRequires.request = () => { requestCalls++; throw new Error("clear CJK script detection must not use the network"); };

	const simplified = await new Promise(resolve => plugin.detectLanguage("目前编辑非本轮历史消息时由于底层转换机", resolve));
	const traditional = await new Promise(resolve => plugin.detectLanguage("目前編輯非本輪歷史訊息時由於底層轉換機", resolve));
	assert.equal(simplified, "zh-CN");
	assert.equal(traditional, "zh-TW");
	assert.equal(requestCalls, 0);
});

test("the channel helper resolves the reported Simplified Chinese sample to language details", async () => {
	const plugin = createPluginInstance();
	plugin.settings.filters.languageDetectionStrategy = "local_first";
	plugin._testBdfdb.LibraryRequires.request = () => { throw new Error("the clear sample must stay local"); };
	const details = await plugin.detectLanguageDetails("目前编辑非本轮历史消息时由于底层转换机");
	assert.equal(details && details.id, "zh-CN");
});
test("language detection accepts an already parsed Google response body", async () => {
	const plugin = createPluginInstance();
	plugin.settings.filters.languageDetectionStrategy = "google_free";
	plugin._testBdfdb.LibraryRequires.request = (_url, _opts, callback) => callback(null, {status: 200}, {src: "ja"});
	const detected = await new Promise(resolve => plugin.detectLanguage("これは日本語の文章です", resolve));
	assert.equal(detected, "ja");
});
