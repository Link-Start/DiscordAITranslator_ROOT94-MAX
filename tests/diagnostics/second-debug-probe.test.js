...
test("message row DOM shapes are sampled from the live scroller for selector evidence", () => {
	const probe = createSecondDebugProbe({log: () => {}});
	const row = {
		id: "chat-messages___1000000000000000003___message-123",
		getAttribute(name) {
			if (name == "id") return this.id;
			if (name == "data-list-item-id") return "chat-messages___1000000000000000003";
			return null;
		}
	};
	...
	assert.equal(entry.shapes[0].id, "chat-messages___1000000000000000003___message-123");
	assert.equal(entry.shapes[0]["data-list-item-id"], "chat-messages___1000000000000000003");
