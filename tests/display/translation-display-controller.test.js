...
test("deferred historical waves expose anonymous resource liveness until their flush", async () => {
	const timers = [];
	let gateOpen = false;
	const harness = createHarness(null, {
		setTimeout: callback => {timers.push(callback); return timers.length;},
		canRepaintNow: () => gateOpen
	});
	harness.store.setChannelGeneration("c1", 1);
	capture(harness.store, "m1");
	const outcome = await harness.controller.commitHistoricalBatch([result("m1")]);
	assert.deepEqual(outcome.deferredIds, ["m1"]);
	assert.deepEqual(harness.controller.getDiagnostics().resources, {
...
test("an asynchronous deferred repaint stays active in diagnostics until render settles", async () => {
	const timers = [];
	const repaint = createDeferred();
	let gateOpen = false;
	const harness = createHarness(() => repaint.promise, {
		setTimeout: callback => {timers.push(callback); return timers.length;},
		canRepaintNow: () => gateOpen
	});
	harness.store.setChannelGeneration("c1", 1);
	capture(harness.store, "m1");
	await harness.controller.commitHistoricalBatch([result("m1")]);
	gateOpen = true;
	const flushing = timers.shift()();
	await Promise.resolve();
	let resources = harness.controller.getDiagnostics().resources;
	assert.equal(resources.pendingChannelCount, 0);
	assert.equal(resources.deferredFlushTimerArmed, false);
	assert.equal(resources.activeDeferredFlushCount, 1);
	assert.equal(resources.activeDeferredMessageCount, 1);
	repaint.resolve({confirmedIds: ["m1"], missingIds: [], fallbackUsed: false});
	await flushing;
	resources = harness.controller.getDiagnostics().resources;
	assert.equal(resources.activeDeferredFlushCount, 0);
	assert.equal(resources.activeDeferredMessageCount, 0);
});

test("a failed deferred repaint is counted after its resources are released", async () => {
	const timers = [];
	let gateOpen = false;
	const harness = createHarness(async () => {throw new Error("render failed");}, {
		setTimeout: callback => {timers.push(callback); return timers.length;},
		canRepaintNow: () => gateOpen
	});
	harness.store.setChannelGeneration("c1", 1);
	...
	assert.equal(diagnostics.deferredFlushErrorCount, 1);
	assert.equal(diagnostics.resources.activeDeferredFlushCount, 0);
});
...
test("a rejected old historical render cannot retry the edited source revision", async () => {
	let rejectRender;
	const reports = [];
	const rendering = new Promise((_resolve, reject) => {rejectRender = reject;});
	const {store, controller} = createHarness(() => rendering, {onRenderOutcome: report => reports.push(report)});
	capture(store, "m1");
	const commit = controller.commitHistoricalBatch([result("m1")]);
	const previousRevision = store.getDisplayState("m1").revision;
	store.captureSource({messageId: "m1", channelId: "c1", generation: 1, sourceSignature: "edited-source", source: {content: "Edited source.", embeds: []}});
	rejectRender(new Error("old render failed"));
	const outcome = await commit;
	assert.deepEqual(outcome.staleIds, ["m1"]);
	assert.deepEqual(outcome.missingIds, []);
	assert.deepEqual(outcome.retryIds, []);
	assert.equal(reports[0].revisionsByMessageId.m1, previousRevision);
	assert.ok(store.getDisplayState("m1").revision > previousRevision);
...
test("one historical three-message batch creates exactly one coherent refresh", async () => {
	...
	const outcome = await controller.commitHistoricalBatch([result("m1"), result("m2"), result("m3")]);

	assert.deepEqual(outcome.committedIds, ["m1", "m2", "m3"]);
	assert.equal(refreshes.length, 1);
	assert.equal(refreshes[0].transactionId, 1);
	assert.equal(refreshes[0].channelId, "c1");
...
test("empty controller operations return the stable no-op outcome", async () => {
	...
	assert.deepEqual(await controller.commitHistoricalBatch([]), {...emptyOutcome(), committedIds: []});
...
test("commitHistoricalBatch commits recorded results and surfaces unrecorded rejections", async () => {
	...
	assert.deepEqual(outcome.committedIds, ["m1"]);
