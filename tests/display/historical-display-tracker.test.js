...
test("manual display retry rearms only failed current revisions and ignores duplicate clicks", () => {
	const revisions = {failed: 1, edited: 1, waiting: 1};
	const tracker = createHistoricalDisplayTracker({getRevision: (_channel, id) => revisions[id]});
	tracker.begin({channelId: "c1", batchKey: "job", outcome: {retryIds: ["failed", "edited", "waiting"]}});
	tracker.handle({channelId: "c1", messageIds: ["failed", "edited"], revisionsByMessageId: {failed: 1, edited: 1}, outcome: {exhaustedIds: ["failed", "edited"]}});
	revisions.edited = 2;
	const scheduled = [];
	const retry = () => tracker.retryFailed({channelId: "c1", schedule: (id, key) => scheduled.push([id, key])});
	assert.equal(retry(), 1);
	assert.equal(retry(), 0);
	assert.deepEqual(scheduled, [["failed", "job"]]);
	assert.deepEqual(tracker.getSnapshot("c1").failedIds, []);
	assert.deepEqual(tracker.getSnapshot("c1").pendingIds.sort(), ["failed", "waiting"]);
	tracker.handle({channelId: "c1", messageIds: ["failed"], revisionsByMessageId: {failed: 1}, outcome: {confirmedIds: ["failed"]}});
	assert.deepEqual(tracker.getSnapshot("c1").pendingIds, ["waiting"]);
});

test("an unavailable display retry leaves its failure visible", () => {
	const tracker = createHistoricalDisplayTracker();
	tracker.begin({channelId: "c1", batchKey: "job", outcome: {retryIds: ["m1"]}});
	tracker.handle({channelId: "c1", messageIds: ["m1"], trackingKeysByMessageId: {m1: ["job"]}, outcome: {exhaustedIds: ["m1"]}});
	assert.equal(tracker.retryFailed({channelId: "c1", schedule: () => {throw new Error("stopped");}}), 0);
	assert.deepEqual(tracker.getSnapshot("c1").failedIds, ["m1"]);
	tracker.clear("c1");
	assert.equal(tracker.retryFailed({channelId: "c1"}), 0);
});
...
test("a tracked historical repaint advances displayed count only for its current channel", () => {
	...
	assert.deepEqual(statusUpdates, [{channelId: "c1", jobId: "batch-1", displayFailed: 0, displayed: 2, displayPending: 0}]);
...
test("virtualized-ready rows resolve pending display tracking and clear removes all batches", () => {
	...
	assert.deepEqual(updates.at(-1), {channelId: "c1", jobId: "batch-1", displayFailed: 0, displayed: 4, displayPending: 0});
...
test("a skipped row can finish repainting without inflating the translated display count", () => {
	...
	assert.deepEqual(updates.at(-1), {channelId: "c1", jobId: "batch-1", displayFailed: 0, displayed: 1, displayPending: 1});
	...
	assert.deepEqual(updates.at(-1), {channelId: "c1", jobId: "batch-1", displayFailed: 0, displayed: 2, displayPending: 0});
});
...
test("an exhausted or unavailable repaint finishes pending tracking without claiming it displayed", () => {
	...
	assert.deepEqual(updates.at(-1), {channelId: "c1", jobId: "batch-1", displayFailed: 1, displayed: 4, displayPending: 0});
...
test("a late old-batch outcome cannot resolve the replacement batch for the same channel", () => {
	...
	assert.deepEqual(updates.at(-1), {channelId: "c1", jobId: "new", displayFailed: 0, displayed: 1, displayPending: 0});
});
...
test("a newer message revision retires the old batch row without counting the newer translation", () => {
	...
	assert.deepEqual(updates.at(-1), {channelId: "c1", jobId: "batch-1", displayFailed: 0, displayed: 0, displayPending: 0});
});
