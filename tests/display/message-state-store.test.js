...
const {
	...
	RENDER_STATUSES,
	MESSAGE_ORIGINS,
	MAX_INERT_TERMINAL_RECORDS_PER_CHANNEL,
...
test("inert terminal receipts have a per-channel oldest-first bound", () => {
	assert.equal(MAX_INERT_TERMINAL_RECORDS_PER_CHANNEL, 500);
	const store = createMessageStateStore({maxInertTerminalRecordsPerChannel: 3});
	for (let index = 1; index <= 8; index++) {
		const messageId = `failed-${index}`;
		store.captureSource(snapshot(messageId, "c1", `source-${index}`));
		store.commitResult({...translated(messageId, "c1", `source-${index}`, "unused"), status: MESSAGE_STATUSES.FAILED, reason: "provider-failed"});
		store.markRenderOutcome({missingIds: [messageId]});
	}

	for (let index = 1; index <= 5; index++) assert.equal(store.getDisplayState(`failed-${index}`), null);
	for (let index = 6; index <= 8; index++) {
		const retained = store.getDisplayState(`failed-${index}`);
		assert.equal(retained.status, MESSAGE_STATUSES.FAILED);
		assert.equal(retained.renderStatus, RENDER_STATUSES.UNCONFIRMED);
	}
});

test("commit-only terminal traffic remains bounded without a render acknowledgement", () => {
	const store = createMessageStateStore({maxInertTerminalRecordsPerChannel: 3});
	for (let index = 1; index <= 9; index++) {
		const messageId = `commit-only-${index}`;
		store.captureSource(snapshot(messageId, "c1", `source-${index}`));
		const status = index % 2 ? MESSAGE_STATUSES.FAILED : MESSAGE_STATUSES.SKIPPED;
		const committed = store.commitResult({...translated(messageId, "c1", `source-${index}`, "unused"), status, reason: status});
		assert.equal(committed.messageId, messageId, "the just-committed receipt remains available to its caller");
		if (index % 3 === 0) assert.equal(store.restoreMessage(messageId)[0].status, MESSAGE_STATUSES.CANCELLED);
	}

	assert.deepEqual(store.listChannel("c1").map(record => record.messageId), ["commit-only-7", "commit-only-8", "commit-only-9"]);
	for (const record of store.listChannel("c1")) assert.equal(record.renderStatus, RENDER_STATUSES.PENDING);
});

test("commit-only terminal batches prune older receipts while preserving the whole active batch", () => {
	const store = createMessageStateStore({maxInertTerminalRecordsPerChannel: 3});
	for (let index = 1; index <= 8; index++) store.captureSource(snapshot(`batch-only-${index}`, "c1", `source-${index}`));
	for (let offset = 1; offset <= 7; offset += 2) {
		const results = [offset, offset + 1].map(index => ({
			...translated(`batch-only-${index}`, "c1", `source-${index}`, "unused"),
			status: MESSAGE_STATUSES.FAILED,
			reason: "provider-failed"
		}));
		const outcome = store.commitBatch(results);
		assert.deepEqual(outcome.committed.map(record => record.messageId), results.map(result => result.messageId));
		for (const result of results) assert.ok(store.getDisplayState(result.messageId), "the active batch remains renderable");
	}

	assert.deepEqual(store.listChannel("c1").filter(record => record.status === MESSAGE_STATUSES.FAILED).map(record => record.messageId), ["batch-only-6", "batch-only-7", "batch-only-8"]);
});

test("terminal receipt eviction is channel isolated", () => {
	const store = createMessageStateStore({maxInertTerminalRecordsPerChannel: 2});
	for (const channelId of ["c1", "c2"]) for (let index = 1; index <= 3; index++) {
		const messageId = `${channelId}-skipped-${index}`;
		store.captureSource(snapshot(messageId, channelId, `source-${index}`));
		store.commitResult({...translated(messageId, channelId, `source-${index}`, "unused"), status: MESSAGE_STATUSES.SKIPPED, reason: "same-language"});
		store.markRenderOutcome({confirmedIds: [messageId]});
	}

	assert.equal(store.getDisplayState("c1-skipped-1"), null);
	assert.equal(store.getDisplayState("c2-skipped-1"), null);
	assert.deepEqual(store.listChannel("c1").map(record => record.messageId), ["c1-skipped-2", "c1-skipped-3"]);
	assert.deepEqual(store.listChannel("c2").map(record => record.messageId), ["c2-skipped-2", "c2-skipped-3"]);
});

test("the terminal cap preserves every record with active display, request, restore, or preview ownership", () => {
	const store = createMessageStateStore({maxInertTerminalRecordsPerChannel: 1});

	store.captureSource(snapshot("translated", "c1", "translated-source"));
	store.commitResult(translated("translated", "c1", "translated-source", "translated-result"));
	store.captureSource(snapshot("pending", "c1", "pending-source"));
	store.markPending({messageId: "pending", channelId: "c1", generation: 1, requestIdentity: "pending-request"});
	store.suppress("suppressed", {channelId: "c1"});
	store.markPreviewPending({messageId: "preview-pending", channelId: "c1", signature: "preview-signature"});
	store.captureSource(snapshot("restoring", "c1", "restore-source"));
	store.commitResult(translated("restoring", "c1", "restore-source", "restore-result"));
	store.restoreMessage("restoring");

	store.captureSource(snapshot("preview-owner", "c1", "owner-source"));
	store.commitResult({...translated("preview-owner", "c1", "owner-source", "unused"), status: MESSAGE_STATUSES.FAILED, reason: "provider-failed"});
	store.markPreviewEligible("c1", "preview-owner");
	store.markRenderOutcome({missingIds: ["preview-owner"]});

	for (let index = 1; index <= 4; index++) {
		const messageId = `inert-${index}`;
		store.captureSource(snapshot(messageId, "c1", `inert-source-${index}`));
		store.commitResult({...translated(messageId, "c1", `inert-source-${index}`, "unused"), status: MESSAGE_STATUSES.FAILED, reason: "provider-failed"});
		store.markRenderOutcome({missingIds: [messageId]});
	}

	for (const messageId of ["translated", "pending", "suppressed", "preview-pending", "restoring", "preview-owner", "inert-4"]) {
		assert.ok(store.getDisplayState(messageId), `${messageId} remains owned`);
	}
	for (let index = 1; index <= 3; index++) assert.equal(store.getDisplayState(`inert-${index}`), null);
});
