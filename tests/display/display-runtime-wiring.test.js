...
function createHarness() {
	...
	const plugin = {
		handleHistoricalDisplayOutcome: report => report,
...
test("received display wiring creates the runtime with the complete host and plugin contract", () => {
	...
	const report = {channelId: "channel-a", revisionsByMessageId: {m1: 3}, outcome: {confirmedIds: ["m1"]}};
	assert.equal(captured.onRenderOutcome(report), report);
