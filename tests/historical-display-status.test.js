...
test("a historical commit keeps unconfirmed mounted rows pending and schedules targeted display retries", async () => {
	const plugin = createPluginInstance({callSetLanguages: false, bdfdb: {LibraryStores: {SelectedChannelStore: {getChannelId: () => "c1"}}}});
	const scheduled = [];
	...
	plugin.getReceivedDisplayRuntimeView = messageId => ({messageId, generation: 1, sourceSignature: `sig-${messageId}`, status: "translated", translation: {content: `translation ${messageId}`}, requestIdentity: null});
	...
	plugin.ensureLoadedStatusCapsuleController().recordTranslationsDisplayed("c1", ["m1", "m2"]);
	await plugin.commitHistoricalTranslationJob(summary, job);

	assert.deepEqual(scheduled, [["c1", "m2", null, "c1:job-1"]]);
	assert.match(plugin.getLoadedAutoTranslationStatusText(), /^1\/2 \d+s$/);
