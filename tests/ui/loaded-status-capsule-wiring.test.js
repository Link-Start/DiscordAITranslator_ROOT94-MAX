...
test("loaded-status capsule wiring creates the controller with the complete dependency contract", () => {
	...
	const plugin = {
		retryFailedHistoricalDisplays: channelId => ({channelId, displayOnly: true}),
		getLoadedHistoricalWorkStatus: channelId => ({channelId, phase: "queued"}),
	...
	assert.deepEqual(captured.getWorkStatus("channel-a"), {channelId: "channel-a", phase: "queued"});
	...
	assert.deepEqual(captured.hooks.onRetryDisplay("channel-a"), {channelId: "channel-a", displayOnly: true});
