...
test("a terminal display failure exposes a display-only retry button", () => {
	const displayRetries = [];
	const {controller, fakeDocument, calls} = createHarness({onRetryDisplay: channelId => displayRetries.push(channelId)});
	controller.update({channelId: "channel-1", active: false, done: false, phase: "failed", displayFailed: 1, retryable: 0});
	const button = getCapsule(fakeDocument).querySelector(".translator-loaded-status-retry");
	assert.ok(button, "64/65 with a display failure must offer recovery");
	button.onclick({stopPropagation() {}});
	assert.deepEqual(displayRetries, ["channel-1"]);
	assert.deepEqual(calls.retry, [], "retrying display must not buy another translation");
});

test("mixed failures route each retry to its own owner", () => {
	const displays = [];
	const {controller, fakeDocument, calls} = createHarness({onRetryDisplay: id => displays.push(id)});
	controller.update({channelId: "channel-1", active: false, phase: "failed", displayFailed: 1, retryable: 2});
	getCapsule(fakeDocument).querySelector(".translator-loaded-status-retry").onclick();
	assert.deepEqual(displays, ["channel-1"]);
	assert.deepEqual(calls.retry, ["channel-1"]);
});

test("a display retry remains available during other work without restarting active translations", () => {
	const displays = [];
	const {controller, fakeDocument, calls} = createHarness({onRetryDisplay: id => displays.push(id)});
	controller.update({channelId: "channel-1", active: true, phase: "requesting", displayFailed: 1, retryable: 2});
	const capsule = getCapsule(fakeDocument);
	assert.match(capsule.className, /translator-loaded-status-requesting/);
	capsule.querySelector(".translator-loaded-status-retry").onclick();
	assert.deepEqual(displays, ["channel-1"]);
