...
function configureHistoricalCoordinatorPlugin(options = {}) {
	...
	// Store batch commits resolve synchronously in coordinator tests; recorded calls and
	// committed views model the controller's atomic store acknowledgement without DOM timers.
	plugin.historicalDisplayBatchCommits = [];
	const historicalAcceptedViews = new Map();
	const getReceivedDisplayRuntimeView = plugin.getReceivedDisplayRuntimeView.bind(plugin);
	plugin.getReceivedDisplayRuntimeView = messageId => historicalAcceptedViews.get(String(messageId)) || getReceivedDisplayRuntimeView(messageId);
	plugin.commitHistoricalReceivedDisplayBatch = results => {
		...
		for (const result of results) historicalAcceptedViews.set(String(result.messageId), Object.assign({}, result, {translated: result.status === "translated", showLoading: false}));
		return Promise.resolve({committedIds, confirmedIds: committedIds, deferredIds: [], missingIds: [], retryIds: [], rejectedIds: [], staleIds: [], fallbackUsed: false});
	};
	return plugin;
}
...
test("a historical snapshot starts translating as soon as its configured limit is full", async () => {
	...
	plugin.setHistoricalBatchExperimentConcurrency(2);
	...
	plugin.requestAiBatchTranslation = (_engineKey, preparedItems) => {
		...
		return new Promise(resolve => pendingBatches.push({ids, resolve}));
	};
	...
	assert.equal(requestedIds.length, 2, "S7 starts both official cap2 chunks without a production probe wait");
	...
	for (const batch of pendingBatches) batch.resolve(Object.fromEntries(batch.ids.map(id => [id, `translated ${id}`])));
...
test("initial loaded-message pass stops at the configured historical job limit", async () => {
	...
	let commitCount = 0;
	const statuses = [];
	plugin.updateLoadedAutoTranslationStatus = update => statuses.push(update);
	...
	plugin.commitHistoricalTranslationJob = (summary, job, options) => {
		if (!options || !options.partial) commitCount++;
		return commitHistoricalTranslationJob(summary, job, options);
	};
	...
	assert.equal(commitCount, 1, "the job finalizes once after progressive blocks");
	assert.deepEqual(plugin.historicalDisplayBatchCommits.map(rows => rows.length), [10, 10, 10, 10, 10]);
	const committedIds = plugin.historicalDisplayBatchCommits.flat().map(row => String(row.messageId));
	assert.equal(new Set(committedIds).size, 50);
	assert.deepEqual(committedIds.slice().sort(), requestedIds.flat().map(String).sort());
	assert.equal(statuses.filter(update => update.done).length, 1);
});
...
test("initial loaded-message pass seals one 50-ID snapshot from rendered cached and prefetched messages without scrolling", async () => {
	...
	assert.deepEqual(plugin.historicalDisplayBatchCommits.map(rows => rows.length), [10, 10, 10, 10, 10]);
	const committedIds = plugin.historicalDisplayBatchCommits.flat().map(result => result.messageId);
	assert.deepEqual(committedIds, requestedIds.flat());
	assert.equal(new Set(committedIds).size, 50);
	assert.equal(statuses.filter(update => update.done).length, 1);
...
for (const queued of [false, true]) test(`initial history collection cannot replace the owner of a ${queued ? "queued" : "in-flight"} live translation`, async () => {
	const plugin = configureHistoricalCoordinatorPlugin();
	const live = createDeferred(), message = createMessage("live-owner", "Please review the updated schedule.");
	const channel = {id: message.channel_id}, source = {content: message.content, embeds: []};
	const queue = plugin.ensureLiveTranslationQueue();
	queue.setBusyTranslating(queued);
	let liveCalls = 0, historicalCalls = 0;
	plugin.translateMessage = () => {liveCalls++; return live.promise;};
	plugin.requestAiBatchTranslation = () => {historicalCalls++; return Promise.resolve({});};
	try {
		assert.equal(plugin.queueAutoTranslateMessage(message, channel, source), true);
		await new Promise(resolve => setImmediate(resolve));
		assert.equal(liveCalls, queued ? 0 : 1);
		const owner = queue.getQueuedMarker(message.id);
		assert.equal(queue.isRequestCurrent(owner, message), true);
		// The initial source scan calls the collector after an asynchronous prefetch,
		// bypassing the render queue's already-queued eligibility check.
		const accepted = plugin.collectHistoricalTranslationMessage({message, channel, originalContentData: source, historicalLoad: true, deferHistoricalSnapshotStart: true});
		assert.equal(accepted, false, "a current live request already owns this source");
		assert.equal(queue.getQueuedMarker(message.id), owner, "history must not overwrite the live marker");
		await plugin.startCollectedHistoricalTranslationJobs(channel.id);
		...
		assert.equal(queue.isMessageQueued(message.id), false, "live completion must release its exact marker");
	}
	finally {queue.setBusyTranslating(false); queue.processQueue(); live.resolve(true); plugin.cancelHistoricalTranslationJobs(channel.id, "test-cleanup"); await new Promise(resolve => setImmediate(resolve));}
});

test("a live request for an older source does not block history collection of an edited message", async () => {
	const plugin = configureHistoricalCoordinatorPlugin(), queue = plugin.ensureLiveTranslationQueue();
	const original = createMessage("edited-live-owner", "Please review the old schedule."), channel = {id: original.channel_id};
	queue.setBusyTranslating(true);
	try {
		assert.equal(plugin.queueAutoTranslateMessage(original, channel, {content: original.content, embeds: []}), true);
		const liveOwner = queue.getQueuedMarker(original.id), edited = {...original, content: "Please review the new schedule."};
		assert.equal(queue.isRequestCurrent(liveOwner, original), true);
		assert.equal(queue.isRequestCurrent(liveOwner, edited), false);
		assert.equal(plugin.collectHistoricalTranslationMessage({message: edited, channel, originalContentData: {content: edited.content, embeds: []}, historicalLoad: true, deferHistoricalSnapshotStart: true}), true);
		const historicalOwner = queue.getQueuedMarker(edited.id);
		assert.equal(historicalOwner.type, "historical");
		queue.finishRequest(liveOwner);
		assert.equal(queue.getQueuedMarker(edited.id), historicalOwner, "retiring the old source cannot erase the new history marker");
	}
	finally {queue.clearQueue(); queue.invalidateRequests(channel.id); queue.setBusyTranslating(false); plugin.cancelHistoricalTranslationJobs(channel.id, "test-cleanup");}
});
...
test("mixed historical cache hits reach the display store before the remaining provider reply", async () => {
	const plugin = configureHistoricalCoordinatorPlugin();
	const channelId = "channel-history-job", primary = createDeferred(), requestedIds = [];
	plugin.requestAiBatchTranslation = (_engineKey, preparedItems) => {
		requestedIds.push(preparedItems.map(item => item.message.id));
		return primary.promise;
	};
	for (const id of ["cached-1", "cached-2", "pending-1"]) {
		const content = `Source for ${id}.`;
		plugin.queueAutoTranslateMessage(createMessage(id, content), {id: channelId}, {content}, {
			historicalLoad: true,
			cachedTranslation: id.startsWith("cached") ? {
				signature: `signature-${id}`, channelId, auto: true,
				content: "缓存译文", translatedContent: "缓存译文", originalContent: content,
				input: {id: "en"}, output: {id: "zh-CN"}
			} : null
		});
	}
	...
	try {
		await new Promise(resolve => setImmediate(resolve));
		assert.deepEqual(requestedIds, [["pending-1"]]);
		assert.deepEqual(plugin.historicalDisplayBatchCommits.map(batch => batch.map(result => result.messageId)), [["cached-1", "cached-2"]]);
		assert.equal(plugin.getReceivedDisplayRuntimeView("cached-1").translated, true);
		assert.equal(plugin.isHistoricalMessagePending("pending-1", channelId), true);
	}
	finally {primary.resolve({"pending-1": "网络译文"}); await running;}
	assert.deepEqual(plugin.historicalDisplayBatchCommits.map(batch => batch.map(result => result.messageId)), [["cached-1", "cached-2"], ["pending-1"]]);
	assert.equal(requestedIds.length, 1, "cache progress adds no provider request");
	const run = plugin.getHistoricalBatchPerformanceSnapshot().recentRuns.at(-1);
	assert.equal(run.h1.cache.hit, 2);
	assert.equal(run.providerMessageCount, 1);
	assert.equal(run.committedCount, 3);
});
...
test("invalid batch items are repaired, but a skip verdict is terminal", async () => {
	...
	// P3-b keeps a short, name-like message that comes back in the wrong language instead of
	// repairing it, so the wrong-language item is prose to stay on the repair ladder.
	for (const id of ["invalid-skip", "invalid-language", "invalid-placeholder"]) {
		const content = id == "invalid-placeholder" ? "translate <@123456789>" : "please translate this whole sentence before the meeting starts";
		plugin.queueAutoTranslateMessage(createMessage(id, content), {id: "channel-history-job"}, {content}, {historicalLoad: true});
	}
...
test("S7 cap2 authentication failure drains its two first-wave attempts and performs zero repair", async () => {
	const plugin = configureHistoricalCoordinatorPlugin();
	const channelId = "channel-history-auth-cap2";
	plugin.setHistoricalBatchExperimentConcurrency(2);
	plugin.getReceivedAutoTranslateLoadedLimit = () => 50;
	let primaryCalls = 0;
	let repairBatchCalls = 0;
	let repairItemCalls = 0;
	plugin.requestAiBatchTranslationDetailed = () => {primaryCalls++; return Promise.resolve({translations: null, failureKind: "auth", statusCode: 401});};
	plugin.repairHistoricalTranslationJobBatch = () => {repairBatchCalls++; return Promise.resolve(null);};
	plugin.repairHistoricalTranslationJobItem = () => {repairItemCalls++; return Promise.resolve({status: "failed"});};
	for (let index = 0; index < 50; index++) {
		const message = createMessage(`auth-cap2-${index}`, `source auth cap2 ${index}`);
		plugin.queueAutoTranslateMessage(message, {id: channelId}, {content: message.content}, {historicalLoad: true, deferHistoricalSnapshotStart: true});
	}
	await plugin.startCollectedHistoricalTranslationJobs(channelId);
	assert.equal(primaryCalls, 2);
	assert.equal(repairBatchCalls, 0);
	assert.equal(repairItemCalls, 0);
	assert.equal(plugin.getFailedHistoricalTranslationCount(channelId), 50);
	assert.equal(plugin.getHistoricalBatchPerformanceSnapshot().physical.active, 0);
});
...
test("failed historical status exposes a visible retry action", () => {
	...
	try {
		...
		assert.equal(text.textContent, "0/2 0s");
		...
		assert.equal(text.textContent, "1/2 0s");
		...
		assert.equal(text.textContent, "2/2", "the finished pill closes the cumulative ratio and hides seconds");
...
test("plugin stop cancels pending historical work and ignores its late result", async () => {
	...
	const running = plugin.startCollectedHistoricalTranslationJobs("channel-history-job");
	await new Promise(resolve => setTimeout(resolve, 0));
	assert.equal(plugin.getHistoricalBatchPerformanceSnapshot().activeRunCount, 1);
	assert.equal(plugin.getHistoricalBatchPerformanceSnapshot().activeChunkCount, 1);

	plugin.onStop();
	assert.equal(plugin.getHistoricalBatchPerformanceSnapshot().activeRunCount, 0);
	assert.equal(plugin.getHistoricalBatchPerformanceSnapshot().activeChunkCount, 0);
	...
	assert.equal(plugin.getHistoricalBatchPerformanceSnapshot().completedRunCount, 0, "a late old-generation metric cannot resurrect after stop");
});
...
test("one historical job performs one acknowledged display commit", async () => {
	...
	plugin.commitHistoricalReceivedDisplayBatch = async results => {
		...
		const committedIds = results.map(result => String(result.messageId));
		return {committedIds, confirmedIds: committedIds, missingIds: [], fallbackUsed: false};
	};
	plugin.applyStoredTranslationToMessage = () => {throw new Error("historical automatic results must not write the legacy display map");};
	const messages = [createMessage("100", "first"), createMessage("200", "second"), createMessage("300", "third")];
	const summary = {
...
test("historical queued markers remain owned until the atomic commit acknowledgement", async () => {
	const plugin = configureHistoricalCoordinatorPlugin({scheduleAutomatically: true});
	const channelId = "channel-history-marker-commit";
	const message = createMessage("marker-commit", "source marker commit");
	plugin.getReceivedAutoTranslateLoadedLimit = () => 1;
	plugin.requestAiBatchTranslation = (_engineKey, items) => Promise.resolve(Object.fromEntries(items.map(item => [item.message.id, "translated marker"])));
	const commit = createDeferred();
	let commitStarted = false;
	plugin.commitHistoricalReceivedDisplayBatch = results => {
		commitStarted = true;
		return commit.promise.then(() => ({committedIds: results.map(result => String(result.messageId)), confirmedIds: results.map(result => String(result.messageId)), missingIds: [], fallbackUsed: false}));
	};
	plugin.queueAutoTranslateMessage(message, {id: channelId}, {content: message.content}, {historicalLoad: true, deferHistoricalSnapshotStart: true});
	const running = plugin.startCollectedHistoricalTranslationJobs(channelId);
	for (let turn = 0; turn < 10 && !commitStarted; turn++) await new Promise(resolve => setImmediate(resolve));
	assert.equal(commitStarted, true);
	assert.equal(plugin.ensureLiveTranslationQueue().isMessageQueued(message.id), true, "the exact job marker survives the display await");
	commit.resolve();
	await running;
	assert.equal(plugin.ensureLiveTranslationQueue().isMessageQueued(message.id), false);
});

test("a live marker that replaces the historical marker during commit acknowledgement survives job cleanup", async () => {
	const plugin = configureHistoricalCoordinatorPlugin({scheduleAutomatically: true});
	const channelId = "channel-history-marker-live";
	const message = createMessage("marker-live", "source marker live");
	plugin.getReceivedAutoTranslateLoadedLimit = () => 1;
	plugin.requestAiBatchTranslation = (_engineKey, items) => Promise.resolve(Object.fromEntries(items.map(item => [item.message.id, "translated marker"])));
	const commit = createDeferred();
	let commitStarted = false;
	...
	for (let turn = 0; turn < 10 && !commitStarted; turn++) await new Promise(resolve => setImmediate(resolve));
	assert.equal(commitStarted, true);
	const liveMarker = {type: "live", id: message.id, channelId, generation: 99};
	plugin.ensureLiveTranslationQueue().markMessageQueued(message.id, liveMarker);
	commit.resolve();
	await running;
	assert.equal(plugin.ensureLiveTranslationQueue().getQueuedMarker(message.id), liveMarker);
	plugin.ensureLiveTranslationQueue().clearQueuedMessage(message.id, liveMarker);
});
...
test("historical status excludes unconfirmed rejected retrying and stale translations", async () => {
	...
	const statusUpdates = [];
	plugin.isHistoricalTranslationJobItemCurrent = () => true;
	plugin.createReceivedTranslationSignature = message => `sig-${message.id}`;
	plugin.updateLoadedAutoTranslationStatus = update => {statusUpdates.push(update);};
	plugin.getReceivedDisplayRuntimeView = messageId => {
		const message = messages.find(candidate => String(candidate.id) === String(messageId));
		return message ? {messageId: String(message.id), generation: 1, sourceSignature: `sig-${message.id}`, status: "translated", translation: {content: `${message.content} translated`, translatedContent: `${message.content} translated`}} : null;
	};
	plugin.commitHistoricalReceivedDisplayBatch = async () => ({
		committedIds: ["confirmed", "deferred", "missing", "retry"],
...
test("a failed row made stale during commit acknowledgement is not resurrected in retry state or status", async () => {
	const plugin = configureHistoricalCoordinatorPlugin();
	const message = createMessage("stale-failed", "source before edit");
	const commit = createDeferred();
	const statusUpdates = [];
	plugin.isHistoricalTranslationJobItemCurrent = () => true;
	plugin.updateLoadedAutoTranslationStatus = update => {statusUpdates.push(update);};
	plugin.commitHistoricalReceivedDisplayBatch = () => commit.promise;
	const summary = {translated: [], skipped: [], failed: [{message, originalContentData: {content: message.content, embeds: []}, reason: "provider_failed"}]};
	const job = {id: "job-stale-failed", channelId: message.channel_id, generation: 1, items: new Map([[message.id, {source: {message}}]])};

	const running = plugin.commitHistoricalTranslationJob(summary, job);
	await new Promise(resolve => setImmediate(resolve));
	commit.resolve({committedIds: [message.id], confirmedIds: [], deferredIds: [], missingIds: [], rejectedIds: [], staleIds: [message.id], fallbackUsed: false});
	await running;

	assert.equal(plugin.getFailedHistoricalTranslationCount(message.channel_id), 0);
	assert.equal(statusUpdates.at(-1).failed, 0);
	assert.equal(statusUpdates.at(-1).retryable, 0);
});

test("a same-source live translation present before commit clears an older retry snapshot", async () => {
	const plugin = configureHistoricalCoordinatorPlugin();
	const message = createMessage("live-before-commit", "source for live winner");
	const originalContentData = {content: message.content, embeds: []};
	const signature = plugin.createReceivedTranslationSignature(message, message.channel_id, originalContentData);
	const item = {message, originalContentData, reason: "provider_failed"};
	plugin.ensureHistoricalJobRegistry().setFailedSnapshot(message.channel_id, {channelId: message.channel_id, items: [plugin.createHistoricalTranslationRetrySnapshot(item, message.channel_id)]});
	plugin.getReceivedDisplayRuntimeView = () => ({messageId: message.id, status: "translated", translated: true, sourceSignature: signature, translation: {content: "live winner", translatedContent: "live winner"}});
	const statusUpdates = [];
	plugin.updateLoadedAutoTranslationStatus = update => {statusUpdates.push(update);};
	const job = {id: "job-live-before-commit", channelId: message.channel_id, items: new Map([[message.id, {status: "failed", source: {message}}]])};

	await plugin.commitHistoricalTranslationJob({translated: [], skipped: [], failed: [item]}, job);

	assert.equal(plugin.getFailedHistoricalTranslationCount(message.channel_id), 0);
	assert.equal(statusUpdates.at(-1).retryable, 0);
	assert.equal(statusUpdates.at(-1).displayed, 1);
});

test("historical phase notifications reread queue ownership before reporting readiness", () => {
	const plugin = configureHistoricalCoordinatorPlugin();
	const job = plugin.createCollectedHistoricalTranslationJob("channel-history-job");
	job.add({message: {id: "translated"}});
	job.items.get("translated").status = "translated";
	job.state = "ready";
	const updates = [];
	plugin.updateLoadedAutoTranslationStatus = update => updates.push(plugin.getLoadedHistoricalWorkStatus(update.channelId));
	plugin.updateHistoricalTranslationJobStatus(job);
	assert.deepEqual(updates.at(-1).pendingMessageIds, ["translated"], "translated content without a store ACK is still pending");
	assert.equal(updates.at(-1).phase, "committing");
	job.state = "committed";
	plugin.updateHistoricalTranslationJobStatus(job);
	assert.equal(updates.at(-1).done, true, "committed notification refreshes the queue instead of retaining the old committing phase");
});
...
test("a successful atomic historical commit writes cache only after the committed view exists", async () => {
	const plugin = configureHistoricalCoordinatorPlugin();
	plugin.isHistoricalTranslationJobItemCurrent = () => true;
	plugin.commitHistoricalReceivedDisplayBatch = Object.getPrototypeOf(plugin).commitHistoricalReceivedDisplayBatch.bind(plugin);
	const message = createMessage("post-commit-cache", "source text");
	const originalContentData = {content: message.content, embeds: []};
	const signature = plugin.createReceivedTranslationSignature(message, message.channel_id, originalContentData);
	const persisted = [];
	plugin.persistTranslationCacheEntry = (...args) => persisted.push(args);
	const translation = {channelId: message.channel_id, auto: true, content: "translated text", translatedContent: "translated text", signature};
	const job = {id: "job-post-cache", channelId: message.channel_id, items: new Map([[message.id, {}]])};
	await plugin.commitHistoricalTranslationJob({translated: [{message, originalContentData, translation}], skipped: [], failed: []}, job);
	assert.equal(plugin.getReceivedDisplayView(message.id).translated, true);
	assert.equal(persisted.length, 1);
	assert.equal(persisted[0][0], message.id);
	assert.equal(persisted[0][1], signature);
	assert.equal(persisted[0][2].translatedContent, translation.translatedContent);
});
...
test("S5 historical repair bypasses the legacy global backoff and uses captured-key admission", async () => {
	// The legacy provider-wide window still serves live/manual requests. Historical
	// repair must reach its request boundary immediately so S5 admission can deny only
	// the unhealthy captured key while a healthy backup key remains dispatchable.
	const plugin = createPluginInstance({callSetLanguages: false});
	const waits = [];
...
test("the loaded-status capsule ticks processed counts while provider chunks settle", async () => {
	...
	assert.equal(pendingChunks.length, 2, "S7 uses both cap2 slots without waiting for chunk zero");
	...
	assert.equal(pendingChunks.length, 2, "the second official chunk was already in flight");

	pendingChunks[1].resolve(Object.fromEntries(pendingChunks[1].ids.map(id => [id, `translated ${id}`])));
	await plugin.waitForHistoricalTranslationJobs(channelId);
});

test("the explicit cap-1 fallback keeps sequential requests and progressively displays two atomic blocks", async () => {
	const plugin = configureHistoricalCoordinatorPlugin({scheduleAutomatically: true});
	plugin.setHistoricalBatchExperimentConcurrency(1);
	const channelId = "channel-history-f1h0";
	plugin.getReceivedAutoTranslateLoadedLimit = () => 20;
	plugin.isUserActivelyScrollingMessages = () => true;
	const pendingChunks = [];
	const progress = [];
	let doneCount = 0;
	plugin.updateLoadedAutoTranslationStatus = update => {
		if (update.done) doneCount++;
		if (Number(update.processed) > 0 && !update.done) progress.push(Number(update.processed));
	};
	plugin.requestAiBatchTranslation = (_engineKey, preparedItems) => new Promise(resolve => {
		pendingChunks.push({ids: preparedItems.map(item => String(item.message.id)), resolve});
	});
	...
	for (let index = 0; index < 20; index++) {
		const message = createMessage(`secret-id-${index}`, `private source ${index}`);
		plugin.queueAutoTranslateMessage(message, {id: channelId}, {content: message.content}, {historicalLoad: true, deferHistoricalSnapshotStart: true});
	}
	await new Promise(resolve => setImmediate(resolve));

	let snapshot = plugin.getHistoricalBatchPerformanceSnapshot();
	assert.equal(pendingChunks.length, 1, "cap=1 still starts exactly one physical chunk");
	assert.equal(snapshot.activeRunCount, 1);
	assert.equal(snapshot.activeChunkCount, 1);
	assert.equal(snapshot.maxActiveChunkCount, 1);
	assert.equal(snapshot.primaryChunkRequestCount, 1);

	pendingChunks[0].resolve(Object.fromEntries(pendingChunks[0].ids.map(id => [id, `translated ${id}`])));
	await new Promise(resolve => setImmediate(resolve));
	await new Promise(resolve => setImmediate(resolve));
	snapshot = plugin.getHistoricalBatchPerformanceSnapshot();
	assert.equal(pendingChunks.length, 2, "the second physical chunk starts only after the first settles");
	assert.equal(snapshot.activeChunkCount, 1);
	assert.equal(snapshot.maxActiveChunkCount, 1);
	...
	assert.deepEqual(pendingChunks.map(chunk => chunk.ids.length), [10, 10]);
	assert.ok(progress.includes(10));
	assert.deepEqual(plugin.historicalDisplayBatchCommits.map(rows => rows.length), [10, 10]);
	const committedIds = plugin.historicalDisplayBatchCommits.flat().map(row => String(row.messageId));
	assert.deepEqual(committedIds, pendingChunks.flatMap(chunk => chunk.ids));
	assert.equal(new Set(committedIds).size, 20);
	assert.equal(doneCount, 1);
	assert.equal(snapshot.activeRunCount, 0);
	assert.equal(snapshot.activeChunkCount, 0);
	assert.equal(snapshot.latestRun.concurrency, 1);
	assert.equal(snapshot.latestRun.collectedMessageCount, 20);
	assert.equal(snapshot.latestRun.providerMessageCount, 20);
	assert.equal(snapshot.latestRun.requestedChunks, 2);
	assert.equal(snapshot.latestRun.settledChunks, 2);
	assert.equal(snapshot.latestRun.maxActiveChunks, 1);
	assert.equal(snapshot.latestRun.atomicCommitCount, 2);
	assert.equal(snapshot.latestRun.commitResultCount, 20);
	assert.equal(snapshot.latestRun.status, "committed");
	assert.equal(snapshot.latestRun.activeChunksAtFinish, 0);
	const serialized = JSON.stringify(snapshot);
	assert.doesNotMatch(serialized, /private source|channel-history-f1h0|deepseek|secret-id/);
});

test("S7 shipped cap2 starts two official chunks immediately and progressively commits each of the 50 IDs once", async () => {
	const plugin = configureHistoricalCoordinatorPlugin({scheduleAutomatically: true});
	plugin.setHistoricalBatchExperimentConcurrency(2);
	const channelId = "channel-history-f1h1";
	plugin.getReceivedAutoTranslateLoadedLimit = () => 50;
	plugin.isUserActivelyScrollingMessages = () => true;
	const pendingChunks = [];
	let doneCount = 0;
	plugin.updateLoadedAutoTranslationStatus = update => {if (update.done) doneCount++;};
	...
	plugin.requestAiBatchTranslation = (_engineKey, preparedItems) => {
		...
		pendingChunks.push({ids: preparedItems.map(item => String(item.message.id)), deferred});
		return deferred.promise.finally(() => {physicalActive--;});
	};
	for (let index = 0; index < 50; index++) {
		const message = createMessage(`f1h1-${index}`, `historical concurrency fixture ${index}`);
		plugin.queueAutoTranslateMessage(message, {id: channelId}, {content: message.content}, {historicalLoad: true, deferHistoricalSnapshotStart: true});
	}
	await new Promise(resolve => setImmediate(resolve));
	assert.deepEqual(pendingChunks.map(chunk => chunk.ids.length), [10, 10], "the first wave uses both cap2 slots");
	pendingChunks[0].deferred.resolve(Object.fromEntries(pendingChunks[0].ids.map(id => [id, `translated ${id}`])));
	await new Promise(resolve => setImmediate(resolve));
	await new Promise(resolve => setImmediate(resolve));
	assert.deepEqual(pendingChunks.map(chunk => chunk.ids.length), [10, 10, 10]);
	pendingChunks[2].deferred.resolve(Object.fromEntries(pendingChunks[2].ids.map(id => [id, `translated ${id}`])));
	await new Promise(resolve => setImmediate(resolve));
	await new Promise(resolve => setImmediate(resolve));
	assert.equal(pendingChunks.length, 4);
	pendingChunks[1].deferred.resolve(Object.fromEntries(pendingChunks[1].ids.map(id => [id, `translated ${id}`])));
	await new Promise(resolve => setImmediate(resolve));
	await new Promise(resolve => setImmediate(resolve));
	assert.equal(pendingChunks.length, 5);
	for (const index of [4, 3]) pendingChunks[index].deferred.resolve(Object.fromEntries(pendingChunks[index].ids.map(id => [id, `translated ${id}`])));
	await plugin.waitForHistoricalTranslationJobs(channelId);
	const snapshot = plugin.getHistoricalBatchPerformanceSnapshot();
	assert.equal(physicalHighWater, 2);
	assert.equal(physicalActive, 0);
	assert.equal(snapshot.latestRun.concurrency, 2);
	assert.equal(snapshot.latestRun.requestedChunks, 5);
	assert.equal(snapshot.latestRun.settledChunks, 5);
	assert.equal(snapshot.latestRun.maxActiveChunks, 2);
	assert.equal(snapshot.latestRun.activeChunksAtFinish, 0);
	assert.equal(snapshot.physical.highWater, 2);
	assert.equal(snapshot.physical.active, 0);
	assert.deepEqual(plugin.historicalDisplayBatchCommits.map(rows => rows.length), [10, 10, 10, 10, 10]);
	const committedIds = plugin.historicalDisplayBatchCommits.flat().map(row => String(row.messageId));
	assert.equal(new Set(committedIds).size, 50);
	assert.deepEqual(committedIds.slice().sort(), pendingChunks.flatMap(chunk => chunk.ids).sort());
	assert.equal(doneCount, 1);
	assert.equal(snapshot.latestRun.atomicCommitCount, 5);
...
test("rows collected across ticks aggregate into one batch through the quiet-window seal", async () => {
	// Keep render ticks real so the old microtask seal would still fail this test, but
	// control BDFDB managed time: a loaded CI event loop can delay setImmediate past
	// the 500 ms window, so one tick is not evidence that two rows arrived within it.
	const {HISTORICAL_COLLECT_QUIET_MS} = require("../src/orchestrator/historical-translation-job");
	const timers = new Map();
	let now = 0, timerSequence = 0;
	const managedTime = {
		timeout(callback, delay) {const handle = ++timerSequence; timers.set(handle, {callback, at: now + delay}); return handle;},
		clear(handle) {timers.delete(handle);}
	};
	const advanceManagedTime = milliseconds => {
		now += milliseconds;
		for (const [handle, timer] of [...timers]) if (timer.at <= now && timers.delete(handle)) timer.callback();
	};
	const plugin = configureHistoricalCoordinatorPlugin({scheduleAutomatically: true, pluginOptions: {bdfdb: {TimeUtils: managedTime}}});
	...
	try {
		...
		const entry = plugin.getHistoricalTranslationJobQueue(channelId, false);
		const sealHandle = entry.startTimer;
		assert.equal(timers.get(sealHandle).at, HISTORICAL_COLLECT_QUIET_MS, "the real cadence arms its configured managed quiet-window timer");
		await new Promise(resolve => setImmediate(resolve));
		assert.equal(requestedIds.length, 0, "the first render tick must not seal a micro-batch");
		advanceManagedTime(HISTORICAL_COLLECT_QUIET_MS - 1);
		...
		await new Promise(resolve => setImmediate(resolve));
		assert.equal(requestedIds.length, 0, "both rows are still collecting just before the real quiet-window deadline");
		assert.equal(entry.startTimer, sealHandle, "the collecting batch retains its single managed seal");

		advanceManagedTime(1);
		await plugin.waitForHistoricalTranslationJobs(channelId);
		assert.equal(requestedIds.length, 1, "rows mounting across ticks within the quiet window share one provider batch");
		assert.deepEqual(requestedIds[0].slice().sort(), ["9001", "9002"]);
		assert.equal(plugin.historicalDisplayBatchCommits.length, 1, "one atomic display commit, not one commit per render tick");
		assert.deepEqual(plugin.historicalDisplayBatchCommits[0].map(result => String(result.messageId)).sort(), ["9001", "9002"]);
		assert.equal(timers.has(sealHandle), false, "the fired seal handle is released");
	}
	finally {
		plugin.cancelHistoricalTranslationJobs(channelId, "test-cleanup");
		timers.clear();
...
function createProgressiveCommitHarness(ids = ["fast", "slow"]) {
	const plugin = configureHistoricalCoordinatorPlugin({pluginOptions: {pluginPath: path.resolve(__dirname, "../src/legacy/runtime.js")}});
	const updates = [], cache = [], snapshots = [];
	plugin.updateLoadedAutoTranslationStatus = value => updates.push(value);
	plugin.persistTranslationCacheEntry = (...args) => cache.push(args);
	const updateSnapshots = plugin.updateFailedHistoricalTranslationSnapshots.bind(plugin);
	plugin.updateFailedHistoricalTranslationSnapshots = (summary, channelId) => {snapshots.push(summary); return updateSnapshots(summary, channelId);};
	const items = ids.map(id => {
		const message = createMessage(id, "source " + id), originalContentData = {content: message.content, embeds: []};
		return {message, originalContentData, translation: {channelId: message.channel_id, auto: true, content: "translated " + id, translatedContent: "translated " + id, signature: plugin.createReceivedTranslationSignature(message, message.channel_id, originalContentData)}};
	});
	const job = {id: "progress-job", channelId: items[0].message.channel_id, state: "translating", progressCommittedIds: new Set(), items: new Map(items.map(item => [item.message.id, {source: item, status: "translated"}]))};
	const summary = translated => ({translated, skipped: [], failed: []});
	return {plugin, updates, cache, snapshots, items, job, summary};
}

test("progressive commit shows a ready block without finalizing the job and final commit deduplicates its cache and display", async () => {
	const h = createProgressiveCommitHarness();
	const ack = await h.plugin.commitHistoricalTranslationJob(h.summary([h.items[0]]), h.job, {partial: true});
	assert.equal(h.updates.at(-1).done, false, "one ready block must not finish the whole history job");
	assert.equal(h.updates.at(-1).active, true);
	assert.equal(h.updates.at(-1).displayed, 1);
	assert.equal(h.snapshots.length, 0, "partial display must not finalize retry snapshots");
	assert.deepEqual(ack, {committedIds: ["fast"]});
	for (const id of ack.committedIds) h.job.progressCommittedIds.add(id);
	h.plugin.updateHistoricalTranslationJobStatus(h.job);
	assert.equal(h.updates.at(-1).jobId, h.job.id, "a phase callback identifies its job without publishing stale display counts");
	h.plugin.getHistoricalTranslationJobQueue(h.job.channelId).jobs.push(h.job);
	assert.deepEqual(h.plugin.getLoadedHistoricalWorkStatus(h.job.channelId).pendingMessageIds, ["slow"], "later status retains the accepted early result");
	await h.plugin.commitHistoricalTranslationJob(h.summary(h.items), h.job);
	assert.deepEqual(h.plugin.historicalDisplayBatchCommits.map(rows => rows.map(row => row.messageId)), [["fast"], ["slow"]]);
	assert.deepEqual(h.cache.map(row => row[0]), ["fast", "slow"]);
	assert.equal(h.updates.at(-1).done, true);
	assert.equal(h.updates.at(-1).displayed, 2);
	assert.deepEqual(h.snapshots.at(-1).translated.map(item => item.message.id).sort(), ["fast", "slow"]);
});


test("progressive commit rejects a late edited deleted stale-generation stale-job or manual-owned ACK without cache", async () => {
	for (const mode of ["edited", "deleted", "generation", "job", "manual"]) {
		const h = createProgressiveCommitHarness(["fenced"]), pending = createDeferred(), views = new Map();
		h.plugin.getReceivedDisplayRuntimeView = id => views.get(String(id)) || null;
		h.plugin.commitHistoricalReceivedDisplayBatch = results => {
			for (const result of results) views.set(String(result.messageId), {...result, translated: true, showLoading: false});
			return pending.promise;
		};
		const running = h.plugin.commitHistoricalTranslationJob(h.summary(h.items), h.job, {partial: true});
		await new Promise(resolve => setImmediate(resolve));
		if (mode === "edited") h.plugin._testBdfdb.LibraryStores.MessageStore = {getMessage: () => ({...h.items[0].message, content: "edited now"})};
		if (mode === "deleted") h.job.items.get("fenced").status = "cancelled";
		if (mode === "generation") views.get("fenced").generation++;
		if (mode === "job") h.plugin.isHistoricalTranslationJobCurrent = () => false;
		if (mode === "manual") views.get("fenced").origin = "manual";
		pending.resolve({committedIds: ["fenced"], confirmedIds: ["fenced"]});
		assert.deepEqual(await running, {committedIds: []}, mode);
		assert.equal(h.cache.length, 0, mode);
		assert.equal(h.snapshots.length, 0, mode);
	}
});


test("progressive commit final summary rechecks an earlier acknowledged source before counting it", async () => {
	const h = createProgressiveCommitHarness();
	const ack = await h.plugin.commitHistoricalTranslationJob(h.summary([h.items[0]]), h.job, {partial: true});
	for (const id of ack.committedIds) h.job.progressCommittedIds.add(id);
	h.job.items.get("fast").status = "cancelled";
	h.plugin._testBdfdb.LibraryStores.MessageStore = {getMessage: (_channelId, id) => id === "fast" ? {...h.items[0].message, content: "edited after early commit"} : h.items[1].message};
	await h.plugin.commitHistoricalTranslationJob(h.summary([h.items[1]]), h.job);
	assert.deepEqual(h.cache.map(row => row[0]), ["fast", "slow"], "the old cache write is not replayed");
	assert.deepEqual(h.snapshots.at(-1).translated.map(item => item.message.id), ["slow"]);
	assert.equal(h.updates.at(-1).displayed, 1, "an old early view must not masquerade as a live success");
});


test("progressive runtime wiring paints the fast primary block before its sibling and records compatibility fallback once", async () => {
	const plugin = configureHistoricalCoordinatorPlugin({pluginOptions: {pluginPath: path.resolve(__dirname, "../src/legacy/runtime.js")}});
	const channelId = "channel-progress-runtime", chunks = [], cache = [];
	plugin.setHistoricalBatchExperimentConcurrency(1);
	plugin.getReceivedAutoTranslateLoadedLimit = () => 20;
	plugin.persistTranslationCacheEntry = id => cache.push(id);
	plugin.requestAiBatchTranslation = (_engineKey, prepared) => {
		const pending = createDeferred();
		if (!chunks.length) for (const item of prepared) item.semanticCompatibilityFallback = true;
		chunks.push({ids: prepared.map(item => String(item.message.id)), pending});
		return pending.promise;
	};
	for (let index = 0; index < 20; index++) {
		const message = createMessage("progress-" + index, "historical progressive source " + index);
		plugin.queueAutoTranslateMessage(message, {id: channelId}, {content: message.content, embeds: []}, {historicalLoad: true, deferHistoricalSnapshotStart: true});
	}
	const running = plugin.startCollectedHistoricalTranslationJobs(channelId);
	try {
		for (let turn = 0; turn < 30 && !chunks.length; turn++) await new Promise(resolve => setImmediate(resolve));
		assert.equal(chunks.length, 1);
		chunks[0].pending.resolve(Object.fromEntries(chunks[0].ids.map(id => [id, "translated " + id])));
		for (let turn = 0; turn < 30 && !plugin.historicalDisplayBatchCommits.length; turn++) await new Promise(resolve => setImmediate(resolve));
		assert.equal(plugin.historicalDisplayBatchCommits.length, 1, "fast validated block paints while slow primary is still pending");
		assert.equal(chunks.length, 2, "display handoff does not hold the physical primary slot");
		assert.equal(plugin.historicalDisplayBatchCommits[0].length, 10);
		const early = plugin.getTranslationTerminalLedgerSnapshot().recent.filter(route => route.outcome === "translated");
		assert.equal(early.length, 10);
		for (const route of early) {
			assert.equal(route.historyPath.filter(value => value === "primary:settled").length, 1);
			assert.deepEqual(route.providerRoles, {primary: 1, fallback: 1});
		}
		chunks[1].pending.resolve(Object.fromEntries(chunks[1].ids.map(id => [id, "translated " + id])));
		await running;
		assert.deepEqual(plugin.historicalDisplayBatchCommits.map(rows => rows.length), [10, 10]);
		assert.deepEqual(cache, chunks[1].ids, "compatibility fallback early block retains its no-cache contract");
		const perf = plugin.getHistoricalBatchPerformanceSnapshot();
		assert.equal(perf.latestRun.requestedChunks, 2);
		assert.equal(perf.latestRun.atomicCommitCount, 2);
		assert.equal(perf.latestRun.commitResultCount, 20);
		assert.equal(perf.physical.active, 0);
	} finally {
		for (const chunk of chunks) chunk.pending.resolve(Object.fromEntries(chunk.ids.map(id => [id, "translated " + id])));
		await running;
	}
});


test("progressive accepted ACK survives throwing cache status ledger and performance side effects", async () => {
	const {createHistoricalCommitCacheFinalizer} = require("../src/orchestrator/historical-commit-cache-finalizer");
	for (const mode of ["cache", "status", "ledger", "performance"]) {
		const h = createProgressiveCommitHarness(["durable"]), fail = () => {throw new Error(mode + " observer");};
		const performance = {recordAtomicCommit() {}, recordCache() {}, recordDomConfirm() {}};
		const finalizer = createHistoricalCommitCacheFinalizer({BDFDB: h.plugin._testBdfdb, performance, displayTracker: {begin: () => 0}});
		if (mode === "cache") h.plugin.persistTranslationCacheEntry = fail;
		if (mode === "status") h.plugin.updateLoadedAutoTranslationStatus = fail;
		if (mode === "ledger") {
			h.job.items.get("durable").source.terminalRouteId = "diagnostic-route";
			h.plugin.recordHistoricalAutoCheckpoint = fail;
			h.plugin.finishTranslationTerminalRoute = fail;
		}
		if (mode === "performance") for (const key of Object.keys(performance)) performance[key] = fail;
		assert.deepEqual(await finalizer.commit(h.plugin, h.summary(h.items), h.job, {partial: true}), {committedIds: ["durable"]}, mode);
		assert.equal(h.plugin.getReceivedDisplayRuntimeView("durable").status, "translated", mode);
	}
});


test("progressive final tracker merges only still-pending early ACKs without replacing prior blocks", async () => {
	const h = createProgressiveCommitHarness(["mounted", "pending", "tail"]), views = new Map(), scheduled = [];
	h.plugin.getReceivedDisplayRuntimeView = id => views.get(String(id)) || null;
	h.plugin.scheduleReceivedDisplayFlush = (_channel, id, _delay, trackingKey) => scheduled.push([id, trackingKey]);
	h.plugin.commitHistoricalReceivedDisplayBatch = async results => {
		for (const result of results) views.set(String(result.messageId), {...result, translated: true, renderStatus: result.messageId === "tail" ? "confirmed" : "unconfirmed"});
		const ids = results.map(result => String(result.messageId));
		return {committedIds: ids, confirmedIds: ids.filter(id => id === "tail"), missingIds: ids.filter(id => id === "mounted"), retryIds: ids.filter(id => id === "pending")};
	};
	for (const item of h.items.slice(0, 2)) {
		const ack = await h.plugin.commitHistoricalTranslationJob(h.summary([item]), h.job, {partial: true});
		for (const id of ack.committedIds) h.job.progressCommittedIds.add(id);
	}
	assert.deepEqual(scheduled, [["mounted", h.job.id], ["pending", h.job.id]], "each early block begins its existing bounded display retries immediately");
	views.get("mounted").renderStatus = "confirmed";
	h.plugin.handleHistoricalDisplayOutcome({channelId: h.job.channelId, messageIds: ["mounted"], trackingKeysByMessageId: {mounted: [h.job.id]}, outcome: {confirmedIds: ["mounted"]}});
	await h.plugin.commitHistoricalTranslationJob(h.summary(h.items), h.job);
	assert.deepEqual(scheduled, [["mounted", h.job.id], ["pending", h.job.id]], "finalization does not restart retries for early rows");
	assert.equal(h.updates.at(-1).displayed, 2);
	assert.equal(h.updates.at(-1).displayPending, 1);
	assert.deepEqual(h.cache.map(row => row[0]), ["mounted", "pending", "tail"]);
});
